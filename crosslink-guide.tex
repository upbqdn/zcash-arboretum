...
own ZIP draft is a Google document that ``will enter the formal ZIP
...
\paragraph{Current-head boundary (2026-08-30).} The four Crosslink design
...
\textsf{crosslink-protocol-guide} at \texttt{b77d845}. Current upstream
\textsf{zips} \texttt{ad30e59} still contains no Crosslink or NU8 ZIP. The
repository-backed design and classification below therefore remain current;
web-sourced rollout statements retain their explicit 2026-07-15 access date.
...
%% 100-block figure is zcashd lineage,
...
unspecified}, proved in book prose only for the Prefix Agreement half (the
Final Agreement proof is defective, \S\ref{sec:xl-sources}) and fixed by no
normative artifact.
...
\paragraph{Deployment status.} As verified on 2026-07-15, Crosslink is
...
the coin-weighted vote fixes its electorate at
...
series constructs in the \emph{Ironwood Guide} (\S``The note and its
commitment''); nothing below alters its note
machinery.
...
transaction/\allowbreak check.rs} at \texttt{6d02a1b8}).
...
(\texttt{Electric-Coin-Company/tfl-book} @ \texttt{fe6e1d6f},
...
defer the proof of branch (i), which runs through the Local fin-depth
lemma and the Linear prefix lemma, to the security analysis
...
Theorem~\ref{thm:xl-af-two-sided} is completely stated, but only branch (i)
carries a correct written proof; branch (ii)'s pasted proof is defective
(Remark~\ref{rem:xl-final-agreement-proof});
...
\texttt{FinalBlockRx} is a broadcast receiver). The RPC methods, six of
the ten doc-commented as ``Placeholder''
...
Crosslink. The \emph{Ironwood Guide}'s anchor-depth discussion (\S``Resting:
...
\S``Resting: the note in the commitment tree'', paragraph ``Anchor choice,
...
theorem twice over, from two independent assumptions --- though the second
statement's pasted proof is defective
(Remark~\ref{rem:xl-final-agreement-proof}) --- and then breaks off:
...
is reconciled with the policy-not-limit framing in
\S\ref{sec:xl-node-interface}.
...
horizon governing anchor robustness (\S``Resting: the note in the commitment
tree''). Crosslink would add a
