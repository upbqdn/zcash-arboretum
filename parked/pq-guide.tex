...
the vulnerable public-key primitives. The Orchard-core analysis is the
\emph{Ironwood Guide}'s; this
...
\emph{Ironwood Guide}'s (\S``The horizon: quantum adversaries and
recoverability''), the cryptographic definitions are the \emph{Crypto
...
\S``The horizon: quantum adversaries and recoverability''.}
...
spending --- are the \emph{Ironwood Guide}'s (\S``The retroactive break:
harvest now, decrypt later''). Three
...
viewing key (\emph{Ironwood Guide}, \S``The doors: shielding, coinbase, migration''),
...
\S``Prospective breaks: the discrete-log integrity stack'').
\end{itemize}
...
quantum collision algorithms have different time and memory costs (the
...
recoverability'') --- the $\mathtt{0x0B}$-tagged
