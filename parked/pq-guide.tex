...
\thispagestyle{empty}
\begin{abstract}
This research snapshot of \emph{The Zcash Arboretum} assembles the post-quantum
...
switching off in time). It illustrates a conditional rate-only mining
model through the
...
contain, at the pinned commits, no deployed post-quantum replacement for
the vulnerable public-key primitives. The Orchard-core constructions are the
\emph{Ironwood Guide}'s and the Orchard-core attack analysis is ZIP-2005's;
this volume cites both and extends the frame to everything else.
\end{abstract}
\clearpage
\tableofcontents
\clearpage
...
re-derivation: the Orchard core's constructions and the assumptions its
security rests on are the \emph{Ironwood Guide}'s (\S``The end-to-end
theorem''), their attack analysis is in ZIP-2005, ``Effects of
discrete-logarithm-breaking attacks before the switch to the Recovery
Protocol'', the cryptographic definitions are the \emph{Crypto Guide}'s,
and this volume constructs only what no lower volume owns ---
...
the discrete-logarithm problem in polynomial time in the efficiently
represented finite cyclic groups used here, elliptic-curve subgroups
included, taking DLP, CDH, and DDH with it (\emph{Crypto
Guide}, \S``The quantum caveat: Shor's algorithm''); every
security property in Zcash that requires hard discrete logarithms fails
against it. Other properties of the same object, such as commitment
hiding, need not fail. Grover's algorithm
gives a quadratic speedup on unstructured search: exhaustive key and
preimage searches retain half their classical security exponent. This
does not describe collision search, whose quantum bounds differ
(\S\ref{sec:pq-backbone}). The standing premise,
...
failure \emph{points in time} is the operational recasting of the attack
analysis in ZIP-2005, ``Effects of discrete-logarithm-breaking attacks after
the switch to the Recovery Protocol'', and it turns the asymmetry into a
test: a break
...
Table~\ref{tab:pq-sources} lists the sources. Protocol claims distinguish
ZIP-2005's Proposed status from the cited Final ZIPs; the NIST standards are
...
Worked mining and byte-size numbers were computed by script
(\texttt{.wip/pq-drafts/compute.py}) and independently checked in
\texttt{research/check\_frontier.py}; deployed-behaviour claims cite crate, file,
...
or ``lattice''; the only post-quantum implementation found in the
checkouts surveyed for that pinned inventory was an inert,
feature-gated transitive dependency
...
Orchard-core rows compress ZIP-2005's attack analysis of the constructions
of the \emph{Ironwood Guide}; the remaining rows are this volume's. The
subsections walk the table by failure mode.
...
Curve25519 DLP & statements and signatures of JoinSplits (Sprout's
shielded transfers, spending and creating notes under one proof)
forgeable & prosp. \\
...
Section~\ref{sec:pq-2005}). Orchard-core rows: ZIP-2005, ``Effects of
discrete-logarithm-breaking attacks before the switch to the Recovery
Protocol''; constructions and assumptions: \emph{Ironwood Guide},
\S``The end-to-end theorem''.}
...
forever. The two structural limits --- the attack is keyed by the
address, and it confers viewing rather than spending --- follow from the
break's need for a known receiver address (ZIP-2005, ``Effects of
discrete-logarithm-breaking attacks before the switch to the Recovery
Protocol'') and from what the incoming viewing key can derive
(\emph{Ironwood Guide}, \S``Key capabilities and scope''). Three
...
viewing key (\emph{Ironwood Guide}, \S``Chain state and pool rules''),
...
\item \emph{The scope of the address requirement.} The attack just
described uses a known address. It does not establish secrecy against an
unbounded adversary: finite-entropy keys and symmetric ciphertexts remain
subject to exhaustive search, and metadata can disclose information
independently. Perfect hiding of ideal Pedersen commitments and statistical
zero knowledge of proofs do not turn the entire recorded protocol into an
information-theoretically private object (\emph{Ironwood Guide},
\S``The end-to-end theorem'', Remark ``Nature of the guarantees'').
\end{itemize}

\subsection{The prospective integrity stack}\label{sec:pq-prosp}

Every other broken row fails forward. For the Orchard core, every
soundness reduction of the \emph{Ironwood Guide} ends in a discrete
logarithm on Pallas and is conditional on the knowledge soundness of the
Action proof (\S``The end-to-end theorem'', Remark ``Nature of the
guarantees''), so the dominoes are Sinsemilla binding, value commitments,
RedPallas, and the IPA's knowledge soundness, the last the single most
concentrated failure point because every soundness guarantee is
conditional on it. The
...
(\textsf{libzcash\_script},
\texttt{depend/\allowbreak zcash/\allowbreak src/\allowbreak pubkey.cpp:23--42},
driven from \texttt{zebra-script/\allowbreak src/\allowbreak lib.rs:62--76}).
Exposure is
...
only a consensus change moves it. ZIP-271's 78{,}750~ZEC one-time
disbursement, by contrast, creates 2-of-3 P2SH multisig outputs whose
extended public keys are published in the ZIP itself, so hash-hiding
provides those outputs no pre-spend quantum cover. This does not assert
that the original outputs remain unspent.
...
Sprout signature layers are forgeable. A BLS12-381 discrete-log solver
breaks the Sprout and Sapling proof systems, each under its own
verification key; analogous attacks on Pallas and Vesta compromise
Orchard and Ironwood's integrity stack.
...
quantum collision algorithms have different time and memory costs
(ZIP-2005's ``Adaptation to the quantum setting'' carries the careful
statement, including its best-known-not-provably-best flag). The
load-bearing consequence, relied on everywhere in
Section~\ref{sec:pq-2005} (ZIP-2005, ``Attacks addressed by the Recovery
Protocol''; \emph{Ironwood Guide}, \S``The note seed: Ironwood derivations
(lead byte $\mathtt{0x03}$)'', Remark ``Purpose of the hashed trapdoor''):
because key derivation is purely symmetric, \emph{ownership remains
provable after the assumption that made it enforceable has fallen} ---
the seed still derives the keys; what is lost is only the DL-based
machinery for exercising them.
...
Aggarwal, Brennen, Lee, Santha, and Tomamichel (arXiv:\allowbreak
1710.10377, 2017)
...
For an illustrative rate-only model, assume independent memoryless block
discoveries and represent the advantage by an effective speed multiplier
$g$ on the attacker's raw hashing. Then the \emph{Consensus Guide}'s
hashrate share becomes
\[
...
Under that additional stochastic assumption, the volume's double-spend
probability $r(q,n)$ and confirmation tables apply to $q_{\mathrm{eff}}$.
Table~\ref{tab:pq-qeff} computes this conditional illustration by script;
it is not a derived quantum-mining security bound.
...
\bottomrule
\end{tabular}
\caption{A rate-only illustration assuming independent memoryless block
discoveries: a per-unit mining speedup $g$ applied to an attacker's raw
...
at the illustrative value $g = 2$, a $25\%$ raw
...
$1\%$ raw share. Those conclusions are conditional on the rate-only
model; they do not establish a quantum double-spend bound when discovery
times or measurement strategies depart from its assumptions.
...
less) is unanalysed here. It is one reason the reduction
to $q_{\mathrm{eff}}$ is not exact; its size is not bounded here. Sattath's
``On the insecurity of quantum Bitcoin mining'' (arXiv:1804.08118)
analyses strategic measurement and correlated discoveries, illustrating why
a memoryless rate substitution needs a separate justification.
\end{remark}
...
Guide}, \S``The safety theorems and their assumptions''). In that
theoretical construction, a complete break of BFT signatures need not
destroy the proof-of-work safety branch, provided its other assumptions
and objective-validity rules continue to hold. This does not transfer
the theorem automatically to the divergent prototype, nor turn the
rate-only illustration above into a quantum PoW bound. The BFT upgrade
itself is lost when an adversary can forge its quorum evidence.
...
soon as'' one exists. Post-quantum threshold-signature research does
exist --- for example Tang et al., \emph{Efficient Lattice-Based
Threshold Signatures with Functional Interchangeability}, ePrint
2024/1067 --- but the pinned Zcash design specifies no integrated
replacement. Its derivation-agreement gap (participants must privately
agree a seed $\mathsf{sk}$, without a merged normative protocol) also
remains open; the \emph{FROST Guide} records an unmerged contributory
generation draft in ZIP-312 PR~\#895.
The frontier inherits the core's
strategy and its unfinished edges both.

\section{ZIP-2005 in the cross-layer frame}\label{sec:pq-2005}

The pinned design answer to this recovery question is one Proposed ZIP
(ZIP-2005, ``Proposed Recovery Protocol'' and ``QROM lift of the security
arguments''): the $\mathtt{0x0B}$-tagged
hashed derivation that makes each Ironwood note's commitment
\emph{conditionally} post-quantum-binding (\emph{Ironwood Guide},
\S``The note seed: Ironwood derivations (lead byte $\mathtt{0x03}$)''),
the $\mathsf{qsk}$ path for non-seed keys, the non-normative Recovery
Statement, and the QROM security bounds. This volume adds only the
...
The honest summary follows ZIP-2005's own framing, promoted here to the
...
transaction table), the $32$-byte ephemeral key (\emph{Wallet Guide}),
...
The byte counts give only an encoding thought experiment. Replacing RedPallas with
...
(\emph{Wallet Guide}). These substitutions do not define a working protocol:
ML-DSA supplies neither RedPallas key re-randomisation nor the binding
signature's Pedersen-balance relation, and a KEM integration needs new
address, note-encryption, and circuit rules. Public-key distribution and
those circuit changes are excluded from the byte estimate.
...
and a Merkle path (ZIP-2005, ``Cost'') --- is small precisely
...
by no protocol code. No deployed replacement for Zcash's vulnerable
public-key primitives was identified at these pins; the symmetric and hash
constructions above already supply surviving post-quantum assumptions.
...
This parked research snapshot sits outside the active series roster. It
owns nothing
...
Guide}'s and \emph{Halo 2 Guide}'s; the Orchard-core constructions and the
note derivation on which ZIP-2005's mechanism rests are the \emph{Ironwood
Guide}'s, and the Orchard-core attack analysis is ZIP-2005's; the
