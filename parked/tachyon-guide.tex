...
The core sibling volumes document deployed mechanisms and distinguish
their implementation state from the status of the corresponding ZIPs.
At this volume's July 2026 pins, Tachyon has neither a consensus
specification nor a merged core ZIP or a deployed network implementation.
...
most unambiguous support (\S\ref{sec:tachyon-standing}); the design
deserves scrutiny \emph{before} it hardens into consensus rules, without
inferring an upgrade assignment from that support. Second, its
...
\paragraph{Current-head boundary (2026-08-30).} The detailed design below
remains a reproducible snapshot of the commits in
Table~\ref{tab:tachyon-sources}, not a reconstruction of later main branches.
For current status, we also checked \textsf{tachyon} \texttt{fd022416}
(2026-08-27), \textsf{ragu} \texttt{f4f6315b} (2026-08-22), and upstream
\textsf{zips} \texttt{ad30e59}. The Tachyon README still lists note
commitments, nullifier derivation, and proof creation, merging, verification,
and stamp compression as stubbed. Ragu has added extensive formal-verification
and fuzzing material and published crate documentation, but still labels
itself unaudited, under heavy development, and unsuitable for production.
Upstream \textsf{zips} still contains no core Tachyon ZIP. Thus the status
classification has not changed; later implementation and book details are not
silently projected backward onto the pinned construction.

\paragraph{Substrate update (2026-09-05).} ZIP~248 merged on
2026-09-03 (upstream \textsf{zips} commit \texttt{e753a6a3}), assigning
the extensible transaction format to v7. This fulfils one revisit trigger:
the July snapshot's repeated statements that ZIP~248 is unmerged are
historical, not current status. It does not itself specify Tachyon's core
protocol or validate the mock-based construction documented here.

\paragraph{NU7 scope update (2026-09-23).} The numbered deployment draft,
\href{https://zips.z.cash/zip-0259}{ZIP~259}, merged on 2026-09-22
(\textsf{zips} \texttt{afa086bd};
\href{https://github.com/zcash/zips/pull/1363}{PR~1363}). It selects
25-second blocks and shielded action limits, halving-preserving reissuance,
and rejection of version~4 transactions. It keeps transaction versions~5
and~6 and introduces no new format. Tachyon, memo bundles, and ZIP~248's
extensible V7 format are outside this scope, not withdrawn by that
decision. NU7 activation heights remain TBD. The February poll and empty
deployment draft discussed in the July snapshot are historical evidence,
not the current upgrade definition.
...
citing the founding blog post. At that July pin, the unnumbered NU7
deployment draft selected no feature ZIPs. It is no longer the current
scope: ZIP~259 selects a bounded upgrade without Tachyon, as recorded above.
...
not scheduling: the later NU7 deployment draft, ZIP~259, does not include
Tachyon (\S\ref{sec:tachyon-sources}).
...
editorships above,
the adjacent network-upgrade context (NU6.2 and NU6.3 active, although the
ZIP-258 document remains Draft), the
...
delegation, aggregation, the distinct shielded pool the notes inhabit ---
every part of it cited to the book at \texttt{26fdc165} as non-normative,
dated, and in places already
...
tree (the \emph{Ironwood Guide}, \S``Trial decryption and note acceptance''
and \S``Authentication paths''; the deployed
...
commitment is $\cmsym$ was minted --- its creation \emph{stamp}, the proof
object recording its transaction's published commitments and nullifiers
(Definition~\ref{def:tachyon-design-bundle}), is folded into the
...
wallet-supplied nullifier absent from one stamp's tachygram set (a
\emph{tachygram} is a 32-byte field element representing a note commitment
or a nullifier, the two indistinguishable to consensus),
...
(\texttt{crates/\allowbreak ragu\_pcd/\allowbreak src/\allowbreak
\{lib.rs,\allowbreak fuse/mod.rs,\allowbreak verify.rs\}} at commit
...
(\emph{Ironwood Guide}, \S``Trial decryption and note acceptance''). Under
Tachyon the wallet never
...
on demand from retained subtree roots (\emph{Ironwood Guide},
\S``Authentication paths''; the design of
...
A node today tracks the append-only commitment tree, from which it
recognises anchors, and the ever-growing nullifier set,
...
(\emph{Ironwood Guide}, \S``Verification of a transaction''). The logical
commitment tree grows, but maintaining its root requires only a frontier,
not all leaves; stored historical anchors and
archival blocks are separate costs. The Tachyon design replaces the
tree-root state with one running Poseidon hash
...
$\mathsf{Poseidon}_{\texttt{Tachyon-EpochStp}}\allowbreak(\mathsf{anchor},\
e + 1)$,
...
current and the immediately preceding epoch, rejects on a hit (book,
\texttt{src/tachygrams.md} at \texttt{26fdc165}), and may permanently
prune tachygrams older than that window (forum t/50789 \#22, 2025-11-04;
the book states no pruning rule). Two epochs suffice in the
...
(book, \texttt{src/tachygrams.md} at \texttt{26fdc165}; the dual
publication itself is in \texttt{src/nullifiers.md}).

\paragraph{Classification.} The window rule and prunability are
\emph{designed but unspecified}: the window rule appears in the book only,
prunability only in a forum statement (t/50789 \#22, 2025-11-04); the
eprint states just that ``validators must still check for duplicates''; no
...
\S``Note encryption''): how does the recipient learn of
the note? The project's
...
flows are thus to use private information retrieval (PIR), a query
protocol in which the server learns nothing about which record the client
fetched, rather than sender-to-recipient out-of-band delivery --- the
roadmap adds a post-quantum key exchange for the channel --- while
in-band secret distribution returns,
...
outputs come from Ragu's provisional \texttt{PoseidonFp} instantiation
(width $T = 5$, rate $4$; the mock sponge evaluates the real permutation
out-of-circuit), not from any Tachyon-pinned Poseidon instance. (The
book's $\F_p$, $\F_q$ are the series' $\F_{p_{\Pallas}}$,
$\F_{p_{\Vesta}}$.)
...
\texttt{wtxid} (the ZIP~239 identifier \texttt{txid} $\|$
\texttt{auth\_digest}; \S\ref{sec:tachyon-design-txid}) of the covering
aggregate: the all-zero \texttt{wtxid},
naming no aggregate, is rejected on read even for a stripped bundle
whose action vector is empty, and
...
\emph{Ironwood Guide}, \S``Action descriptions and bundles''); a tachyaction
is single-effect, ciphertext-free, does not carry its tachygram, and its
proof is strippable and aggregatable across
...
lines 83--98). The protocol does not use $\ask$ to sign directly or
$\aksym$ to verify directly: both are randomized per action into
$\rsk$ and $\rk$
...
normalization, and RedPallas re-randomization survive (the \emph{Ironwood
Guide}, \S``Keys and addresses''; tag bytes
...
\emph{Ironwood Guide}, \S``Notes and note commitments'') and load-bearing
zero-value dummies (\emph{Ironwood Guide},
\S``Actions, bundles, and dummy notes''); Tachyon drops the address and
...
chains $\rho^{\mathrm{new}} := \nfsym^{\mathrm{old}}$ (\emph{Ironwood
Guide}, \S``Nullifier chaining''), $\psi$ reuse
...
$D$ base-$A$ direction chunks, most significant first, and
...
\S``Nullifiers''); Tachyon replaces it with
...
sequence polynomial nonzero (an empty window is the constant $1$) and
encodes its exact length. It does not mathematically guarantee a
nonidentity Pedersen commitment: a nonzero coefficient vector can lie
in the commitment map's kernel. Finding such a vector should be
computationally infeasible under the binding assumption; an explicit
identity check, if required by the circuit representation, is a separate
condition. The source comment in \texttt{primitives/seq.rs} conflates
these properties. Step \texttt{NullifierStep} builds its single-leaf commitment
...
\emph{before} the challenge, and each identity is linear in them. Soundness
also needs binding commitments, enforced degree bounds, and an appropriate
challenge distribution; linearity alone is not a proof. Step
\texttt{MergeStamp} fuses two stamps at
...
\emph{Ironwood Guide}, \S``The note commitment tree''); the
no-tree, proven-once replacement is stated at
...
\emph{Ironwood Guide}, \S``The Action circuit and the Halo 2 proof'').
...
\[
\begin{aligned}
\alpha_{\mathrm{spend}} &= \mathrm{ToScalar}\bigl(
  \text{BLAKE2b-512}_{\texttt{Tachyon-Spend}}(\theta \,\|\, \cmsym)\bigr),\\
\alpha_{\mathrm{output}} &= \mathrm{ToScalar}\bigl(
  \text{BLAKE2b-512}_{\texttt{Tachyon-Output}}(\theta \,\|\, \cmsym)\bigr).
\end{aligned}
\]
Here $\mathrm{ToScalar}$ reduces the little-endian $64$-byte digest
modulo the Pallas scalar-field order (\texttt{digest/blake2b.rs}).
With fresh per-action entropy, repeated signing keys occur only with
negligible probability. Each action's $\rsk$ is published as
...
generators (the \emph{Ironwood Guide}, \S``Value commitments and the binding
signature'') --- while the randomizer differs
in kind: Orchard draws $\alpha$ uniformly at random per Action with no
planner/custody split (\emph{Ironwood Guide}, \S``Randomised validating
keys''), and every Orchard Action is a paired
spend-plus-output carrying one \textsf{SpendAuthSig} (\emph{Ironwood
Guide}, \S``Actions, bundles, and dummy notes''; \S``Verification of a
transaction'').
...
\emph{Ironwood Guide}, \S``Transaction digests and signatures'');
the
...
--- parameters, rate, and capacity exist in no Tachyon artifact (the
crate computes every domain through Ragu's mock sponge, inheriting its
provisional \texttt{PoseidonFp}, width $T = 5$, rate $4$).
...
relay of pruned transactions forbidden (lines 391--434); and a fee treatment
...
\texttt{src/mock/mod.rs} @ \texttt{830bbcda}); it performs no proof
cryptography, though its \texttt{Sponge} does evaluate the real ragu
Poseidon permutation out-of-circuit.
...
\paragraph{What would falsify the design.} Cost: basic single-server PIR
constructions scan the database per query; preprocessing and amortisation
can change that cost, so the unspecified PIR family matters. A database
that grows with chain history can make detection more expensive than the
trial decryption Tachyon set out to eliminate. Privacy: query-pattern or
timing leakage at the PIR server
...
obsolete; delegated absence proofs avoid periodic note expiry. The
concrete GGM index space is nevertheless finite, as
\S\ref{sec:tachyon-design-ggm} records.
...
format the scan runs over; and a soundness statement with a proof. It
must compose two defences: concurrent or neighbouring-epoch spends
collide in the two-epoch scan, while a later spend whose lineage crosses
an earlier spend cannot prove the required historical nullifier absence.
Far-apart spend pairs need not share a scan window. The combined
statement must survive composition with the recursive proof system's
actual extraction bound.

\paragraph{What would falsify the design.} A double-spend across the
pruning boundary: two accepted spends of one note that evade both the
live duplicate scan and the historical absence-proof checks. The window
and absence lineage together replace the permanent validator nullifier
set; a counterexample --- or a
...
accurate statement is: the service learns the nullifiers supplied for
the delegated span, including its tip. Unlinkability is intended only
for other, unshared epochs; those supplied values may be past, current,
or future when the service observes them.
...
The relocation exposes three separate requirements, not a proven
pick-two impossibility theorem. A seed-derived Orchard wallet that scans
an available full chain locally aims to provide:
\begin{enumerate}[label=(\roman*)]
...
\item \emph{chain as backup}: a seed phrase plus the public chain
  recover notes belonging to that seed's supported derivation paths;
  imported keys and non-seed custody require their own backups.
\end{enumerate}
Validator pruning does not itself force any particular pair to fail:
independent archival replication could preserve chain history, and even
Orchard seed restoration depends on access to that history. The sharper
Tachyon question is who retains the data needed to rebuild an absence
lineage when validators need not retain it. A wallet can preserve its own
proof state, but that state is not reconstructed from a seed alone.
Recovery delegated to a small set of services creates an additional
availability concentration: ``the sudden shutdown of these services could render
...
The project's own trajectory changes where recovery data is retained.
The May-2026 pivot retains ``in-band secret distribution for recover
...
Classification of this comparison: the framing is this volume's, and
does not prove that only two requirements can coexist. A specification
must state its archival and service assumptions because they determine
what a wallet must store ---
...
The post-quantum analysis of deployed Orchard (ZIP 2005, Rationale, on the
effects of discrete-logarithm-breaking attacks before and after the switch to
the Recovery Protocol) reaches one headline
conclusion: exactly one quantum
...
published, that particular DH ciphertext is absent from the harvest
surface. It does not follow that all remaining on-chain objects are
quantum-private: the commitments, proofs, and key derivations need their
own security arguments. Three qualifications
...
finds Crosslink only in ZIP 218 (25-second block target spacing, Status:
Draft, created 2026-03-13; zips @ \texttt{c6b70358}, searched 2026-07-08):
one body sentence declaring the proposal ``complementary to, and does not
compete with,'' finality mechanisms such as Crosslink, plus the citation
footnote. The independence is therefore
...
2005; Draft/Draft/Proposed). The current NU7 draft,
\href{https://zips.z.cash/zip-0259}{ZIP~259}, includes neither Tachyon nor
Crosslink; its shorter block spacing does not deploy either design.
The February 2026 support for Tachyon is therefore not an upgrade
assignment. At the July source pin, the
ZIP corpus contained no NU8 artifact of any kind (repository-wide search of
...
proofs exists, the advertised scaling factors remain unvalidated.
