...
tree (the \emph{Ironwood Guide}, \S``Delivery: the note reaches its owner'' and
\S``Resting: the note in the commitment tree''; the deployed
...
(\emph{Ironwood Guide}, \S``Delivery: the note reaches its owner''). Under
...
on demand from retained subtree roots (\emph{Ironwood Guide}, \S``Resting: the
note in the commitment tree''; the design of
...
note''). The logical commitment tree grows, but maintaining its root
requires only a frontier, not all leaves; stored historical anchors and
...
\S``Delivery: the note reaches its owner''): how does the recipient learn of
...
Guide}, \S``The recipient: keys and addresses''; tag bytes
...
\emph{Ironwood Guide}, \S``The note and its commitment'') and load-bearing
...
Guide}, \S``The nullifier, and the loop back to minting''), $\psi$ reuse
...
\S``The nullifier, and the loop back to minting''); Tachyon replaces it with
...
\emph{Ironwood Guide}, \S``Resting: the note in the commitment tree''); the
...
generators (the \emph{Ironwood Guide}, \S``Conservation without disclosure:
value commitments and the binding signature'') --- while the randomizer differs
in kind: Orchard draws $\alpha$ uniformly at random per Action with no
planner/custody split (\emph{Ironwood Guide}, \S``Spend authorisation:
RedPallas re-randomisation''), and every Orchard Action is a paired
spend-plus-output carrying one \textsf{SpendAuthSig} (\emph{Ironwood
Guide}, \S``One Action, one payment: the walkthrough''; \S``What a node
...
and the binding signature'', subsection ``What the signatures sign: the
ZIP-244 digest and its v6 form'');
the
...
\S``The horizon: quantum adversaries and recoverability'') reaches one headline
