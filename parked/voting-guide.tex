...
\emph{Ironwood Guide} (\S``Note commitments'', \S``The nullifier'',
\S``Keys and addresses'', \S``The Merkle hash and the tree'', \S``The RedPallas
signature scheme''), and reused unchanged through the
...
\item \emph{Real nullifier} (condition~12). The circuit derives the note's
  ...
  (\S``The nullifier'') --- in-circuit and
...
\emph{Ironwood Guide} (\S``The nullifier'') ---
\[
\nfsym_{\mathrm{signed}} =
...
\S``Randomised validating keys''); condition~5 ties $\aksym$,
...
Guide} (\S``Note commitments'', \S``The nullifier'', \S``Keys and
addresses'', \S``The Merkle hash and the tree'', \S``The RedPallas signature
scheme''), and reused unchanged through the
\texttt{unstable-voting-circuits} feature (zcash/orchard @
