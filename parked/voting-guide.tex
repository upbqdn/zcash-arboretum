...
\title{\textbf{\Huge Voting Guide}\\[6pt]\large The May--June 2026 Zally shielded-voting deployment snapshot}
\author{m@rek.onl}
\date{}
\begin{document}
\maketitle
\thispagestyle{empty}
\begin{abstract}
This volume of \emph{The Zcash Arboretum} documents the commit-pinned
May--June 2026 shielded-voting deployment snapshot. No specification of the
protocol has merged anywhere; the commit-pinned implementation is the de facto
...
assumptions, the quantisation gap between the published description and
the circuit, and an unestablished delegation-signature binding.
\end{abstract}
\clearpage
\tableofcontents
\clearpage
...
``Zally'', shipped to wallet users as \emph{Coinholder Polling}. The
protocol lets a holder of Orchard notes vote with the weight of their unspent
funds, privately: the funds never move, note and address openings remain
hidden, and weights are encrypted. Individual share decisions do become
public at reveal; the trust map below qualifies the privacy claim. The
internal name appears
...
(ibid., entry \texttt{[3.5.2 (1742)]}). The intended first production use
--- the NU7 sentiment coinholder poll, announced 2026-05-15 with a
2026-06-10 open, its close extended on 2026-05-29 from June 24 to June 29
to end with the parallel ZCAP poll of the same questions --- never ran. On
2026-06-07 --- three days before the open, the day after the NU7 scope
collapsed to the Ironwood supply-soundness upgrade --- the poll was paused
``for both coin holders and the Zcash Foundation ZCAP'', and as of
2026-06-14 the next round ``is delayed until after Ironwood activates''.
No results exist for that unheld June round. A production round was
provisioned in the signed round
registry on 2026-06-05 (\S\ref{sec:vote-trust-map}), but no public
evidence shows a ballot cast (searched 2026-07-15).\footnote{Announcement:
joshs, ``NU7 Sentiment Polling: Questions for Community Review \&
Coinholder Voting via Zodl'', forum topic 55713, 2026-05-15; extension:
ibid.\ post 71, 2026-05-29; pause: ibid.\ post 73, 2026-06-07. Wayback
snapshot of the topic carrying the pause notice, 2026-07-15:
\url{https://web.archive.org/web/20260715193300/https://forum.zcashcommunity.com/t/nu7-sentiment-polling-questions-for-community-review-coinholder-voting-via-zodl/55713};
pre-pause schedule (RSS snapshot, 2026-06-05):
\url{https://web.archive.org/web/20260605074855/https://forum.zcashcommunity.com/t/nu7-sentiment-polling-questions-for-community-review-coinholder-voting-via-zodl/55713.rss}.
Scope change: ``Ironwood: Verifying the Soundness of Zcash's Circulating
Supply'', topic 56044, 2026-06-06, Wayback 2026-07-15:
\url{https://web.archive.org/web/20260715193458/https://forum.zcashcommunity.com/t/ironwood-verifying-the-soundness-of-zcash-s-circulating-supply/56044}.
Deferrals: ZODL updates, topic 56072, 2026-06-07, Wayback 2026-07-15:
\url{https://web.archive.org/web/20260715193328/https://forum.zcashcommunity.com/t/subduing-the-world-update-from-zodl/56072};
and topic 56211, 2026-06-14, Wayback 2026-07-15:
\url{https://web.archive.org/web/20260715193352/https://forum.zcashcommunity.com/t/irrational-thinking-zodl-update/56211}.}

This is not the end of the NU7 polling history. The August--September
rounds have \href{https://forum.zcashcommunity.com/t/nu7-sentiment-polls-results/57590}{published results};
\href{https://zips.z.cash/zip-0259}{ZIP~259}, merged on 2026-09-22,
records their role in the present NU7 scope (\textsf{zips}
\texttt{afa086bd}, checked 2026-09-23). Those later rounds must not be
confused with the unheld June round or used to attribute their deployed
implementation to this volume's May--June code snapshot.
...
eligibility queries by single-server PIR (private information retrieval:
the server answers without learning which entry was queried), a dedicated
chain, and hardware signing, and it lives inside the production wallet
...
weighting; the June 2026 NU7 question was scheduled through both
instruments in parallel and paused for both together
(\S\ref{sec:vote-shipped}), which makes the distinction operational, not
academic. Hopwood's
draft \texttt{draft-ecc-\allowbreak onchain-\allowbreak
accountable-\allowbreak voting.md} (created 2025-02-21,
...
preimages (the arithmetisation-friendly hash constructed in the
\emph{Crypto Guide}, \S``Arithmetisation-friendly hashing: Poseidon''),
but omit the governance-nullifier, VAN-nullifier, and
...
\paragraph{Current-head boundary (2026-08-30).} The implementation has moved
substantially since this snapshot. Current heads are
\textsf{voting-circuits} \texttt{bd2531f} (release~0.11.2),
\textsf{vote-sdk} \texttt{7cf9b9c}, \textsf{zcash\_voting}
\texttt{4535b0d}, and \textsf{vote-nullifier-pir} \texttt{20e9c4a}
(release~0.11.2). The current delegation builder requires Ironwood V3 notes,
the circuit verifying keys and integration APIs have changed, and the client,
chain, and PIR services have gone through several coordinated release lines.
The one-ballot under-claim window proved in this volume remains explicitly
documented in the current circuit. Unless a paragraph names one of these later
heads, every formula and deployment-behaviour claim below is about the pinned
snapshot in Table~\ref{tab:vote-sources}; this is historical protocol
documentation, not a compatibility guide for the current stack.
...
(\texttt{verify\_\allowbreak delegation\_\allowbreak proof},
\texttt{verify\_\allowbreak vote\_\allowbreak proof}, and
\texttt{verify\_\allowbreak share\_\allowbreak reveal\_\allowbreak
proof}, each in its circuit directory's
...
invokes it. At matching circuit versions and configurations, wallet and
chain share the same relation implementation. That reduces one source of
drift but is not independent evidence of correctness. No independent
second implementation is identified at these pins that could catch a
...
\emph{Ironwood Guide} (\S``The note and its commitment'', \S``The nullifier,
and the loop back to minting'', \S``The recipient: keys and addresses'',
\S``Resting: the note in the commitment tree'', \S``Spend authorisation:
RedPallas re-randomisation''), and reused unchanged through the
...
delegation weight is constructed. Separately, the chain verifies a
client-supplied delegation sighash without establishing its binding to
the proved delegation fields (\S\ref{sec:vote-keystone}). This is not a
claim of a demonstrated live exploit, but it prevents treating the
intended hardware-wallet authorisation as established by these checks.
...
bin. The quantisation mismatch is one of this volume's open problems;
we state and prove its exact extent where it arises.
...
The vote chain is a Cosmos-SDK application, and an ordinary transaction
there is a fee-paying envelope of messages signed by an on-chain account.
The three voter-facing messages (2--4) are not ordinary Cosmos
transactions: they bypass that envelope entirely, and the chain
...
(\texttt{vote-sdk/\allowbreak x/\allowbreak vote/\allowbreak
keeper/\allowbreak msg\_\allowbreak server\_\allowbreak
coordinator\_\allowbreak actions.go:294-298}).
...
lifted-ElGamal ciphertext $C_1 \,\|\, C_2$ (ElGamal with the plaintext
in the exponent, Definition~\ref{def:vote-elgamal}) as compressed Pallas
points ---
...
unenforced (\texttt{x/\allowbreak vote/\allowbreak keeper/\allowbreak
msg\_\allowbreak server\_\allowbreak tally\_\allowbreak
decrypt.go:77-79};
...
\subsection{Eligibility: the note-bundle relation
(\texorpdfstring{ZKP\,1}{ZKP 1})}
\label{sec:vote-zkp1}
...
proves the conjunction --- one Halo\,2/IPA circuit (the proof system and
inner-product commitment of the \emph{Halo 2 Guide}, \S``The Halo 2 proof
system'', \S``The inner-product polynomial commitment'') over Pallas at
$K=14$ ($16{,}384$ rows) with $14$ public inputs
...
9--15'' --- yet the condition-by-condition definitions in both files stop
at 14: the README's numbered sections run 1 through 14, and the sole
``condition~15'' in the sources is a synthesis-stage comment
(\texttt{circuit.rs:1089-1098}) labelling the padded slots' zero value ---
a builder convention with no corresponding constraint. We state the total
as fourteen --- eight keystone-global conditions plus six per-slot
conditions instantiated five times --- and record the ``15'' headline
counts and the cond-15 comment as stale.}
(\texttt{src/\allowbreak delegation/\allowbreak
circuit.rs:1497-1805}):
\begin{itemize}
...
\item \emph{Real nullifier} (condition~12). The circuit derives the note's
  ...
  (\S``The nullifier, and the loop back to minting'') --- in-circuit and
...
excludes, assuming the correct canonical snapshot tree, the nullifiers
revealed on the Zcash chain up to the snapshot height: the note was
unspent when the snapshot was taken. It
...
overlap. Second, the span bound: to support the $250$-bit offset checks
throughout each interval, the builder guarantees
$\nfsym_{\mathrm{hi}} - \nfsym_{\mathrm{lo}} \le 2^{250}$ in canonical
field order by pre-populating $33$ sentinel
...
\Sinsemilla{} hash of an Orchard frontier (the incremental-tree append
state constructed in the \emph{Crypto Guide}, \S``Append-only and
incrementally updatable trees'') fetched from a public lightwalletd
server; the nullifier root \texttt{nullifier\_imt\_root} is
...
\texttt{src/\allowbreak delegation/\allowbreak README.md}, Inputs), and
one ballot represents
...
between it and the stated floor-division rule is an
...
$(\gd, \pkd)$ without either point's sign. Delegation additionally binds
the full points through \NoteCommit{}
...
\emph{Ironwood Guide} (\S``The nullifier, and the loop back to minting'') ---
\[
\nfsym_{\mathrm{signed}} =
\mathsf{DeriveNullifier}_{\nksym}\bigl(\rho_{\mathrm{signed}},\
\psi_{\mathrm{signed}},\ \cmsym_{\mathrm{signed}}\bigr),
\]
and exposes it as a public input. An honest client includes this
nullifier in the wrapping Action before obtaining a signature. The chain,
however, verifies the RedPallas signature under $\rk$ over the
\emph{client-provided} sighash and separately verifies the delegation
proof (\texttt{x/vote/ante/validate.go}, \texttt{verifyDelegation}, at
\texttt{cb915f51}). It receives no wrapping transaction from which to
recompute that sighash. The proof's fourteen public inputs include the
nullifier and $\rk$, but not the sighash
(\texttt{voting-circuits/src/delegation/circuit.rs}, at
\texttt{4c39abd5}). Consequently the circuit's $\rho$-binding alone does
not prove that the signed digest commits to this delegation. Sharing
$\rk$ between two verification checks is not a message-binding argument.
The chain fixture generator confirms that the sighash may be arbitrary
$32$ bytes (\texttt{vote-sdk/circuits/tests/generate\_fixtures.rs}).
This is an unestablished authorisation binding at the cited pins, not a
demonstration of an exploit against a live round. A protocol argument or
additional verification would have to close this boundary before the
intended hardware-wallet consent guarantee could be asserted.

The remaining circuit conditions are supporting:
condition~4 proves $\rk = [\alpha]\,\mathsf{SpendAuthG} + \aksym$ for a
witnessed randomizer $\alpha$ (the \emph{Ironwood Guide},
\S``Spend authorisation: RedPallas re-randomisation''); condition~5 ties $\aksym$,
...
BLAKE2b-512 PRF (pseudorandom function; the keyed-BLAKE2 construction of
the \emph{Crypto Guide}, \S``PRFs from hash functions: length extension
and keyed BLAKE2'') with personalisation \texttt{ZcashVote\_Expand} under
...
\texttt{src/delegation/README.md}, \S 8). This is an open
...
non-participants suspended from the active validator set
(\texttt{x/vote/keeper/keeper\_ceremony.go}, lines 46--79;
\texttt{keeper\_ceremony\_jailing.go}).
...
contributor sees every prior commitment and can adapt its contribution.
The repository's design document argues that the resulting bias is
harmless for threshold ElGamal, citing Gennaro et al.\ (2007)
(\texttt{vote-sdk/docs/tss-ceremony.md}, ``Why single-phase''). Two
shortcuts in that explanation must not be read as proofs. Biasing a key
distribution does not by itself let a party choose an arbitrary final
public key while supplying valid shares with known openings. Nor does
ElGamal confidentiality hold for every valid key regardless of its
generation: an adversary that knows the corresponding secret can decrypt.
The cited security argument's assumptions must be matched to this
ceremony, including its abort, acknowledgement, and validator-removal
rules. That composition is not established by this volume.
...
decrypt every individual on-chain ciphertext. If they additionally learn
which sixteen shares belong to a vote (for example through a helper),
summing them yields that vote's exact per-decision weight, which the cast
...
least $t$ stored partials at zero (the coefficients $\lambda_i$ below are
the Lagrange basis polynomials of the \emph{Math Guide}, \S``Lagrange
interpolation'', evaluated at zero),
\[
...
the instruments scheduled for the same June 2026 question. File citations
below are at the commits of Table~\ref{tab:vote-sources}; wallet files
are at \texttt{zashi-android} @ \texttt{b4edd6f6}.
...
(\texttt{vote-sdk/\allowbreak x/\allowbreak vote/\allowbreak
keeper/\allowbreak keeper\_\allowbreak ceremony.go}). Any $t$ of them
can
...
(\texttt{vote-sdk/\allowbreak app/\allowbreak
ante\_whitelist.go}). Round lifecycle, privileged
funding, and software upgrades are threshold-gated \emph{coordinator
actions} proposed and approved by a vote-manager set
(\texttt{MsgPropose\allowbreak Coordinator\allowbreak Action},
\texttt{MsgApprove\allowbreak Coordinator\allowbreak Action});
...
In production the vote-manager custodians are named --- by the
configuration repository, not by the chain. At the volume's pinned commit
of \texttt{valargroup/token-holder-voting-config} (\texttt{2785311d},
2026-05-27; the authentication root discussed below), the
\texttt{trusted\_keys} notes of \texttt{prod/static-voting-config.json}
label three Keplr-derived keys as vote-manager keys for stated
\texttt{sv1} addresses --- key ids \texttt{valargroup}, \texttt{tachyon},
and \texttt{zodl}; a fourth, \texttt{shielded-labs}, arrived on 2026-05-29
(commit \texttt{82711181}), and all four are present at head
\texttt{e7d11990} (2026-06-25). These annotations are the custodians'
claims about themselves; the deployed chain's on-chain
\texttt{VoteManagerAddresses} set is published nowhere we could find. The
production coordinator threshold is not publicly established: the
repository carries no genesis file, no announcement states the value, and
the registry's only production round entry ---
\texttt{3fda6c83b054\ldots}, added 2026-06-05 by pull request \#330 ---
carries a single signature (key id \texttt{valargroup}), consistent with
a threshold of one at the configuration layer but an inference, not a
source. No validator identity is published either: the closest public
artifact is the ten-operator \texttt{vote\_servers} list of
\texttt{prod/dynamic-voting-config.json} (at \texttt{e7d11990}), and a
vote server is an RPC endpoint, not evidence that its operator runs a
validator. We searched public artifacts on 2026-07-15; the deployed
chain's public REST endpoint would settle threshold and validator set
directly and remains unqueried.\footnote{Static configuration at the
pinned commit, Wayback snapshot 2026-07-15:
\url{https://web.archive.org/web/20260715193658/https://raw.githubusercontent.com/valargroup/token-holder-voting-config/2785311d/prod/static-voting-config.json};
at head, via the served URL:
\url{https://web.archive.org/web/20260715193616/https://voting.valargroup.org/prod/static-voting-config.json};
dynamic configuration at \texttt{e7d11990}:
\url{https://web.archive.org/web/20260715193707/https://raw.githubusercontent.com/valargroup/token-holder-voting-config/e7d11990/prod/dynamic-voting-config.json}.
Operators named in \texttt{vote\_servers}: valargroup, ruzcash, zend,
ShieldedLabs-JM, Legends, stardust-staking, zodl, ZecHub, katz, Keplr ---
a list not maintained as a roster: the 2026-06-14 ZODL update reports the
``decommissioned Stardust servers'' removed from the app while
\texttt{stardust-staking} remains in the configuration of 2026-06-25.
Both files follow the schema of draft ZIP 1244, ``Shielded Voting Wallet
API'' (zcash/zips PR \#1244).}
...
the custodians are named only by the configuration's own
\texttt{trusted\_keys} annotations (above), the distribution channel in
...
\texttt{DefaultTallyTimeout}) --- and end-of-block processing then finalizes it
...
the protocol specification and the ZIPs, constructed in the \emph{Ironwood
Guide} (\S``The note and its commitment'',
\S``The nullifier, and the loop back to minting'', \S``The recipient: keys and
addresses'', \S``Resting: the note in the commitment tree'', \S``Spend
authorisation: RedPallas re-randomisation''), and reused unchanged through the
\texttt{unstable-voting-circuits} feature (zcash/orchard @
...
\emph{client-provided} sighash, whose binding to the delegation is
unestablished here, while the cast-vote sighash is recomputed on-chain
...
\paragraph{Open problems.} First, the ballot-quantization relation. The
...
The deviation is self-harming, but it is a concrete
...
shipped weaker relation --- is unresolved. Second, the delegation
authorisation checks do not establish that the signed client-supplied
digest commits to the proved fields (\S\ref{sec:vote-keystone}). This
volume no longer treats the $\rho$-chain as a proof of that missing link.
...
\item the authorization model: which sighashes the chain recomputes,
  which it accepts from the client, and how the signed delegation digest
  is bound to the fields accepted on-chain;
\item the Election Authority: the threshold and quorum formulas, key
  registration and rotation, and an
...
configuration root. Aggregate decryption is publicly checkable: the
tally check executes on-chain, so observers can replay it. That does not
by itself establish the authorisation of every accepted delegation, and
a fabricated snapshot is only auditable after the fact. Individual privacy is
...
published analysis states. The provisioned June 2026 use was a
non-binding sentiment poll, but it had not occurred in this snapshot
(\S\ref{sec:vote-shipped}). This review does not certify suitability for
that use or for binding governance: the delegation-signature binding,
the specification above, independent verification, and the Election
Authority's confidentiality analysis remain to be established.
