...
The later \href{https://zips.z.cash/zip-0259}{NU7 deployment draft,
ZIP~259} (\textsf{zips} \texttt{afa086bd}, checked 2026-09-23) does not
include Crosslink. Its selection of ZIP~218's 25-second spacing changes
the timing of the underlying proof-of-work chain, not its finality model;
activation heights remain TBD. The prototype parameters below retain their
dated source context and are not NU7 consensus constants.
...
The interface-stability goal above motivates citing the deployed-stack
volumes; it does not guarantee that an eventual implementation preserves
every interface. Anchor semantics and reorg behaviour of the commitment
...
The paper uses $(1/3,1/2)$ as threshold labels. They must not be read as
security at equality: the tolerable Byzantine fractions are strictly
below $1/3$ and $1/2$, respectively. Its shorthand adversary notation
does not override the impossibility statement it gives immediately
before it; concrete finite rosters also require the appropriate integer
quorum bounds.
...
power expects to build a 10-block fork in 100 original-network block
times at unchanged difficulty, and the $k$-block tail
...
\texttt{fe6e1d6f}, \linebreak\texttt{security-analysis.md}]
...
$\mathsf{isValid}(\allowbreak\mathit{bft\_store},\allowbreak
...
In this book's model, ``honest at time $t$'' means having followed the
protocol at every time up to and including $t$, not merely behaving
honestly at that instant (\texttt{construction.md}:53).
...
\href{https://zips.z.cash/protocol/protocol.pdf\#coinbasetransactions}{\S``Coinbase Transactions''}). Later uses of the predicate refer only to the
...
\emph{or} $\Pi_{\mathrm{origbc}}$. Its restriction to $i = j$ says that
one node's observations are pairwise compatible, not that they advance
monotonically: a rollback along the same branch preserves compatibility.
The source's claimed equivalence to Local finalization linearity
(\texttt{construction.md}:508) therefore needs the separate no-rollback
update rule. That rule guarantees local monotonicity independently;
agreement between different nodes is the additional safety property.

\paragraph{Why $\mathsf{candidate}(H)$, not
$\mathsf{snapshot}(\mathsf{LF}(H))$.} The Last Final Snapshot rule
guarantees $\mathsf{snapshot}(\allowbreak\mathsf{LF}(H))\allowbreak
\preceq_{\mathrm{bc}} H$,
...
The ``otherwise'' arm is needed because after the best chain switches
...
\href{https://zips.z.cash/protocol/protocol.pdf\#diffadjustment}{\S\,diffadjustment}), the truncated best chain $\mathsf{ch}_i^t
\lceil^{\mu}_{\mathrm{bc}}$ might not descend from $\mathsf{fin}_i^t$
even with all safety assumptions satisfied. The source says that
$\mathsf{fin}$ switches forks, but the no-rollback update rule prohibits
that; the mutable best-chain view is the relevant object. The definition also makes the
...
on snapshot equality, looser on tail size, excluding BFT forks, and silent on
...
\item \emph{Header-end inconsistency.} A source comment claims that
  in-memory order is ``reversed from the specification'', but the actual
  \texttt{FindBlockHeaders} path returns headers after the intersection
  in increasing height order, deepest first. The accessors disagree:
  ...
  walk-back order''. For a stable tip at $T$, the supplied headers span
  $T-\sigma+1$ through $T$: the accessor selects $T$, the handler selects
  $T-\sigma+1$, and the book's snapshot is their predecessor
  $T-\sigma$ (\texttt{zebra-state/src/service/read/find.rs},
  \texttt{collect\_chain\_headers}; \texttt{chain.rs}:52--54, 124--126;
...
value pool accounting (\href{https://zips.z.cash/protocol/protocol.pdf\#chainvaluepoolbalances}{\S4.17}); the Orchard side of that balance is the
...
value fields under \href{https://zips.z.cash/protocol/protocol.pdf\#chainvaluepoolbalances}{\S4.17} is unspecified. The April prototype sidestepped
...
(\texttt{book/src/design/analysis/\allowbreak
history/\allowbreak 2025-12-11-mech-design/}
...
activation, affected finalizers leave the active roster and tracked bonds
that were actively delegated to them during the preceding 300-block
analysis window are marked burned. A delegation interval opened and
closed within one block is dropped from this index; marking a bond
burned does not claw back an already completed withdrawal. One such rule
ships in the feature-net executable. Selection of
...
\path{zebra-crosslink/zebra-state/src/service/finalized_state/zebra_db/slashing.rs};
the burn operation itself is in
...
Begin-unbonding requires 75 blocks since creation, and withdrawal
requires 75 blocks since begin-unbonding; retargeting does not reset that
clock. Except at five hard-coded feature-net heights, every
...
\item The staking transaction format: how a revealed, quantized,
  ...
  protocol specification \href{https://zips.z.cash/protocol/protocol.pdf\#chainvaluepoolbalances}{\S4.17} (\S\ref{sec:xl-stake-actions}).
...
$\mathsf{localfin}$ updates (\texttt{construction.md}:462--488).
Single-node Assured Finality means compatible observations; monotonicity
additionally follows from the update rule, not from compatibility alone
(the source's equivalence claim at line~508 is too strong). Local
monotonicity does not establish agreement across nodes.
...
\paragraph{Latency, and which view an application reads.} The old
analysis estimates a finalized view about $\mu + 1 + \sigma$ bc-blocks
behind the tip (tfl-book \texttt{security-analysis.md}:265--269), but
that passage lies below the ``Not updated for Crosslink~2'' marker and
uses the earlier $\mathsf{LOG}_{\mathrm{fin},\mu}$ construction.
Crosslink~2's $\mathsf{fin}$ does not depend on the per-node $\mu$;
the old formula is not a validated latency bound for it. The book
states the application-facing
...
are additionally provisional: block target spacing is 75\,s before NU7,
while \href{https://zips.z.cash/zip-0259}{ZIP~259 (Draft)} selects
\href{https://zips.z.cash/zip-0218}{ZIP~218}'s 25\,s target at NU7.
Neither draft deploys Crosslink or assigns its production parameters; a
fixed block count must not silently retain its old wall-clock duration.
...
safety assumption alone. A safety failure therefore requires both
premises to fail, although failure of both does not by itself imply an
attack succeeds. This is the conditional guarantee a wallet's ``final''
badge would rest on. We
...
the time it is received''). At the final height, the queried hash is
compared with the stored final hash; below it, with the best-chain hash
at that height. Equality returns \texttt{Finalized}, and inequality
\texttt{CantBeFinalized}
(\texttt{zebra-crosslink/zebra-crosslink/src/lib.rs}:1611--1655).
This implements a local status lookup, not a proof of the SL~book's
global-consistency claim; best-chain membership and stored finality must
themselves remain coherent for that interpretation to hold.
...
\item Transaction finality cannot express \texttt{CantBeFinalized}.
  ...
  $\texttt{tx.height} \le \texttt{final\_height}$, returns
  \texttt{NotYetFinalized} for a best-chain transaction above that height,
  and returns \texttt{None} when the transaction lookup fails; a literal
  ...
  \path{zebra-crosslink/zebra-state/src/request.rs}). The separate blocking
  block-notification method built on \texttt{FinalBlockRx} retries
  \texttt{NotYetFinalized}, but returns immediately on \texttt{None}
  or either terminal status
...
--- evaluated against the transaction's candidate block ancestry, not
against a node's truncated $\mathsf{localba}$ display. No
...
\begin{itemize}
\item \emph{Anchor only in $\mathsf{localfin}$.} Newly received notes
  must wait for their treestate to finalize before they can be spent.
  No Crosslink~2 bound on that latency is established
  (\S\ref{sec:xl-views}); $L$ bounds a block gap, not how long a stall
  lasts. In
  exchange, Assured Finality protects that anchor's block in the agreed
  finalized history. This safety property alone does not guarantee its
  presence in every transient best-chain view, timely inclusion, or
  transaction validity under other rules. The wallet must still check
  the candidate ancestry; the \emph{Wallet Guide}'s retry caveat remains
  relevant whenever an anchor is absent from that ancestry
  (\S``The proposal'').
...
following is this volume's synthesis. Under the ECC branch,
coinbase-only blocks keep heights advancing. A pending spend using the
usual target-plus-40 expiry therefore eventually expires if blocks keep
arriving; a transaction already pending at the stall normally has no
more than that interval left. This is not universal: custom expiries,
transactions with no expiry, and new transactions created during the
stall need separate treatment. Wallet note locks release only after
observing the applicable expiry and applying the wallet's policy
(\emph{Wallet Guide},
\S``Expiry (ZIP-203)'' and \S``Expiry and fund availability''). Under the
SL branch finite-expiry pending spends still expire, but new spends can
keep confirming into an unbounded finalization gap --- the condition the
ECC arguments
...
\texttt{x/vote/keeper/msg\_server.go}:30--48). Requiring the pinned block
hash to identify a block in the finalized prefix would protect the pin
under Assured Finality; comparing heights alone does not authenticate a
hash on another fork. An orthogonal trust gap is untouched:
...
10\% of hash power expects to build a 10-block fork in 100
original-network block times at unchanged difficulty because the
...
W_{\mathrm{total}} \cdot R$ and
$S_{\mathrm{staker}} = (1 - c) \cdot W_{\mathrm{staker}} /
W_{\mathrm{total}} \cdot R$ with commission rate $c = 0.1$. Every staker earns
the staker slice regardless of its target finalizer. Totals may be below
$R$ only because commission attributable to stake on a
...
The ``NU8 Decision Framework'' discussion (forum thread opened 2026-02-11)
considered Crosslink alongside shorter blocks and Tachyon; that historical
discussion is not a deployment assignment. ZIP~259 now selects 25-second
blocks for NU7 without Crosslink or Tachyon. Their asserted independence is
