...
tree are those of the \emph{Ironwood Guide} (\S``The note commitment
tree'', in particular ``Anchors''); what a wallet counts as confirmed is the
...
(\emph{Ironwood Guide}, \S``Anchors''); and
...
value-commitment machinery of the \emph{Ironwood Guide} (\S``Value
commitments and the binding signature''). The draft
...
The \emph{Ironwood Guide}'s anchor-depth discussion (\S``Anchors'',
Remark ``Recommended anchor depth'') states the
...
\S``Anchors''; mainnet floor 1{,}687{,}104)
...
\path{zebra-chain/src/parameters/constants.rs}, line 30). This
local Zebra limit differs from the 100-block rollback depth that the protocol
specification records for validators
(\href{https://zips.z.cash/protocol/protocol.pdf\#blockchain}{\S3.3}); neither
is an anchor-validity window, since the anchor rule bounds no anchor's age
(\emph{Ironwood Guide}, \S``Anchors''). Wallet anchor recency is instead
governed by confirmation policy (ibid., Remark ``Recommended anchor
depth'').
...
policy''), and the protocol specification's 100-block validator rollback
expectation
(\href{https://zips.z.cash/protocol/protocol.pdf\#blockchain}{\S3.3}) differs
from Zebra's 1000-block local rollback policy; neither is an anchor-validity
window (\emph{Ironwood Guide}, \S``Anchors''). Wallet anchor recency
