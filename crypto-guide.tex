...
(\texttt{from\_bytes} in \texttt{src/curves.rs}) reconstructs \(y\)
...
\S~5.4.9.6 mandates: its decoding map returns \(\bot\) (failure) when
...
routine (e.g.\ the \texttt{orchard} crate, \texttt{src/value.rs},
\texttt{ValueCommitment::\allowbreak from\_bytes}, and the key
material of \texttt{src/keys.rs}), and cofactor \(1\) leaves no
...
\item \emph{Context tagging.} Bind an ASCII string naming the use
  ...
  group-hash (\texttt{orchard} crate,
  \texttt{src/constants/\allowbreak fixed\_bases.rs}; protocol
  specification \S\S~5.4.8.3--5.4.8.4), and
  \texttt{z.cash:\allowbreak Orchard-gd} separates diversified key
  generation (\texttt{src/constants.rs}; protocol specification
  \S~5.4.1.6). The deployed framing mirrors the definition's rather
  ...
  carrying the tag's length (\texttt{pasta\_curves} crate,
  \texttt{src/hashtocurve.rs}; protocol specification \S~5.4.9.8); the
  ...
  directly as the domain tag deriving the other (\texttt{sinsemilla}
  crate, \texttt{src/lib.rs}, \texttt{CommitDomain::new}; consumed by
  the \texttt{orchard} crate's \texttt{src/note/\allowbreak
  commitment.rs} and \texttt{src/spec.rs}; protocol specification
  \S\S~5.4.1.9 and 5.4.8.4).
\item \emph{Native personalisation.} BLAKE2b's parameter block carries
  ...
  key expansion (\texttt{zcash\_spec} crate,
  \texttt{src/prf\_expand.rs}; protocol specification \S~5.4.2),
  \texttt{Zcash\_\allowbreak OrchardKDF} for the note-encryption
  key-derivation function (KDF, the object of
  \S\ref{symmetric:subsec:kdf}; \texttt{orchard} crate,
  \texttt{src/keys.rs}; protocol specification \S~5.4.5.6), and
  \texttt{Zcash\_\allowbreak Orchardock} for the outgoing cipher key
  (\texttt{orchard} crate, \texttt{src/note\_encryption.rs}; protocol
  specification \S~5.4.2)---all exactly \(16\) bytes.
...
(\texttt{pasta\_curves} crate, \texttt{src/hashtocurve.rs}: the
...
\texttt{from\_uniform\_bytes}; protocol specification \S~5.4.9.8).
...
reduced by \texttt{from\_uniform\_bytes} (\texttt{halo2\_proofs} crate,
\texttt{src/transcript.rs}); the order in which that state absorbs
...
(\texttt{pasta\_curves} crate, \texttt{src/hashtocurve.rs}: the
\texttt{iso\_map} routine; \texttt{src/curves.rs}: the
\texttt{hash\_to\_curve} closure; protocol specification
\S~5.4.9.8).
\end{remark}
...
one-byte messages \(1\) and \(2\) (\texttt{halo2\_proofs}
crate, \texttt{src/poly/commitment.rs}), and Orchard's value
commitment derives its two bases from the tag
\texttt{z.cash:Orchard-cv} with the one-byte messages \texttt{"v"}
and \texttt{"r"} (\texttt{orchard} crate, \texttt{src/value.rs}).
\end{construction}
...
(\texttt{halo2\_poseidon} crate, \texttt{src/p128pow5t3.rs}; protocol
specification \S~5.4.1.10). It is used in
...
(\texttt{halo2\_poseidon} crate, \texttt{src/lib.rs}: the
...
\(\mathsf{PRF}^{\mathsf{nfOrchard}}\) (\texttt{orchard} crate,
\texttt{src/spec.rs}, function \texttt{prf\_nf}, consumed by
\texttt{src/keys.rs} and \texttt{src/note/nullifier.rs}; protocol
specification \S~5.4.2), from which Orchard constructs the
...
\item \emph{RIPEMD-160.} A \(160\)-bit iterated (Merkle--Damg\aa rd,
  ...
  (\texttt{zcash\_script} crate, \texttt{src/opcode/mod.rs}).
\item \emph{The address hash
  ...
  (protocol specification \S~5.6.1.1). The P2PKH output script---the
  ...
  spender reveals (\texttt{zcash\_script} crate,
  \texttt{src/pattern.rs}); wallets compute it through the
  \texttt{zcash\_transparent} crate
  (\texttt{src/util/\allowbreak hash160.rs}), and a partially created
  ...
  digests (\texttt{src/pczt.rs}).
\item \emph{Double SHA-256 (SHA-256d, or \(\mathsf{hash256}\)).} The
  composition \(x \mapsto \mathrm{SHA256}(\mathrm{SHA256}(x))\)
  (protocol specification \S~5.4.1.1), whose outer application also
  closes the length-extension property of plain SHA-256
  (\S\ref{prf:subsec:hmac}). It hashes block headers
  (\texttt{zebra-chain} crate, \texttt{src/block/hash.rs}); its
  ...
  before converting to base~58 (protocol specification \S~5.6.1.1;
  \texttt{zebra-chain} crate,
  \texttt{src/transparent/\allowbreak address.rs}, through the
  \texttt{bs58} crate's checked mode) and the payload checksum in
  every peer-to-peer message header (\texttt{zebra-network} crate,
  \texttt{src/protocol/external/\allowbreak codec.rs}); and it is the
  legacy transaction identifier: the txid of a version-4-or-earlier
  transaction is SHA-256d of its serialisation (protocol
  specification \S~7.1.1; \texttt{zcash\_primitives} crate,
  \texttt{src/transaction/mod.rs}, hashing through the
  \texttt{zcash\_transparent} crate's
  \texttt{src/util/\allowbreak sha256d.rs}), whereas a version-5
...
specification \S~4.18.4; enforced by the note-commitment gadget's
\texttt{ValueCanonicity} gate, \texttt{orchard} crate,
\texttt{src/circuit/\allowbreak note\_commit.rs}), the committed net
value is the difference of two such values, and the number of
summands is bounded, so the sums cannot wrap (\S~4.14). The prover
...
(\texttt{orchard} crate, \texttt{src/value.rs},
\texttt{ValueCommitment::\allowbreak derive}; protocol specification
\S~5.4.8.3); each Action publishes a commitment to its \emph{net}
...
verification key
(\texttt{src/bundle.rs},
\texttt{Bundle::\allowbreak binding\_validating\_key}). The
...
\S~5.4.9.8)---and write \(\mathrm{LE}_{32}(j)\) for the four-byte
...
\texttt{sinsemilla} crate, \texttt{src/lib.rs}, fixes the two
personalisation strings and derives \(Q\) in
\texttt{HashDomain::\allowbreak new}; the table ships precomputed as
\texttt{SINSEMILLA\_S} in \texttt{src/sinsemilla\_s.rs}, and the
...
(\texttt{halo2\_gadgets} crate,
\texttt{src/sinsemilla/chip/\allowbreak generator\_table.rs});
protocol specification \S~5.4.1.9. The hash iterates over the pieces
...
exactly this for the deployed instantiation (\S~5.4.1.9).
\end{proof}
...
iterator of the \texttt{sinsemilla} crate, \texttt{src/lib.rs}, pads
...
specification \S~5.4.1.9: collision resistance is demanded
...
(\texttt{src/note/\allowbreak commitment.rs},
\texttt{NoteCommitment::\allowbreak derive}; protocol specification
\S~5.4.8.4). Its key commitment \(\mathsf{CommitIvk}\) composes the
same map with \(x\)-coordinate extraction---the specification's
\(\mathsf{SinsemillaShortCommit}\)---returning a base-field element
rather than a point (\texttt{src/spec.rs}, \texttt{commit\_ivk}, via
\texttt{CommitDomain::\allowbreak short\_commit}).
...
combined check is evaluated once (\texttt{halo2\_proofs} crate,
\texttt{src/plonk/verifier/\allowbreak batch.rs},
\texttt{BatchVerifier}; consumed through
\texttt{orchard}'s \texttt{src/bundle/\allowbreak batch.rs},
\texttt{BatchValidator}). Batching amortises the per-proof group
...
\texttt{Zcash\_\allowbreak ExpandSeed} (\texttt{zcash\_spec} crate,
\texttt{src/prf\_expand.rs}; protocol specification \S``Pseudo Random
Functions''), evaluated on the spending key to derive Orchard's key
tree (\texttt{orchard} crate, \texttt{src/keys.rs}); the
note-encryption KDF is personalised with
\texttt{Zcash\_\allowbreak OrchardKDF} (\texttt{orchard} crate,
\texttt{src/keys.rs}; \S``Orchard Key Derivation'') and the outgoing
cipher key with \texttt{Zcash\_\allowbreak Orchardock}
(\texttt{orchard} crate, \texttt{src/note\_encryption.rs};
\S``Pseudo Random Functions'')---all \(16\)-byte tags. On the BLAKE2s
...
\texttt{Zcash\_\allowbreak PH} (\texttt{sapling-crypto} crate,
\texttt{src/constants.rs}; \S``Pseudo Random Functions'' and
\S``Pedersen Hash Function'').
\end{remark}
...
instance (\texttt{orchard} crate, \texttt{src/keys.rs}: type
\texttt{DiversifierKey}; protocol specification \S``Pseudo Random
...
(\texttt{zcash\_note\_encryption} crate, \texttt{src/lib.rs};
protocol specification, \S``Symmetric Encryption''). It is an
...
(\texttt{zcash\_note\_encryption} crate, \texttt{src/lib.rs},
function \texttt{try\_note\_decryption}; protocol specification,
\S``Symmetric Encryption''). For \emph{honestly produced}
...
(\texttt{orchard} crate, \texttt{src/keys.rs}: the constant
\texttt{KDF\_ORCHARD\_PERSONALIZATION} and the function
\texttt{kdf\_orchard}, which hashes the serialised shared point
followed by the ephemeral-key bytes with a \(32\)-byte output;
protocol specification, \S``Orchard Key Derivation''). The
...
bases} (protocol specification \S``Key Agreement''; the
\texttt{zcash\_note\_encryption} crate,
\texttt{src/lib.rs}: the \texttt{Domain} trait's
...
\texttt{orchard} crate, \texttt{src/keys.rs}, receives the base
...
\((\mathsf{sharedSecret}, \mathsf{epk})\) (\texttt{orchard} crate,
\texttt{src/keys.rs}, function \texttt{kdf\_orchard};
...
key-private (\S``Key Derivation''): a chain scanner holding a note
...
to accommodate it (\texttt{orchard} crate, \texttt{src/spec.rs},
function \texttt{diversify\_hash}; \texttt{src/keys.rs}, where
\texttt{DiversifiedTransmissionKey} is derived as
...
from the note's random seed (ZIP~212; \texttt{orchard} crate,
\texttt{src/note.rs}, where \texttt{RandomSeed::esk\_inner} applies
...
encryption (protocol specification \S``Encryption (Sapling and
Orchard)''; the \texttt{zcash\_note\_encryption} crate,
\texttt{src/lib.rs}: \texttt{encrypt\_note\_plaintext}, which chains
\texttt{ka\_agree\_enc}, \texttt{kdf}, and
\texttt{ChaCha20Poly1305} with the zero nonce; the Orchard
instantiation of the \texttt{Domain} trait is
\texttt{orchard} crate, \texttt{src/note\_encryption.rs}).
...
notes addressed to them (protocol specification \S``Decryption using
an Incoming Viewing Key (Sapling and Orchard)'';
\texttt{zcash\_note\_encryption} crate, \texttt{src/lib.rs},
\texttt{try\_note\_decryption} and \texttt{check\_note\_validity}).
...
(\texttt{reddsa} crate, \texttt{src/orchard.rs}; protocol
specification \S~5.4.7):
\begin{itemize}
\item the \emph{spend authorisation signature}, whose key pair can be
  ...
  (\texttt{orchard} crate, \texttt{src/keys.rs},
  \texttt{SpendAuthorizingKey::\allowbreak randomize}; protocol
  specification \S~4.15), and
\item the \emph{binding signature}, which simultaneously certifies
  ...
  knowledge of a discrete logarithm (\texttt{orchard} crate,
  \texttt{src/bundle.rs},
  \texttt{Bundle::\allowbreak binding\_validating\_key}; protocol
  specification \S~4.14).
\end{itemize}
...
line for line (\texttt{reddsa} crate,
\texttt{src/signing\_key.rs}, \texttt{SigningKey::\allowbreak sign};
protocol specification \S~5.4.7). The randomiser \(T\) is \(80\)
...
\(\Fq\) by wide reduction (\texttt{src/hash.rs}, \texttt{HStar})---
...
(\texttt{reddsa} crate, \texttt{src/orchard.rs}):
\begin{itemize}
...
\item the \emph{value-commitment randomness} base point, derived
  ...
  (\texttt{orchard} crate, \texttt{src/value.rs},
  \texttt{ValueCommitment::\allowbreak derive}, with the domain
  strings in \texttt{src/constants/\allowbreak fixed\_bases.rs})---
...
(\texttt{reddsa} crate, \texttt{src/lib.rs}, the \texttt{Sealed}
impls for \texttt{sapling::\allowbreak SpendAuth} and
\texttt{sapling::\allowbreak Binding}); the curve, its two base
...
(\texttt{orchard} crate, \texttt{src/keys.rs},
\texttt{SpendAuthorizingKey::\allowbreak randomize} and
\texttt{SpendValidatingKey::\allowbreak randomize}, delegating to
\texttt{reddsa}'s \texttt{src/signing\_key.rs} and
\texttt{src/verification\_key.rs}).
...
(\texttt{orchard} crate, \texttt{src/builder.rs},
\texttt{SpendInfo::\allowbreak build}) and reveals it only to the
...
\(rk\) as a public input (\texttt{orchard} crate,
\texttt{src/circuit.rs})---and the network
...
transaction's Actions (\texttt{orchard} crate,
\texttt{src/value.rs},
\texttt{ValueCommitment::\allowbreak derive}; protocol specification
\S~5.4.8.3). The Action structure itself---how spends and outputs
...
\(\{0, \dots, 2^{64}-1\}\) (the Action statement, protocol
specification \S~4.18.4), and in Sapling the per-note proofs
...
(\texttt{src/bundle.rs}; protocol specification \S~4.14). The
...
specification \S~4.1.7 names as the script signature scheme.
\begin{enumerate}
\item \emph{The message is the sighash.} The digest \(e\) is the
  \(32\)-byte SIGHASH transaction hash of protocol specification
  \S~4.10 (ZIP~244 for version-5 transactions), so the sighash
  algorithm plays the role of the hash in
  Theorem~\ref{signatures:thm:hashandsign}. Signer and verifier both
  feed it in as a digest (\texttt{zcash\_transparent} crate,
  \texttt{src/builder.rs}; \texttt{zcash\_script} crate,
  \texttt{src/opcode/mod.rs}).
\item \emph{DER encoding and the sighash-type byte.} On the wire a
  ...
  created transaction, \texttt{src/pczt/\allowbreak signer.rs},
  honours whichever valid type the input records). The signer
  appends the byte
  (\texttt{zcash\_transparent} crate, \texttt{src/builder.rs},
  \texttt{construct\_script\_sig}); the verifier splits it off and
  enforces strict DER unconditionally, the BIP-66 rule, so a
  malformed encoding fails script evaluation
  (\texttt{zcash\_script} crate, \texttt{src/signature.rs},
  \texttt{Decoded::\allowbreak from\_bytes}). The public key travels
...
\item \emph{Low-\(s\) normalisation.} If \((r, s)\) verifies then so
  ...
  created signature as always in lower-\(s\) form; called from the
  \texttt{zcash\_transparent} crate, \texttt{src/builder.rs} and
  \texttt{src/pczt/\allowbreak signer.rs}) and which script
...
specification \S~5.4.9.2). The deployed system treats the pairing
...
\S~5.4.10.1)---checked by a constant number of pairing equations,
...
specification \S~5.4.10.2; proofs produced and verified by the
\texttt{bellman} crate's \texttt{groth16} module,
\texttt{src/groth16/\allowbreak prover.rs} and
\texttt{src/groth16/\allowbreak verifier.rs}, consumed through the
\texttt{zcash\_proofs} crate, \texttt{src/prover.rs}). PLONK, the
...
Sapling circuits (protocol specification \S\S~5.7--5.8)---and each
...
\S~5.4.10.3) instantiates the recipe of Theorem~\ref{zk:thm:recipe},
...
copy constraints (\texttt{halo2\_proofs} crate,
\texttt{src/plonk/circuit.rs} for the gate expressions,
\texttt{src/plonk/lookup.rs} and
\texttt{src/plonk/\allowbreak permutation.rs} for the two argument
types). The polynomial commitment is the inner-product-argument
...
trapdoor---with logarithmic-size openings
(\texttt{src/poly/commitment.rs}, \texttt{Params::new} for the
parameter derivation; \texttt{src/poly/commitment/prover.rs} for
the folding rounds of the opening argument). Fiat--Shamir over a
...
(\texttt{src/transcript.rs}, \texttt{Blake2bWrite} and
\texttt{Blake2bRead}).
...
(\texttt{src/poly/commitment/verifier.rs}, the
\texttt{Accumulator} and \texttt{Guard} types;
\texttt{src/plonk/verifier/batch.rs}, \texttt{BatchVerifier};
...
sharing one Sinsemilla instance (\texttt{orchard} crate,
\texttt{src/tree.rs}, \texttt{MerkleHashOrchard::\allowbreak
combine}, prepending \texttt{i2lebsp\_k(level)} to two
\texttt{L\_ORCHARD\_MERKLE} \(= 255\)-bit child encodings;
constants in \texttt{src/constants/\allowbreak sinsemilla.rs};
protocol specification \S~5.4.1.3). Separation between
...
(\texttt{sinsemilla} crate, \texttt{src/lib.rs}:
...
set of recorded anchors (\texttt{zebra-state} crate,
\texttt{src/service/check/\allowbreak anchors.rs},
\texttt{sapling\_orchard\_anchors\_refer\_to\_final\_treestates};
protocol specification \S~3.7, `Action Transfers and their
Descriptions').
...
\texttt{src/frontier.rs}, types \texttt{Frontier} and
\texttt{NonEmptyFrontier}; the empty-subtree labels are the
\texttt{EMPTY\_ROOTS} table of \texttt{orchard}'s
\texttt{src/tree.rs}).
...
(\texttt{halo2\_gadgets} crate,
\texttt{src/sinsemilla/\allowbreak merkle.rs}, gadget
\texttt{MerklePath}; consumed by the \texttt{orchard} crate's
\texttt{src/circuit.rs}; protocol specification \S~4.9, `Merkle Path
Validity').
\end{construction}
...
single multiexponentiation (\texttt{halo2\_proofs} crate,
\texttt{src/plonk/verifier/\allowbreak batch.rs},
\texttt{BatchVerifier}; consumed through the \texttt{orchard}
crate's \texttt{src/bundle/\allowbreak batch.rs},
\texttt{BatchValidator}). The accumulation technique of the Halo
...
Remark~\ref{merkle:rem-poseidon} (\texttt{orchard} crate,
\texttt{src/note/\allowbreak commitment.rs},
\texttt{ExtractedNoteCommitment}, via \texttt{spec::extract\_p};
appended through \texttt{src/tree.rs},
\texttt{MerkleHashOrchard::\allowbreak from\_cmx}). The protocol
...
\item locates the leaf \(\mathsf{cm}_x\) in the public tree and reads
  ...
  (\texttt{src/tree.rs}, \texttt{MerklePath::\allowbreak root}
...
\item proves in zero knowledge, per
  ...
  minting'' (\texttt{orchard} crate,
  \texttt{src/circuit.rs}: the recomputed root is constrained to
  equal the public anchor, and the instance carries the anchor and
  the nullifier; \texttt{src/note/\allowbreak nullifier.rs},
  \texttt{Nullifier::\allowbreak derive}; protocol specification
  \S~4.9, `Merkle Path Validity', and \S~4.18.4, `Action
  Statement');
