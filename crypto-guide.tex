...
(\emph{Ironwood Guide}); unlike the Pedersen and Sinsemilla hashes constructed
...
In deployment the final piece is zero-padded to the piece width, so the deployed \(\mathrm{SinsemillaHash}\) is collision resistant only between inputs of a fixed length for a given personalisation --- exactly the property the protocol specification requires; the variable-length argument above applies to the piece-aligned presentation given here.
