...
\(d\) can invert---and \(H\) maps into \(\Z/N\Z\). To prove unforgeability one
...
\item (\emph{Twist security.}) The \emph{quadratic twist} \(E'\) of \(E\) (the
  ...
  should also have a large prime order, so that an attacker cannot push
...
\]
  ...
  and uses the key algebraic identity
\[
...
\(x \mapsto x^{5}\), which an arithmetic circuit verifies with a couple of
...
\(\varepsilon \le 8\ell_{\max}/2^{106}\). Consider any single verification
...
\(\varepsilon \le 8\ell_{\max}/2^{106}\). A union bound over the \(q_v\)
verification queries gives forgery probability at most
...
For an arbitrary source of min-entropy at least \(\gamma\), an extractor-style
...
Zcash does not use it---indeed the shielded protocol uses HMAC
nowhere---because its requirements are narrower and a single hash call meets
...
\begin{proof}
Sketch. In the random-oracle model,
\(c=H(\mathsf{enc}(m_b,r))\) is a uniformly random \(n\)-bit string unless
the adversary queries that exact point. Since \(r\) is uniform in
\(\{0,1\}^\ell\), each of its \(q\) queries hits the point with probability at
most \(2^{-\ell}\). A union bound gives \(q2^{-\ell}\). Conditioned on no hit,
\(c\) is independent of \(b\), proving the claim.
\end{proof}
...
\(Q\) as independent uniform generators, any collision in
...
model then makes it non-interactive. Its proof is succinct; under the strict
