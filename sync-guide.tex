...
\thispagestyle{empty}
\begin{abstract}
Assuming the best-chain model of the \emph{Consensus Guide}, the Orchard
protocol of the \emph{Ironwood Guide}, and the wallet layer of the
\emph{Wallet Guide}, this volume of \emph{The Zcash Arboretum} documents how a
light client synchronises: the compact-block format (ZIP-307)
and its deployed divergences, the light-client service surface as lightwalletd,
Zaino, and Zebra implement it, the scanning pipeline of
\textsf{librustzcash} with its verify-first reorg discipline, commitment-tree
synchronisation, and --- stated without euphemism --- what the server learns.
The
...
lines~602--685). The overflow variant is emitted by the full-block
...
each subfield while the pool's tree is empty (\textsf{zebra-rpc},
\texttt{src/methods.rs}, \texttt{GetBlockTrees},
\texttt{skip\_serializing\_if}) --- in which case Go's JSON unmarshal
leaves $\texttt{Size} = 0$, the default-zero case
\texttt{TreeSizeInvalid} exists to catch. Historically, \textsf{zcashd}
...
v0.5.0 protocol. The pool and its activation are \emph{specified}
(ZIP~2005, Proposed; ZIP~258 remains Draft as a document, while NU6.3 is
active from Testnet height $4{,}134{,}000$ and Mainnet height
$3{,}428{,}143$); deployed \textsf{Zaino} wires both
...
$\mathtt{lightwallet\_protocol\_version} = \text{``v0.5.0''}$ --- then an
untagged version but now the canonical tagged release
...
\texttt{zaino-state/src/backends/\allowbreak state.rs:2782--2792}).
...
\texttt{docs/\allowbreak internal\_spec.md}, \S~``Zaino-State'';
...
(\texttt{chain\_index/\allowbreak finalised\_state.rs:1--80}). The seam
is
\[
...
hard-coded ``v0.5.0'' before that canonical tag existed \\
\addlinespace
...
(\textsf{zcash\_client\_backend}, \texttt{src/\allowbreak
data\_api.rs:3013--\allowbreak 3106}). A
...
\texttt{src/wallet/\allowbreak scanning.rs:522--523,\allowbreak
582--584};
...
run (\textsf{zcash\_client\_backend},
\texttt{src/scan.rs:194--220,\allowbreak 461--626};
\texttt{src/scanning/\allowbreak compact.rs:101--238};
\texttt{src/data\_api/\allowbreak chain.rs:642}). Within a batch,
...
zcashd's retirement (\textsf{zebra-rpc},
...
(\textsf{zaino-state},
\texttt{src/\allowbreak backends/}\allowbreak\texttt{fetch.rs:1836--1896};
\texttt{src/\allowbreak indexer.rs:756--860};
\texttt{src/\allowbreak chain\_index.rs:509--514,\allowbreak
2397--\allowbreak 2431}). One behavioural
...
source (\texttt{chain\_index.rs:\allowbreak 2421--2431});
...
\texttt{z\_get}\allowbreak\texttt{subtrees}\allowbreak\texttt{byindex}
(\texttt{chain\_\allowbreak index/\allowbreak source/}\allowbreak
\texttt{validator\_\allowbreak connector.rs:545--608}); only
the mockchain test source self-computes
(\texttt{chain\_index/\allowbreak source/}\allowbreak
\texttt{mockchain\_\allowbreak source.rs:579--665}).
...
\texttt{with\_ironwood\_tree\_mut}
...
2558}; \textsf{zcash\_client\_backend},
\texttt{src/data\_api/ll/wallet.rs:1535--1560}).

What survives pruning is governed by per-node \emph{retention}
(\textsf{incrementalmerkletree}, \texttt{src/\allowbreak lib.rs:76--107}):
\begin{itemize}
...
(\textsf{zcash\_client\_backend},
\texttt{src/\allowbreak sync.rs:\allowbreak 80--86,\allowbreak 228--299}).
Type \texttt{CommitmentTreeRoot} pairs the subtree end height with the
root hash
(\texttt{src/\allowbreak data\_api/}\allowbreak\texttt{chain.rs:180--212}),
and the trait documentation
(\texttt{src/}\allowbreak\texttt{data\_api.rs:\allowbreak 3743--3826})
fixes the meaning:
...
(\texttt{src/data\_api/}\allowbreak\texttt{chain.rs:504--596}).
...
(\textsf{zcash\_client\_backend},
\texttt{src/\allowbreak data\_api/\allowbreak chain.rs:616--635};
\texttt{src/\allowbreak data\_api/\allowbreak ll/\allowbreak
wallet.rs:291--348}). The metadata sizes are
...
\item add any cross-pool checkpoints missing above the minimum retained
  checkpoint
  (\texttt{src/\allowbreak data\_api/\allowbreak ll/\allowbreak
  wallet.rs:1591--1664}; the cross-pool
...
is the \emph{Wallet Guide}'s material (\S``The proposal'';
\S``Confirmations, trust, and spendability'').
...
\texttt{src/wallet.rs}). The minimum historic start height visible across
...
\texttt{src/wallet.rs}). The simple \texttt{sync} module does not itself
