...
stale. No current document therefore normatively specifies the deployed
wire format.
...
lines~602--685). The final tree size also drives checkpointing: the
...
describes the pinned July revision. At \textsf{zaino} \texttt{afd609e}
(2026-08-28), validator support is Zebra-only and the old
...
Zebra main \texttt{0a43e0e} contains a third implementation, added at
...
This server is unreleased at the cited head, experimental, and disabled unless
...
supported (hash takes precedence), but \texttt{StateService} ignores
the hash in \texttt{GetBlockNullifiers} \\
\addlinespace
