...
\title{\textbf{\Huge FROST Guide}\\[6pt]\large Threshold RedPallas: FROST and its Zcash integration surfaces}
\author{m@rek.onl}
\date{}
\begin{document}
\maketitle
\thispagestyle{empty}
\begin{abstract}
Assuming Schnorr signatures from the \emph{Crypto Guide}, RedPallas from the
\emph{Ironwood Guide}, PCZT from the \emph{Wallet Guide}, and the finalizer
model from the \emph{Crosslink Guide}, this volume of \emph{The Zcash
Arboretum} documents threshold signing for Zcash. It constructs FROST as
specified in RFC~9591, then its re-randomised form for Zcash
spend authorisation and recoverability custody. The re-randomised Zcash stack
is outside the completed cryptographic-library audits identified here; each
design-stage claim is classified by the rigour of its source.
\end{abstract}
\clearpage
\tableofcontents
\clearpage
...
%%
%% Sources (verified firsthand; repository heads rechecked 2026-09-05):
...
%%     .pdf endpoint was intermittently behind a Cloudflare challenge when
%%     archived (the abstract page remained reachable).
...
%%   - Sibling volumes: ironwood-guide.tex sec:spendauth-redpallas,
%%     sec:spendauth-rk, sec:spendauth-euf; wallet-guide.tex sec:pczt (line
...
of $n$ parties jointly produce, without reconstructing the signing key during
signing, a signature that consensus accepts as an ordinary RedPallas
spend-authorisation signature. Three layers carry the construction.
The base scheme is FROST, the two-round threshold Schnorr protocol of
Komlo and Goldberg, specified in RFC 9591 (\S\ref{sec:fr-frost}). The
...
picture --- the shipped stack, the proposed uses, and the gaps --- closes
the volume (\S\ref{sec:fr-deploy}).

Three custody surfaces anchor the treatment, one per rigour bin
...
keys, for which no threshold design was identified in the cited Crosslink
corpus. The surfaces and their bins are fixed in \S\ref{sec:fr-surfaces}.

\subsection{Scope}\label{sec:fr-scope}

The object under study is a $t$-of-$n$ signing protocol whose output has the
same format and verification rule as single-party spend authorisation. ZIP 312
...
One altitude note governs every deployment claim. Availability is
library-level, not consensus-level: no consensus rule names FROST, and none
needed to change. The verifier accepts the aggregate
...
Table~\ref{tab:fr-sources} pins the primary sources used for the full
review. Repository heads were rechecked on 2026-09-05: the FROST-facing
protocol text is unchanged at \texttt{zcash/zips} \texttt{e753a6a3},
and the algorithms and Zcash ciphersuite behaviour described below remain
unchanged at \texttt{ZcashFoundation/frost} \texttt{0966bd1} and
\texttt{ZcashFoundation/reddsa} \texttt{63c6ac0}. Local copies of the papers
and RFC are archived with the volume's drafts.
...
\midrule
RFC 9591 (IRTF CFRG)
  & June 2024                      & Informational; CFRG consensus \\
...
\texttt{ZcashFoundation/frost-tools}
  & \texttt{06c0dbdb}, 2026-08-12 & demos; not for production \\
ePrint 2020/852; 2022/833
  & SAC 2020; CRYPTO 2022          & peer-reviewed antecedents \\
\bottomrule
\end{tabular}
\caption{Primary FROST sources used for the full review. Current heads
rechecked 2026-09-05.}
\label{tab:fr-sources}
\end{table}

\paragraph{The RFC.} RFC 9591, ``The Flexible Round-Optimized
...
Cloudflare; IRTF CFRG stream, Category Informational, June 2024), records
CFRG consensus on the base two-round protocol and leaves key generation out
of scope. It explicitly says that it is not an Internet Standard and that
IRTF results might not be suitable for deployment. Two of its four authors
are Zcash Foundation, so the RFC and Zcash implementation share provenance.
Its deltas against the SAC 2020 paper are catalogued in
\S\ref{sec:fr-rfc}.
...
venue as of 2026-09-05. We read it in full and work from it throughout
\S\ref{sec:fr-rerand}. Its headline Theorem~1 claims unforgeability in the
random-oracle model under AOMDL\@. That game restricts oracle queries to
declared algebraic combinations, unlike the OMDL game used for plain FROST
in BCKMTZ\@. The printed reduction has
material gaps documented in \S\ref{sec:fr-rerand-sec}; this volume therefore
does not treat that claim as an established security theorem.
...
key-share holder with transaction privacy and unlinkability, but its
security requirement says that a rogue Coordinator must not be able to
create a signed transaction without the approval of
\texttt{MIN\_PARTICIPANTS} participants. What it
...
ciphersuite-generic protocol; five ciphersuite crates instantiate the RFC
(ed25519, ed448, P-256, ristretto255, and secp256k1), while the separate
\texttt{frost-secp256k1-tr} crate implements a Taproot-oriented suite;
...
(\texttt{zcash-frost-audit-\allowbreak report-20210323.pdf}) and the source of the ZF
...
even-$Y$ normalisation hooks --- binned in \S\ref{sec:fr-bins}. Its README
marks the \texttt{frost} feature experimental while ZIP 312 remains Draft
and explicitly excludes that feature from the crate's SemVer and MSRV
guarantees.
...
The series rule is binary for implementation behaviour and ternary for design.
A claim about implementation behaviour cites the implementation --- crate and
...
\item Re-Randomized FROST for spend-authorisation custody is
  ...
  3.0.0 and \texttt{reddsa} 0.5.2. The latter's FROST feature is explicitly
  experimental. Availability is library-level, not consensus-level
  (\S\ref{sec:fr-scope}).
...
missing key-generation text is its stated reason not to couple
disbursement of the \emph{lockbox} (the in-protocol reserve that has
accrued a portion of each block subsidy since NU6, per ZIP 2001) to
that specification. Yet crate \texttt{reddsa}
...
%% DKG bin rechecked 2026-09-05: zips PR #895 (key generation for ZIP
%% 312) is still open, so no merged ZIP or RFC specifies a DKG for Zcash
...
settled in the \emph{Ironwood Guide}: \S``Randomised validating keys''
constructs $\rsk := \ask + \alpha$ and
...
unforgeability'' proves EUF-CMA from discrete log in
...
splitting $\ask$ into shares that a signing coalition need not
reassemble (\S\ref{sec:fr-rerand}). The wallet-side integration path is
likewise on the record: the \emph{Wallet Guide}, \S``Partially created
transactions (PCZT)'', records ZIP 374's motivation (iii), threshold
signing (for example FROST, RFC 9591) --- the multiparty transaction
format FROST signing rides in \S\ref{sec:fr-deploy-pczt}.
...
constrains the ceremony: the point $\aksym$ is derived jointly, via
...
\mathsf{H}^{\mathsf{rivk\_ext}}_{\mathsf{qk}}
(\Extract_{\mathbb P}(\aksym), \nksym)$;
all participants must obtain the same extracted key
$\Extract_{\mathbb P}(\aksym)$, $\nksym$, and $\mathsf{rivk\_ext}$. Subject
to the specified derivations, the ZIP says that this also implies equal
$\mathsf{qsk}$ and $\mathsf{qk}$ under collision resistance of
$\mathsf{H}^{\mathsf{qk}}$ and $\mathsf{H}^{\mathsf{rivk\_ext}}$. Each
participant must hold $\mathsf{qsk}$ at least as
securely as $\ask$. The outlined Recovery Protocol would require a
...
participant, so a quantum adversary with $\mathsf{qsk}$ and the ability to
solve discrete logarithms on Pallas no longer faces a threshold; the ZIP
accordingly recommends moving funds
...
%% notation (g^x, capital-letter group elements); the bridge to the series'
%% additive notation ([k]P on Pallas) is made in the RedPallas instantiation.
%% Equations attributed verbatim were checked against the source render.
%% Where the rerandomization preprint is inconsistent, the body identifies
%% the printed form and labels every normalisation explicitly.
...
equation for the group public key $Y = g^s$. The signature carries no
threshold marker, and a verifier applies the same equation as it would to a
single-signer signature. That compatibility is what lets the scheme use
Zcash's existing RedDSA validators: the consensus rules do not observe the
threshold ceremony. The deployment surfaces this volume targets are
spend-authorisation custody over RedPallas (the \emph{Ironwood Guide},
\S``The RedPallas signature scheme''), custody of the ZIP-2005
...
problem is to compute the pair $(R, z)$ jointly, without reconstructing the
nonce or signing key during signing and without opening the concurrency
...
\]
giving participant $P_i$ the share $(i, f(i))$, with distinct nonzero
identifiers in $\Z_q$ and $1\le t\le n<q$. Coefficients are sampled
independently and uniformly in $\Z_q$. Any $t$ shares determine
...
\item Every participant $P_i$ samples $t$ random values
  ...
  polynomial $f_i(x) = \sum_{j=0}^{t-1} a_{ij} x^j$ of degree at most $t-1$.
...
knowledge of $a_{i0}$} is the delta over plain Pedersen DKG: it addresses
...
$\mathrm{SA}$ coordinates. The paper trusts it for publication and correct
misbehaviour reporting, not for unforgeability, and claims in its security
model that a malicious $\mathrm{SA}$ can deny service or falsely accuse but
cannot learn the private key or cause an unapproved message to be signed.
...
factor is FROST's answer to concurrency attacks that affected or constrained
the earlier two-round schemes discussed by the paper.
...
\paragraph{How binding addresses it.} Because each signer derives
$\rho_i = H_1(i, m, B)$ before responding, its share $z_i$ is bound to
this message, this signing set, and this exact commitment list. Except with
random-oracle collisions, changing those inputs alters some $\rho_\ell$,
hence $R$ and $c$, so directly transplanting a response fails verification.
The paper's own generalisation (its
...
\item an accidentally reused $(d_{ij}, e_{ij})$ can expose the long-term
  share $s_i$;
\item a $\mathrm{SA}$ that maliciously replays a commitment pair cannot
  elicit another response from an honest participant; the replay causes an
  abort and can therefore deny service;
...
The paper gives an EUF-CMA reduction, in the random-oracle model, for an
intermediate scheme under the \emph{discrete logarithm problem} in $\G$ ---
deliberately not to one-more discrete log (the adversary must return the
discrete logarithms of $\ell + 1$ challenges after at most $\ell$
queries to a discrete-log oracle), so that the
negative result of Drijvers et al., which rules out security reductions
from that assumption for a family of two-round Schnorr multisignatures,
does not apply. The proof is for an intermediate scheme,
...
\item concretely (their Theorem 5.1, verbatim bound), for a TS-SUF-2
  ...
  \]
  and an analogous Theorem 5.4 for FROST1 at TS-SUF-3. In these source
  bounds, $p$ denotes the group order and $q$ the query count, replacing
  the earlier section's use of $q$ for the group order.
\end{itemize}
The BCKMTZ analysis for the RFC's FROST1 signing layer is therefore based on
OMDL in the ROM, static corruptions below $t$, and trusted-dealer keys. It
does not establish composition with the DKG shipped by the implementation,
and its basis differs from the original paper's DL reduction for the
interactive variant plus a heuristic bridge.
...
$z = r + c(x + \hat{\alpha})$. Its Theorem 1 \emph{claims}
unforgeability in the ROM under \emph{algebraic} OMDL (AOMDL). Unlike
BCKMTZ's OMDL game, AOMDL restricts discrete-log-oracle queries to declared
algebraic combinations, so these are not the ``same security assumptions''
claimed in the preprint's abstract. The stated bound (its Equation 3) is
\[
...
via the local forking lemma. The intended extraction uses the public
randomizer to strip
$c^* H_r(\alpha^*, \mathrm{aux}^*)$ from $z^*$, but the printed proof has
material defects recorded in \S\ref{sec:fr-rerand-sec}; the claimed bound is
not independently established here. The coordinator chooses the randomizer
and is trusted for privacy and unlinkability only, not for unforgeability.
The full treatment of this paper belongs to the re-randomisation chapter
(\S\ref{sec:fr-rerand}).

\subsection{RFC 9591 against the paper: what the RFC changed}
\label{sec:fr-rfc}

RFC 9591 (IRTF CFRG, Informational, June 2024; Connolly, Komlo, Goldberg,
Wood) is the specification implemented by the
...
\item \emph{Binding factor inputs widened.} The paper binds
  ...
  9591 specifies FROST1: the RFC's \S7.2 rules the
...
\item \emph{Five hash roles.} $H_1$ (binding factor), $H_2$ (challenge),
  $H_3$ (nonce), $H_4$ (message pre-hash), and $H_5$
  (commitment-list pre-hash) are domain-separated with a per-ciphersuite
  \texttt{contextString}, except that FROST(Ed25519, SHA-512) deliberately
  leaves $H_2$ unprefixed for RFC 8032 compatibility. The challenge input order
...
\item \emph{Roles renamed and channels stated.} The signature aggregator
  ...
  Reliable delivery is also assumed for protocol completion;
...
\item \emph{Ciphersuites.} The RFC ships FROST(Ed25519, SHA-512),
  ...
  appears in the RFC itself. The two Zcash suites,
  ...
  sense in which the RFC protocol plugs into the
  spend-authorisation verifier of the \emph{Ironwood Guide},
  \S``The RedPallas signature scheme''.
\item \emph{Security claim.} The RFC claims EUF-CMA per BCKMTZ, citing
  both the original paper and BCKMTZ --- i.e., the specification's claim rests
...
\texttt{dkg} module but specified in no ZIP or RFC).
%% Bin rechecked 2026-09-05: no merged ZIP or RFC specifies any DKG for
%% Zcash custody; zips PR #895, which would specify key generation, remains
%% open.

%% FROST Guide draft: Rerandomized FROST for RedPallas --- ePrint 2024/436,
%% ZIP 312, the released crates, and the custody surfaces.
...
The FROST of \S\ref{sec:fr-frost}, specified in RFC 9591
(\S\ref{sec:fr-rfc}), constructs its aggregate relative to a \emph{fixed}
group public key $PK$. Zcash instead supplies a freshly randomised validating
key for every Sapling Spend and Orchard Action, and consensus checks the
spend-authorisation signature against that per-Action key --- a key to which,
as \S\ref{sec:fr-rerand-gap} makes precise, the unchanged FROST shares do not
interpolate. This chapter presents the
...
ciphersuite FROST(Pallas, BLAKE2b-512), the published implementation in the
\texttt{frost-rerandomized} and \texttt{reddsa} crates, and the custody
surfaces the construction serves.

A rigour note before the material. RFC 9591 is an Informational RFC recording
CFRG consensus, not an Internet Standard. The Gouv\^ea--Komlo paper is a
preprint --- received
...
The protocol specification (\href{https://zips.z.cash/protocol/protocol.pdf\#spendauthsig}{\S4.15}, spend authorization signatures)
...
$\rk = \aksym + [\alpha]\,G^{\mathsf{SpendAuth}}$. This differs from
$\aksym$ except in the possible identity-randomizer case $\alpha=0$.

The randomisation algorithms are RedDSA's, from the protocol
specification (\href{https://zips.z.cash/protocol/protocol.pdf\#concretereddsa}{\S5.4.7}):
\begin{align*}
...
The construction and the cited single-party EUF-CMA result are settled in the
\emph{Ironwood Guide}: \S``Randomised validating keys'' builds
$\rsk := \ask + \alpha$ and
$\rk := \aksym + [\alpha]\,G^{\mathsf{SpendAuth}}$, and
\S``RedPallas unforgeability'' proves EUF-CMA under
DL in the ROM \emph{including} adversary-chosen $\alpha$, by key-prefixed
forking with $\ask = \rsk^* - \alpha^*$. Per the layering
rule we cite those results and do not re-prove them. Because its
Proposition ``Unforgeability under re-randomisation'' requires only a fresh
key--message--signature triple, it also implies the protocol specification's
strong SURK-CMA notion, for a single signer.
...
set $S$; an honest plain-FROST execution constructs a signature under
$\aksym$. For a nonzero Action randomizer, however, the verifier's key is
$\rk$. The noninteractive construction below shifts every participant by
the same effective randomizer. A separately generated Shamir sharing of a
randomizer could also shift the group secret, but would be a different,
...
work from the full text and page renders of its construction and equations.
The transcription below normalises the source-level slips identified after
the construction. As in
...
\begin{remark}[What ``perfectly'' specifies]
\label{rem:fr-perfect-definition}
The printed Definition 5 requires only the existence, inputs, and outputs of
four algorithms. It states neither a correctness relation nor equality of
any distributions. The adjective ``perfectly'' therefore adds no formal
property in that definition. Functional correctness of the concrete scheme
comes from Equation~\eqref{eq:fr-rerand-z}; no distributional guarantee
follows from Definition 5 alone.
\end{remark}
...
released \texttt{reddsa} crate use. We read the paper's orderings as a
presentational slip with no normative weight; the implemented order is
RedDSA's. The paper also twice refers to ``Figure 4'' where Figure 5 is
meant.}
\end{construction}

The displayed construction also normalises several evident errors in Figure 5.
Its \textsf{Setup} lists only $H_{\mathrm{non}}$ and $H_{\mathrm{sig}}$ even
though every randomisation algorithm calls $H_r$. In $\mathsf{Sign}'$, its
formal argument names the local randomised share $\bar{x}_i$ although the
algorithm and its body use signer index $k$; we consistently write
$\bar{x}_k$. Its loop also prints $k$ as the first input to
$H_{\mathrm{non}}$ for every $a_i$; the iterated index must be $i$, as above,
for the commitment $R=\prod_i R_iS_i^{a_i}$ and response term $s_k a_k$ to
use the same per-participant binding factors.
The paper's preliminary Shamir definition has two further typographical slips:
its polynomial omits the factor $Z$ from the $a_1Z$ term and its prose says
that $t$ coefficients are sampled while listing $a_1,\ldots,a_{t-1}$. Its
subsequent evaluation formula is the standard degree-$(t-1)$ sharing used here.

The paper is explicit that this is the whole design: ``We do not make any
changes to the plain FROST protocol.'' Its signing equations are unchanged;
the added algorithms supply the shifted share and public-key inputs.
...
\paragraph{The game.} The paper's notion (Definition 6, game
$\mathrm{TRUF}$ in Figure 4) allows adversarial randomizers and static
corruption of fewer than $t$ parties. Its signing oracle records only the
queried messages $Q$, and a forgery wins only when $m^* \notin Q$. It is
therefore an \emph{existential} notion: a new signature for a previously
signed message is not a winning forgery. The protocol specification instead
requires SURK-CMA, whose oracle records message--signature pairs and accepts
any fresh pair, including a different signature on a previously queried
message. The paper therefore uses a weaker freshness condition, and Theorem 1
does not, as printed, establish the protocol specification's strong notion.
The game pseudocode also assigns
$\mathsf{RandShare}(x_k,\alpha,\mathrm{aux})$ to $\bar{x}_i$ but later passes
$\bar{x}_k$, and its call to $\mathsf{Sign}'$ omits the message argument that
the construction requires. Those are interface errors in the printed game; the
discussion here uses the evident $k$ index and includes $m$.

There is a second mismatch in the unrandomised branch. The winning predicate
also accepts a forgery under $PK$, but every signing-oracle answer first
applies $H_r(\alpha,\mathrm{aux})$; the adversary has no ordinary signing
oracle under $PK$. Contrary to the paper's prose, setting its outer
randomizer $\alpha$ to zero does not produce the identity shift. The
construction's randomizer domain is $\Z_p$ and $H_r(0,\mathrm{aux})$ need
not be zero. Thus the separate verification branch does not embed the
ordinary EUF-CMA game either.

\paragraph{The assumption.} Intended AOMDL is the \emph{algebraic
one-more discrete logarithm} game: the adversary receives $\ell+1$
challenges, may query a discrete-log oracle only on declared linear
combinations of environment-generated elements, and should make at most
$\ell$ queries. The algebraic restriction is absent from
\S\ref{sec:fr-later}'s OMDL\@. Since it reduces the adversary's allowed oracle
queries, AOMDL hardness is formally a weaker hardness assumption than OMDL
hardness, obtained at the cost of an algebraic-adversary restriction; it is
neither literally the same assumption nor a stronger hardness assumption.
Figure 1 is not a well-defined rendition of that game: its oracle accepts
$\{\beta_i\}_{i=1}^{\ell}$ but its return expression uses $\beta_0$ as well,
and it never checks that the queried element $X$ equals the declared
combination $g^\alpha\prod_i X_i^{\beta_i}$.
After the natural repair of supplying coefficients for every challenge, a
second defect remains: the game initialises its query counter $q\gets0$ but
never increments it. Its final test $q<\ell+1$ then imposes no query limit, so
an adversary can request every challenge logarithm. We interpret AOMDL by its
intended one-more definition below, not by that malformed pseudocode.

\paragraph{The theorem claim.} The preprint states the following
(Theorem 1 with Equation (3); the radical was checked against the rendered
page):
...
a negative term inside can make the radicand negative. Neither the theorem
statement nor its proof explains how that expression is to be interpreted in
that case.
\end{remark}

\paragraph{Intended proof shape, and why it is not verified.} The reduction
$\mathcal{B}$ receives $2q_s+1$ AOMDL challenges, sets $PK=X_0$, and
simulates verification shares in the exponent. Even the displayed
public-share simulation is not well formed: its product shadows the output
index $i$ and leaves both $j$ and $k$ unbound. If $C$ is the set of corrupted
identifiers and
$L_j(i)$ is the Lagrange basis coefficient for node $j\in\{0\}\cup C$
evaluated at the honest identifier $i$, the required exponent relation is
\[
  PK_i = PK^{L_0(i)}\prod_{j\in C}\bigl(g^{x_j}\bigr)^{L_j(i)}.
\]
The printed expression does not establish that simulated public shares
interpolate to $PK$.
...
\begin{equation}\label{eq:fr-correct-round-two}
Z_k=R_k S_k^{a_k}\overline{PK}_k^{\,c\lambda_k}.
\end{equation}
The preprint instead prints
$R_i S_i^{a_k}\overline{PK}_k^{\,c}$: it mixes $i$ and $k$ and omits the
Lagrange exponent $\lambda_k$. Its claimed ``perfect'' signing simulation
therefore does not follow from the displayed query.

The random-oracle simulation also is not a consistent lazy sampler as
printed. Its $H_r$ table stores only the queried pair $(\alpha,\mathrm{aux})$,
not the sampled output, so a repeat query has no associated value to return.
Its $H_{\mathrm{non}}$ query signature omits $m$ although its lookup and table
include $m$; when it samples $a_j$ for each other participant, it stores $a_k$
rather than $a_j$; and its new-query branch returns the sampled
$H_{\mathrm{sig}}$ value $c$ rather than the requested binding value $a_k$.
The proof later invokes an unrandomised case $\alpha=\bot$, although neither
the construction's domain $\Z_p$ nor the TRUF oracle defines that sentinel.

The forking step is underspecified too. It says to reprogram
$H_{\mathrm{sig}}(PK',R^*,m^*)$ without defining $PK'$, then asserts a second
forgery with the same $R^*$ but potentially different $(\alpha,\mathrm{aux})$.
It does not show that the reprogrammed query is the challenge query for both
accepted forgeries when their re-randomised public keys differ. Nor does the
proof derive the concrete probability bound in
Equation~\eqref{eq:fr-rerand-bound}; it concludes only with a qualitative
``non-negligible probability'' invocation of the local forking lemma.

The first fork-extraction identity is algebraically sound if both forgeries
share the same commitment $R$, verify under re-randomised keys, and have
$c_0\ne c_1$. Writing
$h_b=H_r(\alpha_b,\mathrm{aux}_b)$, it removes the public shifts and obtains
\[
x=\frac{(z^*_0-c_0h_0)-(z^*_1-c_1h_1)}{c_0-c_1}.
\]
The subsequent nonce extraction is not sound as printed. If two response
shares have the form
\[
u_b=r_i+s_i a_b+c_b\lambda_i(x_i+h_b),
\]
then for $a_0\ne a_1$ the corresponding identity is
\[
s_i=\frac{u_0-u_1-c_0\lambda_i x_i-c_0\lambda_i h_0
                 +c_1\lambda_i x_i+c_1\lambda_i h_1}{a_0-a_1}.
\]
The printed numerator gives the two $c_1$ terms the wrong signs and uses the
outer randomizers in place of their $H_r$ images. More seriously, the proof
claims that when $c_0=c_1$, $a_0=a_1$, and $h_0\ne h_1$, the reduction can
still recover both $(r_i,s_i)$. After subtracting the known randomizer
difference, however, the transcripts give only one linear equation in those
two nonce scalars. They are underdetermined.

Together with Figure 1's missing query-counter increment, these are material
gaps, not merely notation. A repair may be possible, but this volume does not
supply a complete replacement reduction. It therefore records Theorem 1 as
a preprint claim and does not use Equation~\eqref{eq:fr-rerand-bound} as a
verified assurance statement.

\paragraph{Scope even under a repaired proof.} The claimed result is only
existential unforgeability in the paper's weaker game; no cited theorem
establishes threshold SURK-CMA\@. The preprint contains no formal unlinkability
definition or theorem. Corruptions are static, and the threshold-scheme game
uses centralised key generation; the paper says it ``can [be] adapted'' to a
DKG, but gives no composition proof. Finally, the paper's \S5.1
...
\paragraph{The signing-flow changes to RFC 9591.}
\begin{itemize}
...
\item \emph{Round two.} The Coordinator generates the randomizer and
  ...
  channel'' --- plain FROST requires reliability and authentication but
  not confidentiality --- together
...
$\mathsf{SigHash}$, which by itself does not expose enough transaction
semantics for a signer to know what is being authorised. Signers MUST
therefore check the
...
The same authorisation principle applies to any external signer: a signer who
cannot see what it signs authorises whatever the builder chose.

\paragraph{Unlinkability, and where it is proven.} Neither the paper nor
ZIP 312 supplies a formal threshold-unlinkability proof: the paper attempts
only an existential-unforgeability proof
(\S\ref{sec:fr-rerand-sec}), and the ZIP argues unlinkability in its
Rationale by matching distributions
...
formal notion to the protocol specification's SURK-CMA definition. More
precisely, reducing a 512-bit random-oracle output modulo the group order is
statistically close to uniform, not exactly uniform; the same qualification
applies to $\mathsf{RedDSA.GenRandom}$. The distribution-matching argument
does connect to a proved statement: the
\emph{Ironwood Guide}'s Proposition ``Unlinkability of randomised validating
keys'' shows honest uniform re-randomisations are
distributed as fresh Schnorr keys. What no source supplies is a
threshold-specific unlinkability theorem. Nor does the preprint's weaker
existential game establish the threshold SURK-CMA requirement. Bin: the
mechanism is \emph{specified} (ZIP 312, Draft); its unlinkability guarantee
rests on a distribution argument, and its requested strong-unforgeability
guarantee has no matching threshold theorem among the cited sources.
...
Pallas with base point $G^{\mathsf{SpendAuth}}$ as in protocol \href{https://zips.z.cash/protocol/protocol.pdf\#concretespendauthsig}{\S5.4.7.1},
of order $p_{\Vesta}$; \texttt{RandomScalar} is uniform in
...
which only parses $R$ and checks the validation equation. A parallel suite
FROST(Jubjub, BLAKE2b-512) --- personalisations
\texttt{FROST\_RedJubjub\{R,N,M,C,A\}} and \texttt{Zcash\_RedJubjubH}
--- serves Sapling spend authorisation. ZIP 312 defines that suite
separately; this volume develops only the Pallas suite.

\subsection{Published library implementation: \texttt{frost-rerandomized} and
...
version 0.5.2), rechecked against current heads on 2026-09-05. The pins are
...
reference implementation of both ciphersuites. A current-head check on
2026-09-05 found \textsf{frost} \texttt{0966bd1} and \textsf{reddsa}
\texttt{63c6ac0}; their post-release changes update the Rust edition,
minimum compiler, dependencies, benchmarks, and CI, not the re-randomized
protocol derived below.

\paragraph{The generic layer.} Crate \texttt{frost-rerandomized} implements
...
randomizer, and each participant recomputes the randomizer from that seed
and the full round-one commitment map in its signing package. The derivation,
its divergence
...
\texttt{RandomizedCiphersuite::\allowbreak hash\_randomizer} impl uses
\texttt{FROST\_RedPallasA} (the ZIP's $H_R$); scalar reduction is the
$64$-byte-wide \texttt{from\_bytes\_wide}, whose
Pallas implementation delegates to pasta's
\texttt{from\_uniform\_bytes}.
...
\texttt{randomized\_\allowbreak verifying\_key()} of its
\texttt{RandomizedParams}
...
\emph{Ironwood Guide}, \S``Randomised validating keys''. Bin:
...
specified nowhere), as \S\ref{sec:fr-rfc} already concluded for the
...
property does not survive an adversary that can also solve Pallas discrete
logarithms --- are developed once,
...
%% Shared-sk bin rechecked 2026-09-05: zips PR #895 drafts a
...
Zcash Transaction Format, Draft, owner Jack Grigg) names threshold
signing (``for example FROST'', per RFC 9591) in its Motivation among
the multiparty use cases its Signer role must accommodate.

\paragraph{Classification.} In the series' three bins: base FROST and
its five ciphersuites are \emph{specified} in RFC 9591, an Informational
RFC recording CFRG consensus (\S\ref{sec:fr-rfc}). Re-Randomized FROST,
its threat model, and both Zcash ciphersuites are \emph{specified} by
ZIP 312 at Draft status. The preprint's claimed existential-unforgeability
proof has the gaps in \S\ref{sec:fr-rerand-sec}, no cited theorem establishes
threshold SURK-CMA, and unlinkability rests on a distribution argument rather
than a theorem. Key generation --- FROST DKG for Zcash, and ZIP 2005's
...
Implementation-behaviour claims (seed-based randomizer derivation, even-$Y$
...
%%   - zips PR #895 (still open, rechecked 2026-09-05).
%%   - C. P. L. Gouvea, C. Komlo, "Re-Randomized FROST", IACR ePrint
...
%%   - Sibling volumes: ironwood-guide.tex (sec:spendauth-redpallas,
%%     sec:spendauth-rk, sec:spendauth-euf), wallet-guide.tex (sec:pczt,
...
ships. We pin the exact code paths available to a prospective Zcash
threshold-custody deployment, reconcile them against the draft ZIP text where
the two have diverged, then walk the three custody surfaces this volume targets ---
...
\subsection{The shipped library stack}\label{sec:fr-deploy-stack}
...
the generic \texttt{frost-core}, the five RFC 9591 ciphersuite crates
\texttt{frost-ristretto255}, \texttt{-ed25519}, \texttt{-ed448},
\texttt{-p256}, and \texttt{-secp256k1}, the separate
\texttt{frost-secp256k1-tr} Taproot suite, and the generic
re-randomisation layer \texttt{frost-rerandomized}. Beyond
RFC 9591's two-round protocol the workspace implements trusted-dealer key
generation (the RFC's appendix), a variant of the original paper's
Pedersen-with-knowledge-proof DKG that omits its context string (ePrint
2020/852, \S\ref{sec:fr-keygen} --- not in the RFC), Repairable-Threshold-Scheme
...
Zcash ciphersuite modules are the shipped top-level suites that re-export
...
The crate's README calls this feature experimental because ZIP 312 is still
Draft and states that neither its SemVer nor its MSRV guarantee applies to
the feature.

The RedPallas module additionally enforces the even-$y$ requirement that
ZIP 2005's amended protocol-spec \href{https://zips.z.cash/protocol/protocol.pdf\#orchardkeycomponents}{\S4.2.3} places on $\aksym$
...
channel'' --- adding confidentiality to plain FROST's reliable,
authenticated-channel assumption, because the randomizer links the ceremony
to the on-chain $\rk$.

The released crate differs from this draft text. Crate
\texttt{frost-rerandomized} 3.0.0 deprecates
\texttt{Randomizer::new(rng, signing\_package)} --- the earlier,
message-inclusive API\@. Even that API is not byte-for-byte the draft flow: it
samples and serialises a field scalar before hashing, whereas the ZIP starts
with $N_s$ unconstrained random bytes. The recommended API is instead
...
is no longer an input. The crate documentation says that hashing the complete
commitment map hedges the Coordinator's RNG because the map includes signer
commitments. This is its implementation rationale, not a property proved by
the cited RFC, ZIP, or preprint; in particular, the Coordinator receives the
commitments before generating the seed and remains trusted for unlinkability.
...
August 2024, 21 commits, still open and mergeable as of 2026-09-05, with
its latest review comments dated 2026-08-28). It adds a key-generation
specification, revises the
randomizer handling (the randomizer ``couldn't possibly be generated
using the message as an input''), integrates the ZIP-2005
$\mathsf{qsk}$ material, renames the
...
message omission included --- as the observed fact of the pinned commit and
the ZIP text as a divergent Draft specification. No conformance conclusion
follows while that text remains in flight.
...
with transaction privacy and unlinkability, while the requirements say it
must not be able to forge without $\mathtt{MIN\_PARTICIPANTS}$ approvals;
every key-share holder is
...
the re-randomisation of the \emph{Ironwood Guide}, \S``Randomised
validating keys'' --- and checks
...
signature; for it to verify, the effective FROST randomizer must equal the
PCZT action's $\alpha$, so that the re-randomised group key
$\mathsf{RandPK}$ produces equals that action's $\rk$.

PCZT v2 has since shipped in \textsf{pczt}~0.8.0. Current
\textsf{pczt}~0.9.3 carries separate Orchard and Ironwood bundles and tags an
externally produced \texttt{SpendAuthSignature} with its value pool and action
index; \texttt{apply\_orchard\_spend\_auth\_signature} dispatches to the
corresponding bundle and verifies under that action's $\rk$. The invariant in
the preceding paragraph --- one shared shielded sighash and the action's own
$\alpha$ --- is unchanged for v6 Ironwood spends
(\textsf{librustzcash} \texttt{91f448b5},
\texttt{pczt/src/roles/signer/mod.rs}).
...
\end{enumerate}
The current \texttt{frost-tools} head (\texttt{06c0dbdb}, 2026-08-12)
does not exercise the version-3 seed flow described in
\S\ref{sec:fr-deploy-rand}. Its lockfile selects \texttt{frost-core} 2.2.0,
\texttt{frost-rerandomized} 2.1.0, and an older \texttt{reddsa} 0.5.1
revision. For PCZT, the Coordinator accepts each Action's already-generated
scalar randomizer and sends that value to participants through the legacy
raw-randomizer API\@. This is a dated implementation boundary: the demo validates
the PCZT external-signature path, but it is not evidence for integration of the
pinned version-3 library stack or its seed flow.

That distinction creates an ordering requirement for any version-3 integration.
The recommended seed API derives the effective randomizer from round-one
commitments, so it cannot in general be made equal to an $\alpha$ already fixed
inside a PCZT Action. The transaction builder must instead use the derived
randomizer as that Action's $\alpha$ before constructing its proof and $\rk$, or
use a separately justified compatibility path. Neither the legacy demo nor a
specification cited here demonstrates that reordered end-to-end flow.
...
FROST-capable Zcash wallet inherits. The documented Zcash integration cannot
authorise transparent inputs, which require ECDSA; the book accordingly
suggests supporting Orchard only (``Orchard is simpler to handle''). That is
historical integration advice rather than a current-pool limit: the page
predates the Ironwood pool. With a DKG-generated
...
independently generated $\nksym$ and $\mathsf{rivk}$. Under this key-generation
flow, a seed phrase alone does not restore the wallet: recovery also needs the
key share, all verifying shares, the participant identifiers, and the FVK
components. The book stores these in participant configuration files; the
library also provides the Repairable Threshold Scheme
(\S\ref{sec:fr-deploy-stack}).

The Foundation's status report (``The State of FROST for Zcash'',
2025-05-20) named wallet integration the missing piece (``The missing
piece is for wallets to integrate FROST''): at publication time it reported
no mainstream wallet integration, while ``technical-inclined users can use
...
The reference transport is \texttt{frostd}, a JSON-over-HTTPS session server:
...
end-to-end encrypted. Thus the server need not be trusted with message
contents or signing authority, although it remains an availability dependency. The CLI
...
(\S\ref{sec:fr-deploy-rand}). The tools repository labels the command-line
programs as demos and says they should not be used in production.
The \texttt{frost-client} README also warns that it stores secrets unencrypted
in its configuration file and recommends protecting that file or building a
separate tool from the client as a base.

\subsection{Audit coverage, precisely}\label{sec:fr-deploy-audit}

Two cryptographic-library audits are relevant, and neither covers the code
path Zcash custody would run. NCC Group audited the v0.6.0 release (commit
\texttt{5fa17ed}) of
...
\texttt{src/frost.rs} (404 source lines by the report's count, at commit
\texttt{2ebc08f},
2021-02-25) --- two-round FROST with a trusted dealer only, RedJubjub
only --- concluding ``We did not find any major security issue.'' The report
also records that the implementation omitted the paper's secure deletion of
nonces and commitments after signing.
...
\S\ref{sec:fr-deploy-stack} --- is not covered by any completed third-party
audit identified in the repository and audit announcements rechecked
2026-09-05. Of those audits, only the 2021 review touched a Zcash FROST
ciphersuite, and it predates the \texttt{frost-core} rewrite. The separate
Least Authority audit covers the transport tools, not this cryptographic
composition.
%% Audit landscape rechecked 2026-09-05: frost README @0966bd1 still
...
$\ask$, via FROST DKG\@. It then says that participants ``MUST privately
agree on a value $\mathsf{sk}$'' and use it with
$\mathsf{use\_qsk}=\mathsf{true}$ to derive the remaining material. The
protocol must give every participant the same extracted validating-key value
$\mathsf{ak}$, $\nksym$, and $\mathsf{rivk\_ext}$; the specified derivations then imply
equal $\mathsf{qsk}$ and $\mathsf{qk}$ under the ZIP's stated
collision-resistance assumptions. In this series,
$\aksym=[\ask]G^{\mathsf{SpendAuth}}$ denotes the Pallas point. ZIP 2005
writes $\mathsf{ak}^{\mathbb P}$ for that point and
$\mathsf{ak}:=\Extract_{\mathbb P}(\mathsf{ak}^{\mathbb P})$ for its
extracted field element. With that point/field-element distinction explicit,
its derivations are
\begin{align*}
...
(\mathsf{ak}, \nksym) &=
  ...
  \mathrm{I2LEOSP}_{256}(\mathsf{ak}) \,\|\,
  \mathrm{I2LEOSP}_{256}(\nksym))\bigr).
\end{align*}
The displayed key derivations use the $\mathsf{PRFexpand}$ domain-separator
bytes $\mathtt{0x0C}$ and $\mathtt{0x0D}$; ZIP 2005 also reserves
$\mathtt{0x0A}$ and $\mathtt{0x0B}$ for note-related uses. In the
$\mathsf{use\_qsk} = \mathsf{true}$ branch of the amended protocol-spec
\href{https://zips.z.cash/protocol/protocol.pdf\#orchardkeycomponents}{\S4.2.3}, the wallet obtains the point $\mathsf{ak}^{\mathbb P}$ ``by any suitably
secure method that ensures the last bit of
$\mathrm{repr}_{\mathbb{P}}(\mathsf{ak}^{\mathbb P})$ is 0'' --- the
requirement the \texttt{reddsa} \texttt{EvenY} machinery of
\S\ref{sec:fr-deploy-stack} enforces --- and sets the ZIP's field element
$\mathsf{ak}=\Extract_{\mathbb P}(\mathsf{ak}^{\mathbb P})$, $\mathsf{qsk} =
...
$\mathsf{ak}^{\mathbb P}$ explicitly includes ``FROST distributed key
...
FROST setup is the ZIP-312 Coordinator; the ZIP gives it $\nksym$, the
extracted validating-key value $\mathsf{ak}$, and $\mathsf{qk}$.
With FROST and hardware wallets, spend authorisation breaks
...
Orchard protocol is disabled. A quantum adversary holding $\mathsf{qsk}$
--- which every participant holds --- and able to solve discrete logarithms
on Pallas may steal funds, so the ZIP RECOMMENDS that once a fully
post-quantum threshold
multisignature protocol exists, all FROST-held funds migrate to its pool.
FROST's key advantage --- signing with shares without reconstructing $\ask$
during signing --- would be retained in the outlined Recovery Protocol, which
would still require a RedDSA signature.
...
expected there: as of 2026-09-05 the PR's commit history drafts a
...
\texttt{frost-ed25519} crate, whose v0.6.0 predecessor was covered by the
NCC audit, with no re-randomisation layer --- finalizer
...
\item \emph{Specified.} FROST as in RFC 9591 (\S\ref{sec:fr-rfc});
  ...
  \texttt{frost-core}/\allowbreak\texttt{frost-rerandomized}/%
  \allowbreak\texttt{reddsa} API
...
\item \emph{Open problem.} Crosslink finalizer threshold custody
  ...
  hardware-wallet FROST signers, but none was identified in the device
  integrations checked for this audit.
\end{itemize}

Two former revisit triggers have fired without changing these bins. NU6.3 is
active, but \texttt{zcash\_keys} still gates
\texttt{from\_orchard\_fvk} behind \texttt{unstable-frost} pending a specified
FROST derivation. PCZT v2 is released, and the current signer preserves the
$\alpha$ and external-signature surface for Ironwood as described in
\S\ref{sec:fr-deploy-pczt}. The remaining classifications expire on the
following identifiable events, in rough order of expected impact:
