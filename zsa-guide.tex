...
\thispagestyle{empty}
\begin{abstract}
Assuming the Orchard protocol of the \emph{Ironwood Guide} and transaction
construction from the \emph{Wallet Guide}, this volume of \emph{The Zcash
Arboretum} documents Zcash Shielded Assets: per-asset value bases and asset
identity, transparent issuance and finalization (ZIP-227), shielded transfer
and burn (ZIP-226), and the split-note construction that guards the
counterfeiting boundary. The upstream ZIPs are in flux --- their transaction
carrier was withdrawn and their fee terms removed upstream --- so every claim
is classified as specified, designed-but-unspecified, or open. Where ZIP text,
fork text, and the implementation disagree, all three readings are stated and
...
(\emph{Ironwood Guide}, \S``Conservation without disclosure: value commitments
and the binding signature'') is replaced, per Asset, by an \emph{Asset Base}
witnessed
inside the circuit; the same Asset Base must appear in the old and the new
note commitments and as the base of the Action's value commitment. Under the
discrete-log binding assumption for independently derived bases, the binding
signature then enforces balance separately per Asset Identifier without
revealing which Assets --- or how many distinct ones --- a transaction
transfers (\texttt{zip-0226.rst} lines~59--63). The transparent protocol is
unchanged: unshielding is
...
Everything the delta rests on is constructed in earlier volumes and is
cited, never re-derived: notes and note commitments (\emph{Ironwood Guide},
\S``The note and its commitment''); value
commitments, the bases $V^{\mathsf{Orchard}}$ and $R^{\mathsf{Orchard}}$, and
the binding signature (\emph{Ironwood Guide}, \S``Conservation without
disclosure: value commitments and the binding signature''); the Action
statement and dummy
spends (\emph{Ironwood Guide}, \S``The Action statement, formally'');
nullifiers (\emph{Ironwood Guide}, \S``The nullifier, and the loop back to
minting''); Sinsemilla (\emph{Ironwood Guide}, \S``Sinsemilla: hash,
commitment, and short forms''); and, at the wallet layer, transaction
construction, PCZT, and
...
the current \textsf{zips} tree, ZIP-204 assigns NU7 protocol versions
$170180$ (Testnet) and $170190$ (Mainnet), but
\texttt{draft-arya-deploy-nu7} still says $170150$ and $170160$; the draft is
therefore internally stale even though its activation heights remain TBD
(\textsf{zips} \texttt{e753a6a3}, ZIP-204 and
\texttt{draft-arya-deploy-nu7}). The Zcash Foundation's February 2026 NU7
sentiment polls record split support
for ZSAs: ZCAP supported them at 70.4\%, the coinholder poll ran 98.6\%
...
Status Proposed, specified for deployment with NU6.3 ``Ironwood'') applied,
and states that
...
name ``Orchard Quantum Recoverability''; \texttt{zip-2005.md} header). The
dependency points upward: if the NU6.3 content changes, the ZSA specification
inherits the change.
...
home for the ZIP~230/231/246 material, merged as a Draft on 2026-09-03
(zcash/zips\#1156, commit \texttt{e753a6a3}). It defines a generic V7 carrier
and lists OrchardZSA and ZSA issuance only under ``Potential Future Bundle
Types'': the former has a provisional variant and the latter no allocated
bundle type. It does not specify their V7 encodings or digests. The rot also
reaches into the ZSA texts themselves:
...
\texttt{zsa1}-vs-\texttt{main} diff (PR QED-it/orchard\#7) for compatibility
with the ZIPs, excluding the circuit from that comparison; the QED-it
\texttt{halo2} gadget changes (PR QED-it/halo2\#18); and the Orchard
\texttt{src/circuit} folder, including dependencies that differed from
upstream. The initial revisions were Orchard \texttt{a7c02d22} and Halo~2
\texttt{290bc565}; verification used Orchard \texttt{01e85a5f}. The report
found no issues and made four suggestions, three resolved and one planned.
...
\texttt{cf801a5d}. Testnet: QED-it's \texttt{zcash\_tx\_tool} repository
documents public Zebra endpoints hosted by QED-it and test flows for issuance,
transfer, and burn (HEAD \texttt{655298f4}, README and the Running tx-tool
wiki, checked 2026-09-05,
\url{https://github.com/QED-it/zcash_tx_tool}); its README labels the tool
Alpha and says its feature set is incomplete. ZecHub had described a
\emph{feature-complete version} on that testnet on 2025-12-17
(\url{https://zechub.substack.com/p/zcash-shielded-news-vol61}), but that
historical description is not promoted to a claim about the current tool.
This volume accordingly asserts \emph{audited} only of the scoped Orchard and
Halo~2 code at those revisions, and only the existence of QED-it's public test
environment for the stack as a whole.
...
commits; the web sources cited are pinned by URL and date --- the Least
Authority OrchardZSA audit report (2025-01-30), QED-it's public-testnet
documentation (HEAD \texttt{655298f4}, checked 2026-09-05), ZecHub
\emph{Shielded News} Vol.\,61 (2025-12-17), and the Zcash Foundation's NU7
Polling Results post (2026-02-23), all in \S\ref{sec:zsa-status}; and the
unversioned \emph{OrchardZSA circuit} document linked from ZIP~226 (checked
2026-09-05), used only to bound the circuit description's scope in
...
\paragraph{Current-head boundary (2026-09-05).} The QED-it forks have moved
since the source snapshot: \textsf{orchard} \texttt{837ddd1} is based on
0.15.0, adds the NU6.3 cross-address restriction to the ZSA circuit, and
includes later internal circuit refactors, while \textsf{librustzcash}
\texttt{c5c232d} synchronises the NU6.2-era upstream transaction changes.
Upstream ZIP~248 is now a merged Draft for a generic V7 carrier, but it leaves
the ZSA-specific bundle encodings and digests to future specifications;
ZIPs~230 and~246 remain withdrawn, ZIPs~226 and~227 remain Draft, and the NU7
deployment draft still does not select them. Detailed wire, circuit, and fee
claims below therefore continue to describe the explicit pins in
Table~\ref{tab:zsa-sources}; later fork behaviour is not silently attributed
to that public-testnet snapshot.
...
This volume classifies every design-stage claim as \emph{specified},
\emph{designed but unspecified}, or an \emph{open problem}, and keeps the bin
visible in the prose. Bin~(i) holds the constructions and consensus rules that
the live ZIP texts state directly: the per-asset note and commitment, Asset
Base derivation, value commitments and balance equation, the split-note
circuit delta, issuance keys, issue and reference notes, the issuance state
machine, and burn. Bin~(ii) holds both the gate-level realization of that
circuit delta and the carrier layer: the v6 wire format, digest tree and
sighash, note-plaintext lead byte, and ZSA fee terms. Those details are fixed
only by implementation, an externally hosted unversioned circuit description,
withdrawn ZIPs, or the QED-it fork texts (\S\ref{sec:zsa-status}). Bin~(iii)
holds NU7 inclusion and activation, the
eventual lead-byte assignment, and the carrier format's future home. Formulas
below identify whether their source is ZIP text, pinned code, or an explicitly
labelled derivation in this volume.
...
derivation chain,
\begin{gather*}
\texttt{asset\_desc} \;\longrightarrow\; \mathsf{assetDescHash};\\
(\mathsf{issuer}, \mathsf{assetDescHash})
...
(\texttt{zip-0227.rst}:210--224): the issuer-chosen description string is
hashed, the hash paired with the issuer key into an identifier, the
identifier hashed into a digest, and the digest mapped onto the Pallas
curve. Every Asset issued under ZIP~227 is scoped to the issuer that issued
it. Its Asset Identifier is intended to be globally unique and is ``used
across all Zcash protocols that support ZSAs''
(\texttt{zip-0227.rst}:227--240); for two descriptions under one issuer,
uniqueness is computational and rests on BLAKE2b-256 collision resistance.
...
$\bot$ if $\mathsf{Sign}$ fails; its scheme-level encoding is
...
most $252$ bytes with at least $256$ bits of entropy, as ZIP~32's
``Specification: Wallet seeds'' requires.
...
error \texttt{NonZeroAccount}. The
...
The byte $\mathsf{Issuance.}\allowbreak\mathsf{CKDDomain}$
$= \mathtt{0x81}$ equals
$\mathsf{Orchard.CKDDomain}$ (\texttt{zip-0032.rst}:476), and the
implementation reuses \texttt{PrfExpand::\allowbreak ORCHARD\_ZIP32\_CHILD}
...
identifier is a component of $\mathsf{AssetId}$, two distinct issuer-key
encodings cannot produce the same pair; in the ZIP's words, the Custom Asset
...
issuer, distinct descriptions separate assets via $\mathsf{assetDescHash}$
only subject to BLAKE2b-256 collision resistance; for every new Asset there
MUST be a new and unique Asset Identifier (\texttt{zip-0226.rst}:93--96).
...
client configuration, or the Asset Digest as a compact identifying byte
...
matching $\mathsf{EncodeAssetId}$. The function
...
$\GroupHash^{\PP}$ (\href{https://zips.z.cash/protocol/protocol.pdf\#concretegrouphashpallasandvesta}{\S5.4.9.8}, ``Group Hash into Pallas and Vesta''),
...
the isogeny --- the \emph{Ironwood Guide} develops in ``GroupHash, domain
separation, and nothing-up-my-sleeve generators''; we reuse it as a
deterministic,
...
$\Ep(\F_{p_{\Pallas}})$ in our notation. Here uniqueness can only mean that
the derivation is deterministic for a given identifier, not that the map is
injective: a $512$-bit digest is mapped into a roughly $255$-bit group, so
collisions exist in principle. Asset separation at the base and global-state
levels is computational and requires that collisions in the digest/group-hash
chain be infeasible to find. Its byte form is
\[
\texttt{asset\_base} :=
...
(\texttt{zip-0226.rst}:\allowbreak 114;
\texttt{zip-0230.\allowbreak rst}:\allowbreak 540--545). ZIP~226 notes
...
(\texttt{protocol.tex} \href{https://zips.z.cash/protocol/protocol.pdf\#concretegrouphashpallasandvesta}{\S5.4.9.8}; the \emph{Ironwood Guide},
\S``GroupHash, domain separation, and nothing-up-my-sleeve generators'',
constructs the hash-to-curve pipeline). ZIP~227's $\mathsf{ZSAValueBase}$ never
...
is used as
$\mathsf{ValueCommit}^{\mathsf{OrchardZSA}}_{\rcv}$\allowbreak
$(V^{\mathsf{Orchard}},$ $v)$''
(\texttt{zip-0226.rst}:\allowbreak 182--194). The point
$V^{\mathsf{Orchard}} :=$
$\GroupHash^{\Pallas}$\allowbreak$(\mathtt{z.cash:Orchard\text{-}cv},$
$\texttt{"v"})$ is
already constructed, together with its companion $R^{\mathsf{Orchard}}$,
in the \emph{Ironwood Guide}, ``Conservation without disclosure: value
commitments and the binding signature'', Definition ``Net value commitment'';
we cite
...
\texttt{ValueCommitment::\allowbreak derive} computes $[v]\,V + [\rcv]\,R$ with
...
preceded by a $\mathtt{0x00}$ byte indicating BIP~340. The first four
fields are always present; \texttt{issueAuthSig} is present iff
$\texttt{nIssueActions} > 0$, and if $\texttt{nIssueActions} = 0$ then
\texttt{issuerLength} MUST be $0$ (\texttt{zip-0230.rst}:166--179,
215--217, 430--443). Each
Issue Action carries \texttt{assetDescHash} as \texttt{byte[32]},
\texttt{nNotes} (compactSize, possibly zero; for an existing Asset, this
permits finalization without further issuance), \texttt{vNotes} ($115$ bytes
per note description: $43$-byte
...
$\mathsf{rseed}$ and the memo key (the $32$-byte key that replaces the
in-band $512$-byte memo under ZIP-231 and from which the recipient
derives the key decrypting its memo in the transaction-level memo
bundle; \S\ref{sec:zsa-carrier}),
\[
...
carried on the wire for issuance, always re-derived. Redundantly
...
otherwise. The Asset Identifier is therefore not serialized as a
standalone field: its two components are consensus-visible, and the Asset Base
is a recomputable function of them, anchored to the issuer key that signs the
bundle.
...
The transfer mechanism below moves Custom Assets between shielded notes; this
section brings them into existence. ZIP-227 (Status Draft, Category Consensus;
...
(ZIP-227, header and closing sections @ zips \texttt{69610984}). Deployment
standing --- Draft ZIPs, externally audited at the stated Orchard/Halo~2 scope,
exercised in QED-it's Alpha public test environment, contested for NU7 --- is
fixed once in \S\ref{sec:zsa-scope} and not relitigated here.
...
be tracked'' (ZIP-227, \emph{Motivation}). The issuer, Asset, issued amounts,
finalization status, raw recipient addresses, $\rho$ values, and random seeds
are serialized in the clear. For an ordinary recipient, the address does not
reveal the nullifier key, so a later spend is designed to be unlinkable to the
public issue-note description. The mandatory all-zero-key reference note is an
explicit exception (\S\ref{sec:zsa-issuance-notes}).
...
warnings @ zips \texttt{69610984}) --- and ZIP-229 carries no
...
as what it is: the fork-specified design, exercised in QED-it's public test
environment (\S\ref{sec:zsa-status}) and
implemented by the pinned fork libraries (at an older Orchard/Halo~2 revision
audited by Least Authority, 2025-01-30), with no merged upstream v6
...
\paragraph{Implementation pins.} Claims about implemented behavior cite the
...
independent of the spending key tree but derived analogously (via
ZIP~32), so an organization can delegate issuance without exposing
funds, and a future scheme can replace the issuance leaf with a
multisignature without touching spending keys (ZIP-227,
\emph{Rationale}, and \emph{Issuance Key Derivation}).
...
Every Custom Asset is named by its issuer plus an issuer-chosen description,
and consumed by the protocol as a Pallas point. The full
...
(\S\ref{sec:zsa-native-asset}). ZIP~226 excludes the Pallas identity from
the Asset-Base type; \texttt{AssetBase::custom} panics after hashing to it,
while \texttt{AssetBase::from\_bytes} rejects it on parse
(Remark~\ref{rem:zsa-identity-point}). What the issuance layer adds
...
\subsection{Issue notes and the derived \texorpdfstring{$\rho$}{rho}}%
...
Issue-note commitments are not serialized: the wire format carries the note
fields themselves in the clear (\S\ref{sec:zsa-issuance-wire}), and each
validator computes the commitment --- by the OrchardZSA $\NoteCommit$ of
\S\ref{sec:zsa-transfer} --- and appends it to the Orchard note commitment
tree. For an ordinary issue note, the recipient address does not reveal the
nullifier key; under the nullifier PRF assumptions, a later nullifier is
computationally unlinkable to that public description. This privacy claim does
not apply to the reference note, whose all-zero spending key is public
(ZIP-227, \emph{Issue Note}, \emph{Reference Notes}, and \emph{Issuance
Protocol}). Issuance is transparent at creation; later transfers of ordinary
issue notes are shielded.

\paragraph{Computation of $\rho$.} An Orchard note takes
$\rho := \nfsym_{\mathrm{old}}$ from the Action that creates it (the
\emph{Ironwood Guide}, \S``Delivery: the note reaches its owner''); an issue
note has no such Action, so ZIP-227 derives $\rho$ from the
...
bundle (the v6 format partitions a bundle's Actions into one or more
Action Groups, each carrying its own anchor and proof;
\S\ref{sec:zsa-v6}), and the two indices are the zero-based positions of
the Issue
...
$\mathrm{ToBase}$. This derivation is why an issuance
...
resamples $\mathsf{rseed}$ until both the derived ephemeral key is valid and
the note commitment is not $\bot$\footnote{Strictly, these
negligible-probability rejection steps condition $\mathsf{rseed}$, a
technical deviation from the ZIP's ``sampled uniformly at random''
(ZIP-227, \emph{Issue Note}).}
...
The reference note is retained in global state as
...
\texttt{nNotes} & \texttt{compactSize} &
  may be $0$ (existing-Asset finalization) \\
...
(\S\ref{sec:zsa-issuance-notes}). Second, the encoding permits
\texttt{nNotes} $= 0$; for an existing Asset this lets the issuer finalize
without issuing more (ZIP-230, \emph{Issuance Action Description}; the
new-Asset corner case is recorded in \S\ref{sec:zsa-issuance-state}). Third, the
...
standard parameters, not RedPallas; ZIP-227 gives no rationale for that curve
choice. Batch verification MAY be used, and precomputation MAY
...
$\mathsf{SigHash}$ as defined by protocol specification \href{https://zips.z.cash/protocol/protocol.pdf\#sighash}{\S4.10} under the
...
\texttt{decode()} rejecting any other lead byte.
...
notes' $\rho$ values and the sighash exist. On the transaction
...
signatures sign. The builder also realizes the $\nfsym_{0,0}$ rule
...
The v6 digest tree extends the ZIP-244 structure the \emph{Ironwood Guide}
presents in \S``What the signatures sign: ZIP-244 digests and their v6
form'': the txid digest is BLAKE2b-256 with personalization
...
The code computes exactly these issuance-specific hashes. On the txid side,
\texttt{hash\_issue\_\allowbreak bundle\_txid\_data} hashes
\[
...
side, \texttt{hash\_issue\_\allowbreak bundle\_auth\_data} hashes
\[
...
\texttt{issuance\_\allowbreak auth\_\allowbreak sig.rs}), exercised by the
asset-base and authorization tests (\texttt{orchard-zsa},
\texttt{src/\allowbreak bundle/\allowbreak commitments/\allowbreak
issuance.rs} lines~156--190 and
...
2^{64} - 1$ caps the recorded circulating balance of any Custom Asset at any
one state (ZIP-227,
...
The fold realizes the per-action and per-note rules
as typed errors:
\begin{itemize}
...
\item \texttt{IssueBundleIkMismatchAssetBase} and
  ...
  Pallas identity. The latter variant is intended but unreachable at this
  pin because \texttt{AssetBase::custom} panics first
  (Remark~\ref{rem:zsa-identity-point});
...
\item \emph{Empty-action divergence.} The implementation rejects an
  ...
  \texttt{nNotes} may be zero ``to only finalize''. For an already-existing
  Asset, the written consensus rules admit an empty, non-finalizing action
  that the reference implementation rejects; for a new Asset, the ZIP's
  instruction to take the first note as the reference note is undefined when
  the list is empty.
...
\item \emph{NFTs}: issue multiple actions in one transaction, each with
  ...
  $\mathsf{finalize} = 1$ --- each action defines a distinct finalized Asset
  Identifier with supply one, subject to the collision-resistance assumptions
  of \S\ref{sec:zsa-asset-id}.
\end{itemize}
...
\texttt{69610984}) --- while ZIP-227's own \emph{Changes to ZIP 317}
...
\texttt{cfg(zcash\_unstable = "nu7")}
(\texttt{transaction/fees/zip317.rs},
...
computational per-asset balance check and gives burn its consensus meaning
...
``The note and its commitment'' --- by exactly one
field:
\[
\mathbf{n} :=
...
other components are as in protocol specification \href{https://zips.z.cash/protocol/protocol.pdf\#notes}{\S3.2}. Formally,
\[
\begin{aligned}
...
In a transfer, an Asset Base is not an arbitrary point chosen by the
transactor. Every Custom Asset's base descends from the ZIP-227 issuance chain
--- constructed in
...
without disclosure: value commitments and the binding signature'' (ZIP-226).
This
...
baseline layout of the \emph{Ironwood Guide}, ``The note and its commitment'':
\[
...
\texttt{""})$. ZIP-226's rationale says that ``the nested structure of the
Sinsemilla-based commitment allows us to add the Asset Base as a final
recursive step'', and calls the output ``still indistinguishable from the
original Orchard ZEC note commitments''. More precisely, the common output
type lets ZSA commitments inhabit the same note commitment tree, while their
computational indistinguishability rests on the hiding and hash assumptions of
the randomized Sinsemilla commitment (ZIP-226). One caveat bounds the claim:
Orchard (v5) note commitments and OrchardZSA (v6) note commitments are
contextually distinguishable when observed in transactions, because the two
protocols use different transaction versions (ZIP-226).
...
them in constant time on \texttt{asset.is\_zatoshi()}.
The \textsf{sinsemilla} crate appends the
\texttt{-M} and \texttt{-r} suffixes to these prefixes.
\end{remark}

The nullifier of an ordinary (non-split) OrchardZSA note is generated
exactly as in Orchard (protocol specification \href{https://zips.z.cash/protocol/protocol.pdf\#rhoandnullifiers}{\S4.16}; \emph{Ironwood Guide},
``The nullifier, and the loop back to minting''); only Split Inputs use the
...
V^{\mathsf{Orchard}} + [\rcv]\,R^{\mathsf{Orchard}}$ of the \emph{Ironwood
Guide}, ``Conservation without disclosure: value commitments and the binding
signature'': the fixed-base multiplication by
...
of its base --- displayed in this volume as \eqref{eq:zsa-bvk}
(\S\ref{sec:zsa-split-attack}). The verifier checks one Pallas-group
equation, not independent algebraic coordinates for the assets. Since Pallas
is cyclic, every pair of non-identity bases has some discrete-log relation;
the separation is computational. After the randomness terms cancel, any
nonzero per-asset imbalance that still verifies yields a useful relation among
the independently group-hashed Asset Bases, $V^{\mathsf{Orchard}}$, and
$R^{\mathsf{Orchard}}$. Under the discrete-log and hash-to-curve assumptions,
finding such a relation is infeasible. Subject to those assumptions, the
binding signature therefore enforces that each Custom Asset's net Action
value equals its $\mathsf{assetBurn}$ entry (or $0$ if absent), while the ZEC
component equals $v_{\mathrm{balance}}^{\mathsf{Orchard}}$. This is the
computational content of ZIP-226's ``per Asset Base'' claim; its wording that
Pedersen commitments add homomorphically only for the same base is not a
literal group-theoretic statement. The
...
\emph{Ironwood Guide}, ``Conservation without disclosure: value commitments
and the binding signature'', and we do not restate them.
...
\texttt{value\_balance} at \texttt{AssetBase::\allowbreak zatoshi()}
minus, over the
burn set, the zero-trapdoor commitment of each \texttt{value} at its
\texttt{asset}. Function
...
the value signed per the sign of the value sum.
\end{remark}
...
contained within the shielded pool'' (ZIP-226). Burning is the only
supply-reduction path. What a burn reveals on the wire is exactly its
$\mathsf{assetBurn}$ entry: the $32$-byte Asset Base and amount --- what
ZIP-226 calls ``the amount and identifier of the Asset being burnt''. In a
transfer, ``the overall balance is checked without revealing which (or how
many distinct) Assets are being transferred''; under the note-encryption and
commitment-hiding assumptions, the transferred amounts and Asset Bases remain
private (ZIP-226).
...
\S\ref{sec:zsa-split-input} (\texttt{zip-0226.rst}:287--291). Burn entry
...
A bin statement first. ZIP-226's ``Circuit Statement'' section specifies the
constraint delta in prose and formulas, while deferring ``all modifications in
the Circuit'' to the externally hosted, unversioned document \emph{OrchardZSA
circuit}. That document describes the public and private inputs, intermediate
values, and constraints, but the ZIP does not embed or pin a complete statement
at the rigor of the \emph{Ironwood Guide}'s Definition ``Orchard Action
statement, Ironwood circuit (NU6.3)''. The pinned implementation is therefore
the reproducible source for the exact gate realization, and every gate-level
claim below cites it (\textsf{orchard-zsa},
\texttt{src/circuit/circuit\_zsa.rs}). Bin: \emph{externally described and
implemented, but not fixed by a versioned upstream specification}.
...
$[\rcv]\,R^{\mathsf{Orchard}}$
(\texttt{src/\allowbreak circuit/\allowbreak value\_commit\_orchard.rs}).
...
\texttt{src/\allowbreak constants/\allowbreak sinsemilla.rs}) on
$\mathsf{is\_zatoshi\_asset}$,
...
$\mathsf{enable\_zsa}$ is always false; the instance-construction API pads the
missing tenth element with zero, preserving the vanilla flavor's public-input
semantics. The crate carries two circuit flavors,
...
sections (\href{https://zips.z.cash/protocol/protocol.pdf\#orchardsend}{\S4.7.3} ``Sending Notes'', \href{https://zips.z.cash/protocol/protocol.pdf\#orcharddummynotes}{\S4.8.3} ``Dummy Notes'') are invoked
...
\texttt{src/\allowbreak primitives/\allowbreak
zcash\_note\_\allowbreak encryption\_\allowbreak domain.rs},
\texttt{src/\allowbreak primitives/\allowbreak orchard\_primitives.rs}).
No lead byte is currently
reserved for ZSA notes upstream. Bin: \emph{open}.
\end{remark}

\paragraph{Digests.} Withdrawn ZIP-246 extends the ZIP-244 digest tree ---
the baseline is the \emph{Ironwood Guide}, ``What the signatures sign:
ZIP-244 digests and their v6 form'' --- with a per-Action-Group
...
T.1; ZIP-230, Transaction Format).
...
\paragraph{Fees.} The QEDit fork of ZIP-317 adds
$\mathsf{contribution\_ZSAIssuance}$ and
$\mathsf{contribution\_ZSACreation}$ terms --- issue notes and asset
creations respectively --- to the conventional-fee formula; it adds no
burn term, so a burn contributes to fees only through the Actions that
fund it. Upstream
...
An Orchard bundle pairs inputs and outputs into Actions by padding the shorter
side with \emph{dummy notes}: an Action with no real input spends a zero-valued
note under a freshly sampled key, and the membership constraint waves it
through because the check is gated by the spent value --- constraint A3 of the
Action statement,
...
spends are accepted by design). The construction is protocol \href{https://zips.z.cash/protocol/protocol.pdf\#orcharddummynotes}{\S4.8.3},
implemented in \texttt{orchard-zsa},
...
$V^{\mathsf{Orchard}}$ (\emph{Ironwood Guide}, ``Conservation without
disclosure: value commitments and the binding signature''): a dummy spend can
move no
...
ZIP-226 substitutes for the dummy note on custom assets, the ZIP-specified
circuit delta, and the implemented gate that draws the counterfeiting
boundary.
...
net value commitment (``Conservation without disclosure: value commitments and
the binding signature''), unchanged. The native asset keeps the old base:
``the Asset Base of the ZEC Asset will be kept as the original value base
point $\mathcal{V}^{\mathsf{Orchard}}$'' (ZIP-226, ``Asset Identifiers'';
in code \texttt{AssetBase::zatoshi()} is
$\GroupHash^{\Pallas}(\mathtt{z.cash:Orchard\text{-}cv},$ $\texttt{"v"})$).
Verification subtracts the
...
verifies --- and a $v$-valued ZEC note now exists that no one paid for.
Suitably chosen known relations among accepted bases can be exploited
analogously.
...
value commitment must either be the fixed native base or descend from an
accepted $\GroupHash$ output. Security then assumes that no useful
discrete-log relation among that base, $V^{\mathsf{Orchard}}$, and
$R^{\mathsf{Orchard}}$ can be found. The one place an unproven base could
otherwise enter is an Action with no real input.
...
637--655 takes $v_{\mathrm{old}}$ as witnessed). The Action therefore neither
moves value from the copied note nor publishes that note's ordinary nullifier;
instead it publishes the randomized split nullifier of
\S\ref{sec:zsa-split-nf}. The copy drags the original note's asset base into
the proof, where an equality constraint passes it to the output note.

The split Action participates in the same computational per-asset binding
argument as \S\ref{sec:zsa-value}: a successful imbalance would require a
useful discrete-log relation among the independently derived bases. For a
correct transaction the custom-asset commitments satisfy
\[
...
of $0$ when $\mathsf{split\_flag}$ is set, and \texttt{derive\_bvk\_raw}
...
\S\ref{sec:zsa-asset-base} (ZIP-227, ``Asset Bases''),
...
notes (ZIP-227, ``Addition to the Note Commitment Tree''). Hence, under
note-commitment binding, a custom-asset note commitment resident in the tree
commits, by induction along the two rules, to a genuine $\GroupHash$ output.
ZIP-226 states the
...
$\mathsf{flagsOrchard}$ field), with the constraint
...
(\texttt{src/\allowbreak circuit/\allowbreak circuit\_zsa.rs}, lines
68--185, versus
...
of $\psi^{\nfsym}$ is always constrained through the A5 equation to the
published nullifier. Only the equality
$\psi^{\nfsym}=\psi_{\mathrm{old}}$ is gated off in the split case; no
constraint derives $\psi^{\nfsym}$ from a seed. How a wallet samples it for a
split note is therefore wallet-level, not consensus-level. The three extant
texts disagree; per the volume's policy (\S\ref{sec:zsa-sources}), the upstream
...
\texttt{d5e84264}). Direct field sampling is uniform; the two PRF-and-reduction
constructions are intended to be pseudorandom and have at most the negligible
modular-reduction bias of $\mathrm{ToBase}$. The implementation matches neither
ZIP text exactly. (\emph{Classification:}
each derivation is specified in its own text; the divergence is
consensus-invisible.)

\paragraph{Why the offset $L^{\mathsf{Orchard}}$?} The randomised
$\psi^{\nfsym}$ is intended to make the split nullifier unlinkable to the
...
elsewhere in the transaction. The constant offset makes the pre-extraction
group point different even under that choice. It is not an absolute
noncollision proof for the extracted nullifier, since elliptic-curve points
with opposite signs have the same extracted $x$-coordinate.
(\emph{Classification:}
...
(\S\ref{sec:zsa-split-input}). Both the proof and the Action signature narrow
that claim. Spend-authority consistency (A6) and address integrity (A7) are
unchanged and unconditional in the ZSA circuit: the proof establishes
...
\nksym, \rivk)$ consistent with it --- the full-viewing-key components or an
equivalent witness. More decisively, every Action must carry a RedPallas spend
authorization signature under $\rk$. A split spend has
\texttt{dummy\_sk = None}; preparation auto-signs only true dummy spends, and
\texttt{apply\_signatures} finishes a split Action only after receiving a
\texttt{SpendAuthorizingKey} whose validating key matches $\aksym$, otherwise
returning \texttt{MissingSignatures} (\texttt{src/builder.rs}, lines~308--315
and 1347--1432, \texttt{SpendInfo::create\_split\_spend},
\texttt{Bundle::prepare}, \texttt{Bundle::apply\_signatures}, and
\texttt{Bundle::sign}). Thus constructing a valid transaction
requires the copied note's spend-authorizing key, absent a signature forgery
or discrete-log break. The ZIP's sentence holds for \emph{spentness} --- a
nullified note can still be copied because its value is zeroed --- but not for
ownership. The builder consequently copies the first real spend of that Asset
in the same bundle, option (1) of \S\ref{sec:zsa-split-input}
(\texttt{src/builder.rs}, \texttt{pad\_spend}). (\emph{Classification:}
the circuit and signature behavior are implemented; the ZIP rationale
overstates the available choices.)
...
anyone can satisfy A6/A7 and produce the Action signature for the reference
note, which makes it a
...
real spend of that asset in the bundle. If the bundle spends none,
\texttt{pad\_spend} produces \texttt{BuildError::NoSplitNoteAvailable}; the
current per-asset iterator unwraps that result and panics with the error rather
than returning it from \texttt{bundle\_for\_version}. Unlike a dummy, a split
input cannot be conjured from nothing. Output-side padding uses dummy outputs
of the padding asset in both regimes.
...
viewing key and Merkle path, sets no dummy signing key, and raises
$\mathsf{split\_flag}$
...
factor); under note-commitment binding, every custom-asset tree leaf descends
from a publicly recomputed $\GroupHash$ output (the issuance anchor of
\S\ref{sec:zsa-split-membership}); and the copy that makes padding
possible moves no value (the zeroed $v'$). Those links enforce the
counterfeiting boundary of \S\ref{sec:zsa-split-attack}. Independently, the
randomized nullifier \eqref{eq:zsa-split-nf} avoids publishing the
original note's ordinary nullifier except with negligible collision
probability under honest randomisation; it is not an absolute
noncollision guarantee (\S\ref{sec:zsa-split-nf}).
...
ZIPs, externally audited at the stated Orchard/Halo~2 scope, exercised in
QED-it's Alpha public test environment, contested for NU7 --- is stated once
in the scope section (\S\ref{sec:zsa-status}) and not relitigated per object;
...
stated commits, most recently on 2026-09-05.
...
\midrule
\texttt{zcash/zips} & \texttt{69610984} & 2026-07-09
  & upstream ZIP texts \\
...
\bottomrule
\end{tabular}
\caption{Pinned sources for this section. The upstream tracking branch excluded
in \S\ref{sec:zsa-sources} contains no ZSA implementation and is not a source.}
...
Under the three-bin classification of \S\ref{sec:zsa-method}, the byte-level
format below is fixed at ZIP rigor only by withdrawn upstream text and pinned
fork code --- \emph{designed but unspecified} ---
...
Format). Making the fee an explicit field replaces its derivation as the
residual difference between transaction input and output value and lets
clients read it without resolving prior transparent outputs (ZIP-2002).
Separately, signatures become versioned below. The transparent fields add
...
\texttt{nActionGroups\allowbreak Orchard} $> 0$; an absent
\texttt{valueBalanceOrchard}
...
atomic swaps. ZIP-228 is now a Draft that uses this field to expire swap
orders, but it still depends on the withdrawn ZIP-230/246 carrier; for the
core OrchardZSA proposal the field is zero
...
(\texttt{uint64}). ZIP-230 says the amount is non-zero and strictly less than
$\mathsf{MAX\_BURN\_VALUE}$, but ZIP-226's consensus rule and the pinned code
permit equality (\S\ref{sec:zsa-burn}).
...
finalize-only action for an existing Asset is expressible; the new-Asset
corner case remains undefined in ZIP~227
(\S\ref{sec:zsa-issuance-state}). An \texttt{IssueNoteDescription} is
...
(ZIP-230, consensus rules). Its mining discussion says draft ZIP-235 would
require all coinbase transactions to use this v6 format from that proposal's
then-planned NU7 activation (ZIP-230, Implications for Mining); neither
withdrawn ZIP-230 nor draft ZIP-235 supplies a current deployment rule.
...
in the \emph{Ironwood Guide}, \S``What the signatures sign: ZIP-244 digests
and their v6 form'' --- from four top-level branches to six:
\[
...
T.3b.i & \texttt{ZTxId6SOutC\_Hash} & Sapling v6 outputs, compact \\
T.3b.ii & \texttt{ZTxId6SOutN\_Hash} & Sapling v6 outputs, non-compact \\
...
\paragraph{Signature digest.} The sighash reuses the tree with the
transparent branch replaced by an input-specific variant:
\[
\mathsf{sighash} := \mathrm{BLAKE2b\text{-}256}\bigl(
\texttt{ZcashTxHash\_} \,\|\, \mathsf{branchid},\;
d_{\mathrm{hdr}} \,\|\, d^{S}_{\mathrm{transp}} \,\|\,
d_{\mathrm{Sap}} \,\|\, d_{\mathrm{Orch}} \,\|\, d_{\mathrm{issue}}
\,\|\, d_{\mathrm{memo}}\bigr),
\]
produced per transparent input, per Sapling input, per OrchardZSA Action,
and --- new in v6 --- per Issue Action; the Sapling, Orchard, issuance,
and memo branches are identical to their txid-side counterparts, so the
issuance bundle and the memo bundle enter every sighash (ZIP-246,
Signature Digest).
On this digest ZIP-227 hangs its authorization rule: the SigHash is the
\href{https://zips.z.cash/protocol/protocol.pdf\#sighash}{\S4.10} SIGHASH transaction hash with the ZIP-226 modifications, computed
...
Changes). The vector form is used by draft ZIP-228's proposed ZSA swaps,
which would relax the one-group rule for swap bundles; it is not active in
the core ZIP-227 rules.
...
Issue Action and note indices (ZIP-227, Computation of $\rho$). The indices
domain-separate notes within the transaction, while the nullifier-keyed PRF
makes collisions across transactions negligible under its pseudorandomness
assumption; it does not give literal global uniqueness.
...
nullifier that consensus consumes (ZIP-227, Rationale).
...
declared balance from the sum of net value commitments (the \emph{Ironwood
Guide}, \S``Conservation without disclosure: value commitments and the binding
...
\rcv_i$ (ZIP-226, Burn Mechanism). Under the unknown-discrete-log assumptions
made explicit in \S\ref{sec:zsa-value}, the signature computationally enforces
that every non-ZEC asset nets to exactly its declared burns.
...
issuance state (fork ZIP-317, Fee calculation). The rationale is a one-time
charge for state growth: each new asset adds an $\mathsf{issued\_assets}$ row
tracked in perpetuity, while subsequent issuance, finalization, and burn only
mutate the row. The implementation matches the fork text:
\texttt{zcash\_primitives}' \texttt{transaction/fees/zip317.rs} defines
...
Table~\ref{tab:zsa-personalizations}, asserting the $\mathtt{0x00}$
...
under the global issuance state; and
...
\texttt{RAW\_REFERENCE\_RECIPIENT} constant equals the array in ZIP-227. The
...
\paragraph{Divergences from the withdrawn ZIP text.} Four are material. The
first three block byte-compatibility with ZIP-230/246 as written; the fourth
changes private witness construction but is not enforced by the circuit.
...
QED-it \texttt{orchard} \texttt{zsa1}-vs-\texttt{main} diff for ZIP
compatibility (PR QED-it/orchard\#7, excluding the circuit from that
comparison), the QED-it \texttt{halo2} changes (PR QED-it/halo2\#18), and
the Orchard \texttt{src/circuit} folder. Initial revisions were Orchard
\texttt{a7c02d22} and Halo~2 \texttt{290bc565}; verification used Orchard
\texttt{01e85a5f}. The report identified no issues. QED-it's
current test-tool documentation exposes public Zebra endpoints and labels the
tool Alpha with an incomplete feature set; the earlier ZecHub report's
\emph{feature-complete} description is retained only as a dated report.
Sources are pinned in \S\ref{sec:zsa-status}. Neither the audit nor those
testnet claims establish coverage of \texttt{librustzcash-zsa} or the v6
carrier layer, and the audited revisions predate \texttt{cf801a5d}.
...
The withdrawn ZIP-230 and ZIP-226 design imposes two obligations. First, all
wallets SHOULD switch to sending only its v6 transactions once permitted: a
v5 transaction reveals that all its Orchard notes are ZEC, and the design's
v6 OrchardZSA output format incorporates the quantum-recoverability changes
(ZIP-230, Implications for Wallets; ZIP-226, Backwards Compatibility with ZEC
Notes). This is a claim about the withdrawn carrier, not the different v6
format now assigned to ZIP-229. Second, wallets MUST support parsing and
...
and lists \emph{no} ZIPs. Those version numbers now conflict with ZIP-204's
NU7 assignment of $170180$ and $170190$, so the deployment draft needs an
editorial update. The repository's NU7 candidate list (ZIPs 218,
...
cross-cutting respecification problems.
\begin{enumerate}[itemsep=2pt]
\item \emph{A carrier.} Transaction version 6 now belongs to NU6.3's
  ...
  ZIP-248, now a merged Draft for V7. Its registry lists OrchardZSA with a
  provisional variant and ZSA issuance without an allocated bundle type,
  under ``Potential Future Bundle Types''; it does not specify either
  bundle's encoding or digest. The ZSA-specific transaction format therefore
  remains incomplete: if ZSAs are selected for NU7, the byte format of
  \S\ref{sec:zsa-v6} must be mapped or respecified for V7, and
  \S\ref{sec:zsa-digests} re-anchored to the resulting digest rules.
\item \emph{A base protocol.} Upstream ZIP-226 is now ``defined relative to
  ...
  activation $4{,}134{,}000$, Mainnet $3{,}428{,}143$) introduces the
  Ironwood pool and itself restricts transfers into the Orchard pool; its
  intended supporting ZIP-2006 remains only a Reserved stub. No ZIP specifies
  whether OrchardZSA then targets the restricted
...
\item \emph{Reconvergence of spec and code.} The divergence list of
  ...
  respecified ZIPs adopt what the code does, or the code moves. Each
  divergence is identified, but resolving it still requires protocol choices
  and implementation work.
\end{enumerate}

The summary judgment this volume can defend: a ZSA transaction layer is
designed and implemented in the pinned forks, but the deployable protocol is
\emph{currently unspecified} in the sense this series requires --- there is
no complete merged carrier specification under which a mainnet node could
implement it. The byte-level facts of this section are
fixed by withdrawn ZIP text and pinned fork code, and they will remain the
diff base for any respecification; the remaining open items are enumerated
above and in the scope section.
