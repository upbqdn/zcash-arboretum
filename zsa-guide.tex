...
(\textsf{zips} \texttt{ad30e59}, ZIP-204 and
\texttt{draft-arya-deploy-nu7}). In
the Zcash Foundation's February 2026 NU7 sentiment polls, the proposal that
drew ``universal or near-universal support'' was Tachyon, not ZSA. ZSA's
own standing in those polls is split: ZCAP supported ZSAs at 70.4\%, the
coinholder poll ran 98.6\%
...
\paragraph{Current-head boundary (2026-08-30).} The QED-it forks have moved
since the source snapshot: \textsf{orchard} \texttt{b578ad9} is based on
0.15.0 and adds the NU6.3 cross-address restriction to the ZSA circuit, while
\textsf{librustzcash} \texttt{c5c232d} synchronises the NU6.2-era upstream
transaction changes. Neither change supplies a current normative carrier for
ZSA: upstream ZIPs~230 and~246 remain withdrawn, ZIPs~226 and~227 remain
Draft, and the NU7 deployment draft still does not select them. Detailed
wire, circuit, and fee claims below therefore continue to describe the
explicit pins in Table~\ref{tab:zsa-sources}; later fork behaviour is not
silently attributed to that public-testnet snapshot.
...
(\emph{Voting Guide}, \S``The source situation: code as the de facto
specification''): every claim is \emph{specified}, \emph{designed but
unspecified}, or an \emph{open problem}, and the bin stays visible in the
prose. The distribution differs sharply from
...
curve. Every Asset issued under ZIP~227 is scoped to the issuer that
issued it, and its Asset Identifier is globally unique, ``used across all
Zcash protocols that support ZSAs'' (\texttt{zip-0227.rst}:227--240).
...
\emph{Ironwood Guide}, \S``Delivery: the note reaches its owner''); an issue note has
no such Action, so ZIP-227 derives $\rho$ from the
...
The v6 digest tree extends the ZIP-244 structure the \emph{Ironwood
Guide} presents in \S``Conservation without disclosure: value commitments and
the binding signature'': the txid digest is BLAKE2b-256 with personalization
...
\emph{Ironwood Guide}'s Definition ``Orchard Action statement, Ironwood
circuit (NU6.3)'' exists in any specification. The implemented circuit is the
de facto specification, and
...
the baseline is the \emph{Ironwood Guide}, ``What the signatures sign: the
ZIP-244 digest and its v6 form'' --- with a per-Action-Group
...
in the \emph{Ironwood Guide}, \S``Conservation without disclosure: value
commitments and the binding signature'' --- from four top-level branches to
six:
\[
