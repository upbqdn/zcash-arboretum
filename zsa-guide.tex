...
\href{https://zips.z.cash/zip-0226}{ZIP~226, Transfer and Burn}, and
\href{https://zips.z.cash/zip-0227}{ZIP~227, Issuance}, both drafts.
The combined protocol, OrchardZSA, enables
...
(\emph{Ironwood Guide}, \S``Value commitments'') is replaced, per Asset, by
an \emph{Asset Base} witnessed inside the circuit; the same Asset Base must
appear in the old and the new note commitments and as the base of the
Action's value commitment. Under the
...
\S``Notes and note commitments''); value
commitments, the bases $V^{\mathsf{Orchard}}$ and $R^{\mathsf{Orchard}}$, and
the binding signature (\emph{Ironwood Guide}, \S``Value commitments and the
binding signature''); the Action statement and dummy spends (\emph{Ironwood
Guide}, \S``The Action statement'', \S``Actions, bundles, and dummy notes'');
nullifiers (\emph{Ironwood Guide}, \S``Nullifiers''); Sinsemilla
(\emph{Ironwood Guide}, \S``Sinsemilla: hash, commitment, and short forms'');
and, at the wallet layer, transaction construction, PCZT, and
...
ZSAs are not part of NU7. Its deployment draft,
\href{https://zips.z.cash/zip-0259}{ZIP~259}, introduces no transaction
format and retains versions~5 and~6 without a ZSA carrier. ZIPs~226
and~227 remain draft proposals for a future upgrade; their mechanisms are
not rules an NU7 validator enforces.

The essential distinction is between a cryptographic construction and its
\emph{carrier}: the transaction fields that encode it, together with the
digests signed by its participants. ZIP~226 assumes the recoverability
changes of \href{https://zips.z.cash/zip-2005}{ZIP~2005}, but its carrier
references the withdrawn ZIPs~230 and~246. The deployed version~6 format
is \href{https://zips.z.cash/zip-0229}{ZIP~229}, which does not carry ZSAs.
The extensible version~7 draft,
\href{https://zips.z.cash/zip-0248}{ZIP~248}, lists potential ZSA bundles
without specifying their encodings or digests. The experimental format
below is therefore not a standardised ZSA transaction format.

Two other assignments remain incomplete: ZIP~226 leaves the note-plaintext
lead byte unassigned, and the conventional fee rule in ZIP~317 defines no
ZSA issuance or asset-creation terms. The fork-specific choices below are
identified where used; neither is silently promoted to consensus.

\subsection{Sources}\label{sec:zsa-sources}

ZIPs~226 and~227 define the proposed constructions. The experimental
implementation is not identical to those texts; Table~\ref{tab:zsa-sources}
fixes which versions supply the fork-specific formulas below. Current
upgrade scope follows ZIP~259, not the experimental code's feature names.
...
The two library pins are separate snapshots, not a single deployed stack.
Checks of the implementation below establish library behaviour, not
node-side enforcement of ZIP~227's issuance state.
...
holds assignment to a future network upgrade and activation, the
...
in the \emph{Ironwood Guide}, ``Value commitments'', Definition ``Net value
commitment''; we cite it and do not re-derive it.
...
section brings them into existence. ZIP-227 specifies issuance on the Zcash
...
issuance forever, and a node-maintained global supply map.
...
\paragraph{Encoding boundary.} The issuance mechanism follows ZIP~227.
Its encodings and digests below follow the experimental ZIP~230/246
carrier, not ZIP~229's deployed v6 format (\S\ref{sec:zsa-status}).
Fork-only fee rules and differences between the ZIP and the pinned libraries
are marked where they affect a construction.
...
\emph{Ironwood Guide}, \S``Nullifier chaining''); an issue
...
presents in \S``Transaction digests and signatures'': the txid digest is
...
ZIP~317 defines neither contribution. These formulas are
\emph{fork-specified}: they describe the experimental ZSA fee policy,
not the conventional fee rule for deployed transactions.
...
ZIP~226 assumes ZIP~2005's recoverability changes, whereas the experimental
fork does not implement them. Where the difference affects a construction
--- the split-note $\psi^{\nfsym}$
...
\]
the fixed value-commitment base of the \emph{Ironwood Guide}, ``Value
commitments'' (ZIP-226). This single identification is what keeps every ZEC
computation below identical to NU5: ZIP-226 uses
$\mathsf{ValueCommit}^{\mathsf{Orchard}}_{\rcv}(v)$ as
...
baseline layout of the \emph{Ironwood Guide}, ``Note commitments'':
\[
...
``The nullifier''); only Split Inputs use the
...
Guide}, ``Value commitments'': the fixed-base multiplication by
...
\emph{Ironwood Guide}, ``The binding signature'', and we do not restate
them.
...
the \emph{Ironwood Guide}, ``The Action statement''). For Custom
...
statement''. The pinned implementation is therefore
...
The following encodings belong to the experimental ZIP~230/246 carrier,
not ZIP~229's deployed v6 format (\S\ref{sec:zsa-status}). The transfer
and burn constructions are draft-specified; their transaction encoding
remains \emph{designed but unspecified} for deployment.
...
the baseline is the \emph{Ironwood Guide}, ``Transaction digests and
signatures'' --- with a per-Action-Group
...
%% 3f9b53fc. Deployment status (Draft ZIPs, testnet, outside NU7 scope) is
...
Action statement,
$v_{\mathrm{old}} \cdot (\mathsf{root} - \mathsf{anchor}) = 0$, which
exempts a consumed note of value zero from membership so that an Action with
a dummy consumed note can create a real note (the \emph{Ironwood Guide},
``The Action statement''). The construction is protocol
\href{https://zips.z.cash/protocol/protocol.pdf\#orcharddummynotes}{\S4.8.3},
...
$V^{\mathsf{Orchard}}$ (\emph{Ironwood Guide}, ``Value commitments''): a
dummy spend can move no value, whatever path it fails to prove. ZIP-226
replaces the fixed base with a witnessed one, and the same exemption
becomes a mint. This section
...
net value commitment (``Value commitments''), unchanged. The native asset
keeps the old base: ``the Asset Base of the ZEC Asset will be kept as the
original value base
...
Action statement''). Two new boolean witnesses appear:
...
combinations of
\[
(\mathsf{is\_zatoshi\_asset},\mathsf{split\_flag})
\]
---
...
\end{equation}
where $F_{\nksym}$ and $G^{\mathsf{nf}}$ abbreviate the nullifier PRF
$\mathsf{PRF}^{\mathsf{nfOrchard}}_{\nksym}$ and the nullifier base
$K^{\mathsf{Orchard}}$ of condition A5 of the \emph{Ironwood Guide}'s
Definition ``Orchard Action statement'' (ZIP-226 writes
$\mathcal{K}^{\mathsf{Orchard}}$ for $G^{\mathsf{nf}}$ and computes the
bracketed sum in the Pallas base field). Two deltas against A5: the note's
$\psi_{\mathrm{old}}$ is replaced by a fresh $\psi^{\nfsym}$, and a fixed
offset point
\[
L^{\mathsf{Orchard}} :=
...
implementation stack at the pins in Table~\ref{tab:zsa-sources}, and the
obligations the format imposes on wallets. The format is experimental;
\S\ref{sec:zsa-status} states its deployment boundary.

\subsection{Experimental and deployed carriers}
\label{sec:zsa-format-status}

The carrier below is the experimental OrchardZSA format of ZIP~230, with
digests from ZIP~246. Both ZIPs are withdrawn. Its version number~6 must not
be confused with \href{https://zips.z.cash/zip-0229}{ZIP~229}'s deployed
version~6 format, which carries Ironwood and no ZSA bundles. The two byte
formats are incompatible despite the shared number.

We use the experimental format to explain how the note, issuance, burn,
and signature constructions compose into one transaction. Its fields and
digests are \emph{designed but unspecified} for deployment; the remaining
specification obligations are in \S\ref{sec:zsa-outlook}.

\subsection{The experimental wire format (ZIP-230)}\label{sec:zsa-v6}
...
\S\ref{sec:zsa-tx-rules}). This restriction concerns
\texttt{nAGExpiryHeight}, not the transaction-level expiry field
\texttt{nExpiryHeight}. Burn fields sit inside each Action Group
...
in the \emph{Ironwood Guide}, \S``Transaction digests and signatures'' ---
from four top-level branches to six:
\[
...
Guide}, \S``The binding signature''), ZIP-226 subtracts one zero-randomness
commitment per burn entry as well:
\begin{align*}
...
\subsection{The experimental fee policy}
\label{sec:zsa-fees}
...
ZIP~317 contains no ZSA fee contributions. The experimental fork adds a
charge for creating an asset, to discourage permanent state growth from
unused assets: the parameter
...
mutate the row. The implementation uses the same contributions. As with
the deployed rule (the \emph{Wallet
...
\paragraph{Serialization.} The experimental reader parses the header,
one
...
\item \emph{A carrier.} Transaction version 6 now belongs to NU6.3's
  ...
  remains incomplete: before a future ZSA deployment, the byte format of
  \S\ref{sec:zsa-v6} must be mapped or respecified for V7, and
  \S\ref{sec:zsa-digests} re-anchored to the resulting digest rules.
\item \emph{A base protocol.} ZIP~226 assumes ZIP~2005's recoverability
  changes, but the experimental implementation does not. No ZIP specifies
  whether OrchardZSA targets the restricted Orchard pool or is rebased onto
  Ironwood. Classification: \emph{open}.
