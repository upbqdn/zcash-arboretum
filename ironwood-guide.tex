...
modulus. Three maps relate integers, bit strings and byte strings:
\begin{itemize}
...
in BLAKE2b's native keyed mode (protocol specification, \S``Pseudo Random
Functions''), so the \emph{Crypto Guide}'s PRF argument for native keying
...
requirement that the protocol specification, \S``Pseudo Random
Functions'', places on $\mathsf{PRFexpand}$.
\end{assumption}
...
spend-authorisation signature scheme (protocol specification, \S``Spend
Authorization Signature (Sapling and Orchard)''). For $\ask \neq 0$:
\begin{enumerate}
...
\item the \emph{diversifier} is
  ...
  of NIST SP~800-38G''; protocol specification, \S``Pseudo Random
  Permutations'');
...
specification, \S``Pseudo Random Permutations'', whose note records that
...
\S``Shielded Pools and Notes'' and \S``Mainnet and Testnet''). Every value in
this volume, note values included, is an integer number of zatoshi.
\end{definition}
...
not allowed (\S``Decryption using an Incoming Viewing Key (Sapling and
Orchard)'' and \S``Decryption using an Outgoing Viewing Key (Sapling and
Orchard)''). For an Ironwood-pool output this is check~(i) of the acceptance
...
specification, \S``Pseudo Random Functions'').
\end{construction}
...
security requirement that the protocol specification, \S``Pseudo Random
Functions'', places on $\mathsf{PoseidonHash}$ keyed by its first argument.
...
the specification's security requirement assumes (\S``Computing $\rho$
values and Nullifiers''). Two notes with equal commitments are equal except
...
re-randomisation (protocol specification, \S``Spend Authorization
Signature (Sapling and Orchard)'' and \S``Binding Signature (Sapling and
Orchard)''):
\begin{enumerate}
...
(protocol specification, \S``Spend Authorization Signature (Sapling and
Orchard)'').
\end{construction}
...
$\{0, \dots, 2^{255} - 1\}$ (\S``Commitment''). As for $\rcm$
...
verifying key by upgrade in ZIP 258. The specification's \S``Action
Statement (Orchard)'' and the proof-validity rule of \S``Action
Descriptions'' list the first seven primary inputs and conditions A1 to A9
only; Definition~\ref{def:action} follows \S``Action Descriptions'' for the
rest. The specification names the conditions; the numbering A1 to A10 is this
...
\S``Orchard Key Derivation'' and \S``Symmetric Encryption'').
\end{construction}
...
specification, \S``Symmetric Encryption''). The scheme is not
...
(Sapling and Orchard)'', \S``Sending Notes (Orchard)'' and \S``Pseudo Random
Functions'').
\end{construction}
...
(Orchard)'', \S``Spend Authorization Signature (Sapling and Orchard)'' and
...
components of $T$; the remaining rules of \S``Transaction Consensus Rules''
