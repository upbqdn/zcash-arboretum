...
NU6.3; the \emph{Ironwood pool} is the current pool (activated on Testnet
at height $4134000$; Mainnet activation at height $3428143$, the parameter
shipped in \texttt{zebra}~6.2.x and falling due late July 2026 on the
75-second block schedule, while ZIP-258, Draft, still lists Mainnet TBD at
\texttt{zips} commit \texttt{69610984}).
...
empty at NU6.3 activation (Testnet height $4134000$, Mainnet height
$3428143$ per \texttt{zebra}~6.2.x; the ZIP-258 draft still lists Mainnet
TBD), whose anchors are valid from activation onward. In neither pool is there a
...
deferred fund --- negative, or total supply past
...
$\mathtt{0x37A5165B}$ and activates on Testnet at height $4134000$. On
Mainnet the deployed validator fixes the height: \texttt{zebra}~6.2.x ships
$3428143$ (\texttt{zebra-chain/src/parameters/\allowbreak constants.rs},
released 2026-07-24), falling due late July 2026 on the 75-second block
schedule, and \texttt{librustzcash} hardcodes the same value
(\texttt{zcash\_protocol/src/consensus.rs}); the ZIP-258 draft still lists
Mainnet TBD --- implementations ahead of their specification, which we flag
rather than resolve.
