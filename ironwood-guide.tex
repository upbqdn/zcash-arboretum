...
\node[fulln]  (fvk)  at ($(0,-5.7)-(\fvkakX,0)$) {$\fvk=(\aksym,\,\nksym,\,\rivk)$};
...
\node[spendn] (nk)   at (1.8,-1.9) {$\nksym$};
\node[spendn] (rivk) at (3.4,-1.9) {$\rivk$};

% ===== outgoing viewing key: paired with ivk as in/out viewing =====
\node[inoutn] (ovk)  at (-2.6,-7.8) {$\ovk$};

% ===== diversifier key + address row (the FF1-AES256 formula is in the gutter) =====
\node[recvn]  (dk)   at (-2.6,-9.4) {$\dk$};
\node[inputn] (dpln) at (-4.5,-10.6) {$d_{\mathrm{plain}}$};
...
\draw[fork] ([yshift=1.6mm]fvk.west) .. controls (-4.4,-6.0) and (-4.6,-8.9) .. (dk.west);
...
\node[op] at (\gx,-10.6) {$d := \mathsf{FF1\text{-}AES256}_{\dk}(d_{\mathrm{plain}})$;\\[1pt]
