...
$\mathtt{0x37A5165B}$. Its activation heights are fixed by the deployed
validator (\texttt{zebra-chain/src/parameters/\allowbreak constants.rs},
with \texttt{librustzcash} hardcoding the same values in
\texttt{zcash\_protocol/src/consensus.rs}), while the ZIP-258 draft still
lists Mainnet TBD --- implementations ahead of their specification, which
we flag rather than resolve.
%% TODO: MIN_NETWORK_PROTOCOL_VERSION is still TBD on both networks
%% (zip-0258.md:72-74); revisit when ZIP-258 leaves Draft.
...
Ironwood Migration'' --- is an eleven-line Reserved stub (owners Schell Carl
