...
This is the \emph{Ironwood Guide}, the volume of \emph{The Zcash Arboretum}
that develops the cryptography behind Zcash's Orchard protocol as deployed.
It assumes three earlier volumes: the \emph{Math Guide}
...
NU6.3; the \emph{Ironwood pool} is the current pool.
...
is trivial: the output is present in the set of unspent transaction
outputs (the UTXO set) that every node maintains. But Alice must prove
...
layer $\ell'$ is
\begin{multline*}
\mathsf{MerkleCRH}^{\mathsf{Orchard}}_{\ell'}(\ell, r) :=\\
\mathsf{ShortHash}_{\mathtt{z.cash:Orchard\text{-}MerkleCRH}}(\mathrm{LE}_{10}(\ell') \,\|\, \mathrm{LE}_{255}(\ell) \,\|\, \mathrm{LE}_{255}(r)) \in \F_{p_{\Pallas}},
\end{multline*}
...
Orchard activation onward (NU5) --- for the
...
empty at NU6.3 activation, whose anchors are valid from activation
onward. In neither pool is there a
...
deeper anchors make the event rarer, but no depth rules it out --- the
often-cited $100$-block figure is the specification's description (protocol
\S\,3.3; the retired \textsf{zcashd} actually enforced $99$), and the
deployed \textsf{zebra} rolls back automatically to $1000$ blocks, node
policy in both validators rather than consensus
(\emph{Consensus Guide}, \S``Reorg rails, maturity, and the finality
floor'').
Deployed wallets nonetheless anchor close to the tip, by
...
\subsection{How value enters the pool}\label{sec:pool-entry}

The walkthrough moved value already inside the pool. Value \emph{enters}
through three doors, each public in amount: the two below, and migration
from another shielded pool (\S\ref{sec:iw-turnstile}).

\paragraph{Shielding transactions.} The instrument is the sign of the
balancing value: by the convention of \S\ref{sec:value}, a positive
$v_{\mathrm{balance}}^{\mathsf{Orchard}}$ is net value leaving the pool,
so a \emph{shielding} transaction is one whose transparent inputs fund a
negative $v_{\mathrm{balance}}^{\mathsf{Orchard}}$: its Actions spend
dummy notes ($v_{\mathrm{old}} = 0$, the anchor check A3 gated off,
\S\ref{sec:circuit}) and create real ones, and the binding signature of
\S\ref{sec:value} proves the books balance. An observer learns the
entering amount --- the balancing field is a cleartext transaction field
--- but not the receiving note or address. The deployed construction
path is function \texttt{propose\_shielding}
...
UTXOs, at zero confirmations under the default policy, and wallets
auto-shield transparent receipts to internal keys.

\paragraph{Shielded coinbase (ZIP-213).} Since Heartwood, new issuance
may enter directly: ``[c]oinbase transactions MAY contain Sapling
outputs'', extended at NU5 to the Orchard pool and re-scoped at NU6.3
--- from then coinbase may carry ``Sapling-pool and/or Ironwood-pool
outputs'', and ``it is only possible to mine to an Orchard address by
sending the funds to the \emph{Ironwood pool}'' (ZIP-213). Two riders
keep this auditable and consistent. Coinbase maturity is amended to
bind transparent outputs only (the rule the \emph{Consensus Guide}
reads through its theorem, \S``Reorg rails, maturity, and the finality
floor''), and every shielded coinbase output ``MUST have valid note
commitments'' when recovered under the \emph{all-zero} outgoing viewing
key: coinbase shielding is publicly decryptable by construction, so
anyone can verify what issuance entered which pool, while the notes
spend exactly like any others afterwards.

\paragraph{The ledger of crossings.} Every entry and exit is metered by
ZIP-209's pool balances: for each shielded pool, the \emph{chain value
pool balance} is the negation of the sum of that pool's balancing
fields over the whole chain (for Sprout, the sum of
$\mathtt{vpub\_old} - \mathtt{vpub\_new}$), and a block is invalid if
it would drive any pool balance --- shielded, transparent, or the
deferred fund (ZIP-2001's lockbox, a chain value pool accruing a portion
of each block subsidy for later disbursement) --- negative, or total
supply past
$\mathsf{MAX\_MONEY}$. Entry, transfer, and exit therefore reconcile
publicly at pool granularity while individual notes stay hidden; the
Ironwood instantiation, and the one-way turnstile that closes the
Orchard pool's entry door entirely, are \S\ref{sec:iw-turnstile}'s.
...
released crate \texttt{orchard}~0.15.0 \texttt{src/circuit.rs}). The legacy NU5/NU6
...
circuit: arithmetisation in practice''; here we describe what each gadget
enforces.
...
tables; the $j$-invariant-$0$ endomorphism serves only the recursion
layer, where it cheapens multiplication by verifier challenges, not this
circuit) and
...
inputs alone, and batch verification --- the deferred linear MSMs merged
across proofs (the \emph{Halo~2 Guide}) --- lets a node verify many such
proofs at amortised cost approaching that of a single verification.
...
constant-size proofs ($296$ bytes) and constant verification time. Its
...
\texttt{PostNu6\_3} in the released crate \texttt{orchard}~0.15.0
(\texttt{src/circuit.rs}; the \texttt{bef8a27} dev tree names the third
variant \texttt{Ironwood}). And from NU6.3 the one protocol carries two value
...
Proof size & 296 B & 192 B & \textasciitilde 5 kB (one Action) \\
...
foundation for recursion: future generations (Tachyon, eventual designs
that aggregate many transactions under a single succinct proof) can
build on Orchard's foundation without revisiting the cryptographic
stack.
...
implementation and the specification --- here the released crate
\texttt{orchard}~0.15.0 (the version \texttt{librustzcash} pins; the
Ironwood note-format, note-encryption, and v6-digest work ships in it), the
\texttt{orchard} git tree at \texttt{bef8a27} (a 0.14.0 dev tree predating
the release, cited for the circuit and cross-address work it carries),
...
$\mathtt{0x37A5165B}$. ZIP-258 now fixes activation at height
$4{,}134{,}000$ on Testnet and $3{,}428{,}143$ on Mainnet, agreeing with
\texttt{zebra-chain/\allowbreak src/\allowbreak parameters/\allowbreak
constants.rs} and \texttt{zcash\_protocol/\allowbreak src/\allowbreak
consensus.rs}. It also fixes the minimum network
protocol version at $170160$ on both networks (ZIP-258 at \textsf{zips}
\texttt{ad30e59}; ZIP-204 records that NU6.3 is the last upgrade for which
the two networks may share a protocol version).
...
\texttt{zcash\_protocol/\allowbreak src/\allowbreak constants.rs}) is
the v5 format --- with $\mathsf{enableSpendsOrchard}$/\allowbreak
...
\texttt{sizeProofsIronwood} & \texttt{compactSize} & length of
\texttt{proofsIronwood} \\
...
component
hashes the empty string under its personalisation (ZIP-229). This digest
tree is deployed: the released crate \texttt{orchard}~0.15.0 (the
\texttt{librustzcash} pin) defines the seven Orchard and Ironwood
\texttt{\_v6} personalisations and selects them per pool and transaction
version (\texttt{bundle/commitments.rs}, enum
...
\texttt{Bundle::commitment}, the file itself defines only the two Sapling
personalisations \texttt{ZTxIdSSpendNH\_v6} and \texttt{ZTxAuthSapliH\_v6}
(lines $44$ and $54$), and \texttt{transaction/tests.rs} re-derives the
empty-component digests as test vectors. The \texttt{bef8a27} dev tree
predates this work: there \texttt{bundle/commitments.rs} (\texttt{:7--11})
defines only the five non-versioned personalisations
(\texttt{ZTxIdOrchardHash}, \texttt{ZTxIdOrcActCHash},
\texttt{ZTxIdOrcActMHash}, \texttt{ZTxIdOrcActNHash},
\texttt{ZTxAuthOrchaHash}), with no per-pool or per-version dispatch. One
structural
change
...
Action circuit and its keys. In the released \texttt{orchard}~0.15.0 a
\texttt{BundleVersion} pairs a \texttt{ValuePool} (\texttt{Orchard} or
\texttt{Ironwood}) with a \texttt{ProtocolVersion} (\texttt{InsecureV1},
\texttt{V2}, or \texttt{V3}): \texttt{circuit\_version()} maps \texttt{V3}
to \texttt{OrchardCircuitVersion::PostNu6\_3} for both pools,
\texttt{note\_version()} maps \texttt{Orchard} to \texttt{V2} and
\texttt{Ironwood} to \texttt{V3}, and
\texttt{permits\_cross\_address\_transfers()} is false exactly for
(\texttt{V3}, \texttt{Orchard}) (\texttt{src/lib.rs},
...
(\texttt{src/circuit.rs}); the \texttt{bef8a27} dev tree's
\texttt{BundleFormat} enum and \texttt{Ironwood} circuit-version name were
reworked into these before release. The pools are
...
activation bound to NU6.3). The format is specified --- ZIP-2005
\S\,4.7.3, landed in \texttt{protocol.tex} at \texttt{zips} commit
\texttt{69610984} --- and deployed in the released \texttt{orchard}~0.15.0
(the \texttt{librustzcash} pin): enum \texttt{NoteVersion}
(\texttt{V2}/\texttt{V3}) carries the lead byte,
\texttt{Note::from\_parts} takes it as a fifth argument, and
\texttt{Note::rcm()} dispatches on it (\texttt{src/note.rs}). The
\texttt{bef8a27} dev tree predates the release: its
\texttt{Note::from\_parts} (\texttt{src/note.rs:182}) still takes the four
components $(\mathsf{recipient}, v, \rho, \mathsf{rseed})$ with no version
tag, and its \texttt{Note::rcm()} is the legacy single-tag trapdoor of
Remark~\ref{rem:iw-legacy-rcm}.
...
$\mathsf{IronwoodPool}$ --- and let $h_{\mathsf{Canopy}}$ be the
...
The dev-tree \texttt{Note::rcm()} at \texttt{bef8a27} implements only the
legacy branch above (\texttt{src/note.rs:132--136}), with
\texttt{PrfExpand::\allowbreak ORCHARD\_RCM} $= \mathtt{0x05}$
(\texttt{zcash\_spec}~0.2.1,
\texttt{src/prf\_expand.rs:88}), no version dispatch, and no
$\mathtt{0x03}$/hashed-$\mathsf{pre\_rcm}$ branch. The released
...
$\mathsf{pre\_rcm}$ (\texttt{src/note.rs}) --- deploying the derivation of
Definition~\ref{def:iw-derive}.
\end{remark}
...
$\rcm$ (ZIP-2005 \S\S\,4.20.2--4.20.3 changes). The routing is deployed:
the released \texttt{orchard}~0.15.0 routes the two lead bytes to two
domains, \texttt{OrchardDomain} ($\mathtt{0x02}$) and
\texttt{IronwoodDomain} ($\mathtt{0x03}$)
(\texttt{src/note\_encryption.rs}), which \texttt{zcash\_client\_backend}
scanning consumes (\texttt{src/scanning.rs}); the \texttt{bef8a27} dev tree
still has a single $\mathtt{0x02}$-only \texttt{OrchardDomain}. Because
keys are protocol-scoped
...
and link the Action's nullifier to the address (ZIP-326). In a PCZT (a
\emph{partially created Zcash transaction}, the format that carries an
in-progress transaction between its constructor, prover, and signers)
the fabricated output carries its recipient, value, and $\mathsf{rseed}$
...
analogue of every pool-balance rule of \S\ref{sec:pool-entry} (the
ZIP-209 extension):
\[
...
treestate (\S\ref{sec:tree}), the tree capacity is $2^{32}$, and the
ZIP-221 history tree (the Merkle mountain range over the chain's history
whose root every block header commits to; \emph{FlyClient Guide},
\S``NU5: the commitment becomes one of two'') gains three Ironwood
fields (earliest root, latest root,
...
pool, the only pool whose output notes are recoverable, and SHOULD treat the
...
\paragraph{ZIP-318's construction.} At \textsf{zips} commit
\texttt{ad30e59} (2026-08-25), ZIP-318 is a Draft specification rather than
the former Reserved stub. Its privacy-preserving migration has two phases.
First, the wallet decomposes the migratable balance into canonical crossing
denominations $n \cdot 10^k$ ZEC for $n \in \{1,2,5\}$, from $0.01$ through
$10000$ ZEC, and creates exact-value funding notes by shielded send-to-self
preparation transactions. Each funding note contains one denomination plus
its canonical fee; less than $0.01$ ZEC remains as a residual. Preparation
transactions should contain exactly 16 Orchard Actions, use ordinary
near-tip anchors, and be separated by independently drawn delays with mean
16 blocks and cap 96 blocks. Layering supplies fan-in or fan-out when one
transaction cannot prepare all notes.

Second, each scheduled crossing should spend exactly one funding note in an
Orchard bundle padded to two Actions and create exactly one unpadded Ironwood
Action paying one canonical denomination. It has no transparent component
or other shielded pool, pays the canonical fee, and sets lock time to zero.
The wallet shuffles the parts and draws independent exponential broadcast
delays with mean 66 blocks and cap 576 blocks. Each proof uses an Orchard
anchor at a shared boundary height divisible by 144; its age in boundaries
is drawn from a geometric distribution with parameter $1/2$ and truncated
at four. A part scheduled at height $h$ uses the common expiry height
\[
  69120 + 34560\left\lfloor\frac{h}{34560}\right\rfloor,
\]
giving between one and two months of validity at the 75-second target
spacing. Because version~6 signatures do not bind the anchor, the wallet may
pre-sign the schedule and later update each anchor and proof without
re-signing (ZIP-318, Specification).

The procedure mitigates, but cannot remove, the public turnstile. It requires
the user's consent to the visible crossing values, separates synchronisation
sessions from broadcast sessions, and offers a network-privacy path such as
Tor, Nym, or a VPN. Its local schedule is deliberately stateful: restoring
only the seed need not recover an in-progress migration (ZIP-318,
Requirements, Non-requirements, and Privacy Implications).

\paragraph{Implementation boundary.} The released
\textsf{zcash\_pool\_\allowbreak migration}~0.1.0 crate and
\textsf{zcash\_client\_\allowbreak sqlite}~0.22.0 implement the backend engine
and persistent store described above: denomination and preparation planning,
PCZT construction and signing, block-height scheduling, proof refresh, and state
transitions (\textsf{librustzcash} \texttt{91f448b},
\texttt{zcash\_pool\_\allowbreak migration/} and
\texttt{zcash\_client\_\allowbreak sqlite/\allowbreak src/\allowbreak
pool\_migration/}). The consuming-wallet
boundary remains unfinished at the cited heads. Zallet \texttt{f904040}
pins the engine but its source tree exposes no migration command or RPC, and
ZIP-318's own ``Reference implementation'' section still contains only a
TODO. The backend is therefore released; the specified mobile scheduling,
network path, user consent, and application-open fallback are not a deployed
...
generic quantum speedups (\S\ref{sec:pq-grover}). This section is the
Orchard core seen from inside; the same reversibility axis applied across
every layer of Zcash --- the transparent stack, proof of work, the frontier
protocols, and the migration arithmetic --- is the \emph{PQ Guide}'s subject.
...
\texttt{bef8a27e} (v0.14.0, 2026-06-16) for circuit claims; and the
released crate \texttt{orchard}~0.15.0 (the \texttt{librustzcash} pin) for
the $\mathtt{0x03}$ note path; we
...
ciphertext \dots\ and the receiver address''.
...
Orchard output recorded since NU5 activation is already in that
...
(ZIP-258, Draft: consensus branch $\mathtt{0x37A5165B}$); the scheduling
follows the NU6.2 emergency circuit
...
Recovery Statement carries the $\mathsf{qk}$ and $\mathsf{sk}$ alternatives
as a tagged union, and the key-binding condition pins that tag as well as the
key value: a recovery-authorised $\mathsf{qk}$ witness cannot collide with an
$\mathsf{sk}$-branch witness for the same $\ivk$. The
