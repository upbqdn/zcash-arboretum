...
The protocol target is draft NU7, whose
\href{https://zips.z.cash/zip-0259}{ZIP~259} leaves both activation heights
unassigned as of 23~September 2026. Proposed rules and implementation-specific
behaviour are distinguished where they affect the construction.
\end{abstract}
\clearpage
\tableofcontents
\clearpage
...
ZIP-32 (\S``Keys and addresses'' there). This section supplies
that origin. A
...
runs \texttt{ExtendedSpendingKey}\linebreak\texttt{::}\allowbreak%
\texttt{from\_path} over the three hardened
...
\emph{Ironwood Guide} constructs in ``Keys and addresses'', and
we do not
...
(\emph{Ironwood Guide}, ``Trial decryption and note acceptance''), which is
...
``Nullifier chaining''). Rows $\mathtt{0x04}$ and
$\mathtt{0x06}$--$\mathtt{0x09}$ are constructed in the \emph{Ironwood
Guide}; rows $\mathtt{0x04}$/$\mathtt{0x05}$ carry their Orchard meanings,
$\mathtt{0x05}$ that of the legacy trapdoor of Orchard-pool notes.
...
\emph{unified} container encodings. The \emph{Ironwood Guide} (``Keys and
addresses'') already constructs the
...
\emph{diversifier} $d$ per address. The \emph{Ironwood Guide} (``Diversified
addresses'') constructs the map and its
...
The totality of the Orchard hash, which removes Sapling's
try-the-next-index rejection loop, is established in the \emph{Ironwood
Guide} (``Diversified addresses''). The residual
...
\texttt{kind/\allowbreak unified/\allowbreak address/\allowbreak
test\_\allowbreak vectors.rs}). Every value below is
script-computed; the key half runs the deployed
\texttt{SpendingKey::\allowbreak from\_\allowbreak zip32\_\allowbreak seed} and
...
Guide}, ``The outgoing ciphertext''): the Sender determines a
...
\begin{remark}[NU7 does not redefine the conventional fee]
\label{rem:fees-nu7}
The Draft \href{https://zips.z.cash/zip-0259}{ZIP~259} does not replace
the \href{https://zips.z.cash/zip-0317}{ZIP~317} fee formula or its
$5000$-zatoshi marginal fee.

Nor is a logical action the unit used for NU7's proposed block work limit.
For example, the fee counts the larger of Sapling's spend and output
counts, whereas the proposed block limit counts their sum
(\href{https://zips.z.cash/zip-0218}{ZIP~218}). Paying the conventional
fee therefore does not guarantee that a transaction fits in a particular
block. The proposed block limits and allocation of fees between miners and
the sustainability mechanism are separate matters, constructed in the
\emph{Consensus Guide}, \S``NU7: 25-second blocks and bounded shielded
work''; they do not add a field to an ordinary v5 or v6 payment.
\end{remark}
...
spends one note \emph{and} creates one (the \emph{Ironwood Guide}, ``Actions,
bundles, and dummy notes''), and Orchard bundles are padded to a
...
\texttt{bundle\_\allowbreak required} set) uses
$\mathit{num\_outputs} = \max(\mathit{outputs}, 2)$, the constant $2$
being \texttt{MIN\_\allowbreak SHIELDED\_\allowbreak OUTPUTS}.
Spends are never padded under
...
\texttt{MultiOutput\allowbreak ChangeStrategy}, splitting change across
several notes
...
of synchronisation, which accepts notes by the trial decryption of the
\emph{Ironwood Guide} (``Trial decryption and note acceptance''); this section
takes that spendable set as given. Section~\ref{sec:sync-scanning} develops
the scanning pipeline that reconstructs it.
...
Under the proposed NU7 rules, both selection phases would also have to
respect the shielded block-work budget of
\href{https://zips.z.cash/zip-0218}{ZIP~218}, including Ironwood if the
open correction in \href{https://github.com/zcash/zips/pull/1361}{pull
request~1361} is adopted. That validity constraint is independent of the
unpaid-logical-action policy just described.
...
Synchronisation (\S\ref{sec:sync-scanning}), which accepts notes by the trial
decryption of the \emph{Ironwood Guide} (\S~``Trial decryption and note
acceptance''), leaves a wallet holding a set of unspent notes, each with a
value, a position, and an authentication path available on demand. This
section documents how the
...
witness derivation are those of the \emph{Ironwood Guide} (\S~``The note
commitment tree'', in particular ``Anchors'', which already states the ZIP-315
anchor depth of $3$); here we record only what the pipeline feeds in.
...
witness (the \emph{Ironwood Guide}, \S~``Authentication paths'') --- is
unknown before the note is mined.
...
are those of the
\emph{Ironwood Guide} (\S~``The note commitment tree'') and are
not repeated here.
...
For NU7, \href{https://zips.z.cash/zip-0218}{ZIP~218} recommends an expiry
delta of $120$ blocks (\S\ref{sec:lifecycle-expiry}). The builder must use
the target height's consensus branch; NU7 retains v5 and v6, with v6 needed
for an Ironwood component. The lock time of built transactions is $0$.
...
\S~``Viewing keys''). Ironwood spends use
...
\S~``Trial decryption and note acceptance''). This matches the
...
of the \emph{Ironwood Guide} (\S~``Value commitments'' for $\cvsym$/$\rcv$;
\S~``Randomised validating keys'' for $\aksym$/$\alpha$/$\rk$; \S~``The note
seed: Ironwood derivations (lead byte $\mathtt{0x03}$)'' for the
$\mathsf{rseed}$ derivations).
...
(\S~``Transactions and consensus rules'', subsection ``Transaction digests
and signatures'')
...
The scheduling rules and their privacy analysis are those of ZIP-318
(\emph{Specification}; \emph{Privacy Implications}).
...
\paragraph{A minimal PCZT, byte by byte.} A v6 PCZT with no spends or
outputs makes the positional encoding concrete. Use NU7's proposed branch
identifier \texttt{0x77190AD9}, illustrative expiry height $10{,}000{,}000$,
and Mainnet coin type $133$. Serialising the fields under
\href{https://zips.z.cash/zip-0374}{ZIP~374}'s version-$2$ rules yields
the following $32$ bytes. This is an encoding example, not a payment or an
assignment of an activation height.
...
$8$  & \texttt{tx\_version} $= 6$ (varint)      & \texttt{06} \\
$9$  & \texttt{version\_group\_id} (varint)     & \texttt{98 ed 92 c4 0d} \\
$14$ & \texttt{consensus\_branch\_id} (varint)  & \texttt{d9 95 e4 b8 07} \\
...
The convenience \texttt{Pczt::serialize} chooses the minimum encoding
version that can represent the logical PCZT. A v6 transaction requires PCZT
v2 even when its bundles are empty; PCZT v1 represents only v5 transactions.
...
sapling\_anchor, orchard\_anchor)}, selects a transaction format compatible
with the target branch. NU7 permits v5
($\texttt{version\_group\_id}=\mathtt{0x26A7270A}$) and v6
($\texttt{version\_group\_id}=\mathtt{0xD884B698}$); an Ironwood component
requires v6. A Creator and Signer must recognise the NU7 branch identifier;
retaining v6 encoding is not permission to retain an old branch commitment
(\href{https://zips.z.cash/zip-0259}{ZIP~259}).
It starts with
...
\begin{gathered}
\rsk := \ask + \alpha
\quad\text{(the RedPallas SpendAuth randomization)},
\\
...
\texttt{sign\_ironwood\_with}/\allowbreak
\texttt{sign\_orchard\_with}/\allowbreak
\texttt{sign\_sapling\_with}/\allowbreak
...
its \texttt{zkproof}), \texttt{requires\_\allowbreak orchard\_proof}, and
\texttt{requires\_\allowbreak ironwood\_proof} (actions non-empty and the
corresponding
bundle \texttt{zkproof} \texttt{None}), letting a
wallet skip downloading the Sapling parameters when they are unneeded;
\texttt{create\_\allowbreak orchard\_proof(pk)} parses the bundle and calls the
\textsf{orchard} crate's \texttt{Bundle::\allowbreak create\_proof};
\texttt{create\_\allowbreak ironwood\_proof(pk)} does the same for Ironwood.
...
\emph{Ironwood Guide} assembles this tree in full (``Transaction digests and
signatures''); we do not re-derive it.
...
\paragraph{The NU7 expiry policy.}
\href{https://zips.z.cash/zip-0218}{ZIP~218}, ``Default expiry delta'',
recommends a default of $120$ blocks. Let $\delta$ denote the wallet's
chosen expiry delta, normally $120$ under that recommendation. With the
light-wallet convention of applying the delta to the next-block target
height, construction sets
\[
\mathsf{nExpiryHeight} :=
\begin{cases}
  \mathrm{targetHeight} & \text{coinbase},\\
  \mathrm{targetHeight} + \delta &
    \text{otherwise},
\end{cases}
\]
and the default target-time duration is
\[
120 \cdot 25 = 3000\ \text{seconds} = 50\ \text{minutes}.
\]
This is an estimate, not a guaranteed validity duration: actual block
intervals vary. The recommendation is wallet policy, not a new consensus
expiry predicate. ZIP~218 preserves explicitly configured overrides.
Wallets should otherwise use a common delta, because idiosyncratic expiry
heights fingerprint transactions just as idiosyncratic fees do
(\href{https://zips.z.cash/zip-0315}{ZIP~315}). Expiry estimation,
pending-input locking, rebroadcast, and the unknown-expiry fallback must
agree with the delta actually used in construction. The coordinated
migration schedule is separate (\S\ref{sec:pipeline}).

\begin{remark}[The policy and its implementation]
\label{rem:lifecycle-builder-doc}
The pinned wallet implementation predates NU7 and does not implement the
recommended delta. The formulas here expose $\delta$ explicitly rather
than claiming that its defaults have already changed. An implementation
must select the applicable policy for the transaction's target height.
\end{remark}
...
Relay policy may reject a transaction that consensus would still permit in
the next block; eligibility for inclusion is not a promise of propagation.
Wallets should submit before the final eligible block rather than rely on
last-block acceptance.

\paragraph{Expiry at a network upgrade.} The expiry height does not
determine the consensus branch for which a transaction is valid.
This matters at NU7 even though v5 and v6 retain their layouts.
The Draft \href{https://zips.z.cash/zip-0259}{ZIP~259} proposes branch
identifier $\mathtt{0x77190AD9}$: signatures for the old branch do not
authorise the new one. Once the activation height is assigned, a wallet
must either complete a pre-upgrade payment before it or reconstruct and
authorise the payment for the new branch. An expiry height extending past
activation does not extend the old signature's validity. Under
\href{https://zips.z.cash/zip-2003}{ZIP~2003}, v4 transactions would be
invalid from that height even if their expiry were zero. Sprout funds,
spendable only through v4, would then be inaccessible; this does not disable
Sapling, transparent, Orchard, or Ironwood components supported by the
remaining formats.
...
compact-block trial decryption (\emph{Ironwood Guide}, ``Trial decryption and
note acceptance''), so scanning alone can never confirm their fate.
...
\end{array}
\]
where $\mathrm{certaintyDepth} := \mathtt{PRUNING\_DEPTH} + \delta$
blocks, using the expiry delta defined in \S\ref{sec:lifecycle-expiry}.
This parameterised form separates the policy from the pinned
implementation's old default. The result is unioned with explicit
\texttt{tx\_retrieval\_queue} entries. A
...
$\mathrm{certaintyDepth}$ blocks past first observation; and one with
expiry $0$ only until
...
height $H$ of the new proposal, with the expiry-policy parameter $\delta$
of \S\ref{sec:lifecycle-expiry}:
...
\;\vee\; \bigl(\mathtt{expiry\_height} \text{ IS NULL} \wedge
  \mathtt{min\_observed\_height} + \delta \ge H\bigr)
...
$\mathtt{min\_observed\_height} + \delta + 1$ onward while it is still
...
contributes its nullifier, so that scanning (\S\ref{sec:sync-scan}) detects
the spend by that nullifier (\emph{Ironwood Guide}, ``Nullifier sets'') if the
...
recommendations for the $3/10$ confirmation split, zero-conf shielding,
and fixed anchor depth. ZIP-315 supplies their rationale, not a finalised
standard. NU7's expiry recommendation is described in
\S\ref{sec:lifecycle-expiry}.
\end{remark}
...
\paragraph{Block counts are not elapsed time.} At NU7's proposed
$25$-second target spacing, a ten-block interval corresponds to
$10\cdot25=250$ seconds. Counts that
include the mining block must still be distinguished from intervals
\emph{after} that block. ZIP~218 explicitly recommends leaving the
three-block anchor depth unchanged; it does not prescribe multiplying the
wallet's trusted or untrusted confirmation counts by three. Neither an
equal block count nor an equal elapsed time alone establishes equal
double-spend risk: the attack model and confirmation policy belong to the
\emph{Consensus Guide}, \S``NU7: 25-second blocks and bounded shielded
work''. The defaults below describe the pinned wallet, not a newly
standardised NU7 policy.
...
is fully scanned (witness data exists --- the \emph{Ironwood Guide},
``Authentication paths''); its mined
...
\texttt{ScanPriority::\allowbreak Verify} whenever that height is at or below
the
...
At NU7's proposed $25$-second target spacing, that $10$-block lookahead
covers $250$ target seconds. It is a scan-scheduling heuristic, not a
consensus rule or a guarantee that no recent wallet transaction is missed.
...
\item Group parameters by payment index, rejecting any duplicate --- the
  ...
  (\texttt{Zip321Error::\allowbreak DuplicateParameter}).
...
\item a Unified Address with a supported Orchard receiver maps to Orchard
  ...
  fails (\texttt{GreedyInputSelectorError::\allowbreak UnsupportedAddress}).
\end{itemize}
...
\texttt{recipient\_\allowbreak address()}\allowbreak%
\texttt{.can\_receive\_as(pool)};
...
shielded output on the chain (\emph{Ironwood Guide}, \S``Trial decryption and
note acceptance'').
...
Orchard, \texttt{Chain\allowbreak Metadata}, or transparent compact data
...
\subsection{NU7: block frequency and scanning work}
\label{sec:sync-nu7-work}

At the $25$-second target spacing of
\href{https://zips.z.cash/zip-0218}{ZIP~218}, a target day contains
$86{,}400/25=3456$ blocks. Its shielded-work limit would bound scanning
work per block. The calculation below uses the proposed shared
$330$-unit budget including both Orchard and Ironwood, whose inclusion
still awaits the open
\href{https://github.com/zcash/zips/pull/1361}{pull request~1361}.

Each Orchard or Ironwood Action contributes one $148$-byte compact field
payload (Table~\ref{tab:sync-compact-sizes}); each Sapling spend or output
uses one unit of the proposed budget and carries a smaller payload.
Consequently, filling every unit with Orchard-protocol Actions gives
\begin{align*}
\text{Actions per target day} &= 330\cdot3456 = 1{,}140{,}480,\\
\text{field bytes per target day} &= 148\cdot1{,}140{,}480
  = 168{,}791{,}040.
\end{align*}
This is about $168.79$ decimal MB of \emph{shielded field payload}, not a
bound on total network traffic. Serialising those Actions as repeated
message elements already costs $159$ bytes each, or $181{,}336{,}320$
bytes over the same target day, before transaction and block metadata,
transparent data, transport overhead, and full-transaction fetches.
Actual daily totals also vary with the number of blocks mined: target
spacing is not a deterministic rate limit.

Trial decryption remains per output and per candidate incoming viewing
key. The Action-only case therefore calls for up to $1{,}140{,}480$
attempts per key over that target day. It is not a universal two-key
budget per account: \href{https://zips.z.cash/zip-0326}{ZIP~326} allows
restoration to require more candidate keys before the account's received
funds establish which derivation is in use. Faster blocks require updated
throughput and retention estimates, not a new decryption algorithm or a
new compact-note format.
...
\texttt{TransactionData\allowbreak Request::}\allowbreak%
\texttt{Enhancement(txid)} ---
...
trial decryption and its acceptance checks in \S``Trial decryption and note
acceptance''; we do not repeat the construction, and record here only the
compact variant's deployed mechanics and its integrity argument.
...
deployed strengthening beyond the tag: it rejects an ephemeral key not formed
on the plaintext's diversified base (\emph{Ironwood Guide}, Remark ``Role of
check (iii)'').
...
field up front: each
\texttt{Compact\allowbreak Sapling}\allowbreak%
\texttt{Spend.}\allowbreak\texttt{nf}
and each
...
shardtree that consumes these marks, a sparse store of the tree of the
\emph{Ironwood Guide} (\S``The note commitment tree''), is developed in
\S\ref{sec:sync-tree-store}.
...
\mathsf{MixingPedersenHash}(\cmsym,\allowbreak \mathsf{NotePosition})$;
Orchard's does
...
(RPC added in \textsf{lightwallet-protocol} v0.3.5), whose mechanics
\S\ref{sec:sync-tree} develops from the authentication paths of the
\emph{Ironwood Guide} (\S``Authentication paths'').
...
A light wallet holds keys and no chain. What synchronisation consumes
follows from the \emph{Ironwood Guide}: every compact shielded output on the
chain, for trial decryption (\S``Trial decryption and note acceptance'');
every nullifier, for spentness (\S``Nullifier sets''); the note commitment
tree's frontier and subtree roots, for witness assembly
(\S``Authentication paths''); and, for the few transactions that prove to be
the wallet's own, the full serialised bytes. The deployed layer that delivers
...
\S``Trial decryption and note acceptance''.
\end{definition}
...
replays the chain's prefix (\emph{Ironwood Guide}, \S``Authentication
paths''). Zaino resolves
...
Guide}, \S``Authentication paths''). Zaino converts
...
(\textsf{lightwalletd},
\texttt{cmd/\allowbreak root.go:\allowbreak
139--180,\allowbreak 375--424,\allowbreak 498--499}).
...
--- key agreement, KDF, the commitment recomputation --- is the
\emph{Ironwood Guide}'s material (\S``Note encryption''); this volume treats
it as a black box mapping a
(compact
...
sizes \texttt{saplingCommitmentTree\allowbreak Size=1},
\texttt{orchardCommitmentTree\allowbreak Size=2}, and
...
$\mathsf{VERIFY\_LOOKAHEAD} = 10$ blocks covers $250$ target seconds at
NU7's proposed spacing (\S\ref{sec:lifecycle-spend}). This is a pinned
implementation heuristic, not a specified NU7 lookahead. In all cases,
...
present section: shard-tree structure follows in \S\ref{sec:sync-tree}, and
witness derivation and anchor selection are the \emph{Ironwood Guide}'s
material (\S``The note commitment tree'').
...
\paragraph{The recovery window.} A wallet's chosen pruning depth bounds
the recent checkpoint and nullifier history available for a direct rewind.
The pinned \textsf{librustzcash} implementation uses
$\mathsf{PRUNING\_DEPTH}=100$; this is an implementation assumption, not
consensus finality.
\href{https://zips.z.cash/zip-0218}{ZIP~218} recommends a $600$-block node
rollback window at $25$-second spacing, while retaining $100$-block
transparent coinbase maturity. This is a proposed node-policy target,
not a consensus finality rule or a verified change to the pinned wallet's
$100$-checkpoint cache. A wallet cannot infer rollback safety from the
coinbase maturity constant or from a server's ability to follow a deeper
reorganisation. Its checkpoints, nullifier history, scan queue, and restore
path must support its chosen recovery window together. Durable anchors
alone do not supply all of that state.
...
a recent anchor. That path, each entry of which is the root of a subtree of
public leaves, is the \emph{Ironwood Guide}'s material (\S``The note
commitment tree'', Lemma ``Authentication paths from public data''). The
deployed construction cuts the depth-$32$ tree at half depth into
height-$16$ shards under a height-$16$ cap, the wallet hashing its own shard
from scanned leaves and taking every other shard as a single downloaded
$32$-byte root.
...
$\texttt{SubtreeData}\{\allowbreak\mathsf{leadbyte},\allowbreak \mathsf{root},
\allowbreak \mathsf{nHeight}\}$ and
$\texttt{LatestSubtree}\{\allowbreak\mathsf{leadbyte},\allowbreak
\mathsf{index},
\allowbreak \mathsf{root},\allowbreak \mathsf{nHeight}\}$ records under
a \texttt{uint64\_t}
subtree index, fixes
$\mathsf{TRACKED\_}\allowbreak\mathsf{SUBTREE\_HEIGHT} = 16$, and
...
\texttt{IncrementalMerkleTree.hpp:\allowbreak 20--70,\allowbreak 409--411};
...
$\texttt{ShardTree}\langle\allowbreak \mathsf{Store},\allowbreak
\mathrm{DEPTH} = 32,\allowbreak
...
shards --- cuts the depth-$32$ tree of the \emph{Ironwood Guide}
(\S``The note commitment tree'') at half depth.
...
depth is the pinned wallet's assumed maximum reorganisation depth and
ZIP~307's historical cache-size guidance
(\S\ref{sec:sync-tree-linear}), not a NU7 consensus limit. The shorter
block spacing and proposed node rollback policy require those assumptions
to be reviewed together (\S\ref{sec:sync-reorg}); no automatic tripling of
these implementation constants is specified here.
...
\emph{Ironwood Guide}'s (\S``Authentication paths''); what
...
decryption on the device (\emph{Ironwood Guide}, \S``Trial decryption and
note acceptance'').
...
\S``Trial decryption and note acceptance''; witness maintenance over the
resulting positions (\S\ref{sec:sync-tree}) yields that volume's
authentication paths (\S``Authentication paths'').
