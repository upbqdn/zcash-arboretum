...
(\textsf{orchard}, \texttt{src/keys.rs}; \textsf{sapling-crypto}~0.7.0,
\texttt{src/zip32.rs}).
...
\texttt{src/keys.rs}; \textsf{sapling-crypto}~0.7.0,
\texttt{src/zip32.rs}).
...
followed by the value in $2$, $4$, or $8$ little-endian bytes --- and
typecode and length values MUST be at most $\mathtt{0x02000000}$
...
UAs derivable from a UIVK (\textsf{sapling-crypto}~0.7.0,
\texttt{src/zip32.rs}).
...
(\textsf{zcash\_}\allowbreak\textsf{address},
\texttt{kind/}\allowbreak\texttt{unified/}\allowbreak\texttt{fvk.rs},
\texttt{kind/}\allowbreak\texttt{unified/}\allowbreak\texttt{ivk.rs}).
...
the account (\textsf{sapling-crypto}~0.7.0, \texttt{src/zip32.rs}) ---
...
revisions. This volume documents Revision~0 as the deployed rule and
states Revisions 1--2 below; Revision~1 is implemented in the deployed
crates (\S\ref{sec:fees-librustzcash}), inert until NU6.3 bundles exist,
while Revision~2 remains \emph{specified, not deployed}.
...
memo bundles, dependent on the unmerged ZIP~248 transaction format) adds
\begin{align*}
...
(ZIP-317, Revision~2). Revision~1 is implemented in the deployed crates
(\S\ref{sec:fees-librustzcash}), inert until NU6.3 bundles exist;
...
\texttt{Cargo.lock}):
\begin{itemize}[leftmargin=*]
\item \emph{Sapling} (\textsf{sapling-crypto}~0.7.0, \texttt{builder.rs}):
...
\item \emph{Orchard} (\textsf{orchard}~0.15.0, \texttt{builder.rs}): a
...
transfers, as every bundle the deployed wallet builds does
...
$\texttt{DEFAULT\_MIN\_ACTIONS} = 2$ when nonempty) (\textsf{zcash\_primitives},
...
flag-dependent (\texttt{src/builder.rs}, \texttt{num\_actions}): when a
...
\texttt{WalletTransparentOutput::}\allowbreak%
\texttt{serialized\_size}). An \texttt{Unknown} input makes the fee
...
\texttt{data\_api/}\allowbreak\texttt{wallet/}\allowbreak%
\texttt{input\_selection.rs},
...
$\texttt{fee\_required}([\texttt{Known}(150)], [34], 0, 0, 0, 0) =
...
(\texttt{zcash\_client\_backend/}\allowbreak\texttt{src/}\allowbreak
\texttt{data\_api/wallet.rs}) ---
...
\textsf{zcash\_client\_backend}, \texttt{src/proposal.rs}), each
...
(\texttt{proposal.rs}, \texttt{Proposal::multi\_step}) rejects forward
...
\textsf{zcash\_client\_}\allowbreak\textsf{sqlite}
(\texttt{src/}\allowbreak\texttt{wallet.rs},
\texttt{get\_target\_}\allowbreak\texttt{and\_anchor\_}\allowbreak%
\texttt{heights}), the
heights are
\[
...
\texttt{ConfirmationsPolicy}; ZIP~315
...
shielding when the flag is set (\texttt{wallet.rs},
\texttt{ConfirmationsPolicy}).
\end{definition}
...
(\texttt{wallet.rs}, \texttt{propose\_transfer}) --- even when the notes
...
(\texttt{input\_selection.rs}): starting with no inputs, call the
...
(\texttt{selectable\_pool\_preference} and the \texttt{use\_pools} logic,
\texttt{input\_selection.rs}): the shielded pools are ordered with the
...
(\texttt{zcash\_client\_sqlite/src/wallet/common.rs},
\texttt{select\_spendable\_notes}).
...
each) (\texttt{fees/common.rs}, \texttt{single\_pool\_output\_balance}).
...
enforces with a runtime assertion (\texttt{input\_selection.rs}). Step
...
(\texttt{PaysTexFromShielded}, \texttt{wallet.rs}).

\paragraph{Shielding.} Function \texttt{propose\_shielding}
(\texttt{wallet.rs} and \texttt{input\_selection.rs})
...
whose one obligation is \texttt{compute\_}\allowbreak\texttt{balance}
(\texttt{zcash\_client\_backend/}\allowbreak\texttt{src/}\allowbreak%
\texttt{fees.rs}, \texttt{ChangeStrategy}). The
deployed
...
(\texttt{fees/zip317.rs}; the ZIP~317 parameter table). The Ironwood count
...
(\texttt{fees/common.rs};
\texttt{zcash\_primitives/src/transaction/builder.rs}, \texttt{get\_fee}).
\end{definition}
...
\texttt{single\_pool\_output\_balance} (\texttt{fees/common.rs})
...
change memo (\texttt{fees/common.rs}, \texttt{single\_pool\_output\_balance}).
Zero-valued change
...
forces a change output to exist (\texttt{fees/common.rs},
...
zatoshi (\texttt{fees.rs}, \texttt{DustOutputPolicy};
\texttt{fees/zip317.rs}; \texttt{fees/common.rs}). If the total change
falls below the
...
as a defence against value loss (\texttt{fees/common.rs}).
...
positive (\texttt{fees/common.rs}, \texttt{check\_for\_uneconomic\_inputs}).
Some dust rides for
...
500\,000$ zatoshi (\texttt{fees.rs}, \texttt{SplitPolicy};
\texttt{fees/common.rs}; \texttt{fees/zip317.rs},
\texttt{MultiOutputChangeStrategy}). The split count is
\[
...
(\texttt{fees.rs}, \texttt{SplitPolicy::split\_count}). If fewer than the
...
\texttt{src/data\_api/wallet.rs}, \texttt{create\_proposed\_transactions}).
Only
...
same height (\texttt{wallet.rs},
\texttt{build\_}\allowbreak\texttt{proposed\_}\allowbreak%
\texttt{transaction}). The mechanics
are those of the
...
the per-pool anchors, which selects \texttt{BundleType::DEFAULT} for Sapling
and Orchard alike; the Orchard builder exists only if NU5 is active at the
target height
(\texttt{zcash\_primitives/src/}\allowbreak%
\texttt{transaction/builder.rs}, \texttt{BuildConfig} and
\texttt{Builder::new}).
...
the NU6.3 (Ironwood) branch (\textsf{zcash\_primitives},
\texttt{src/transaction/mod.rs}, \texttt{suggested\_for\_branch}) --- and
the lock time of built transactions is $0$ (\texttt{builder.rs},
\texttt{build\_internal}).
...
\texttt{SpendInfo::new}) (\texttt{wallet.rs};
\textsf{orchard}~0.15.0 \texttt{src/builder.rs}). Payment outputs are added
...
(\texttt{wallet.rs}). The signing keys marshalled for the build
(\texttt{wallet.rs}): transparent secret keys derived per input
...
The deployed bundle builders are \textsf{orchard}~0.15.0 and
\textsf{sapling-crypto}~0.7.0, per \textsf{librustzcash}'s
\texttt{Cargo.lock} (workspace \texttt{Cargo.toml:66,69}); the line
references below are to the \textsf{orchard}~0.12.0 /
\textsf{sapling-crypto}~0.6.0 sources the section was written against,
whose padding logic 0.15.0 preserves under the deployed
cross-address-enabled flags: the requested-action count stays $\max(s, o)$
(floored at $2$ below), rising to $s + o$ only once NU6.3 forces
cross-address transfers off (\S\ref{sec:pczt-v2}).
...
\texttt{src/transaction/builder.rs}, \texttt{build} and
\texttt{build\_internal}):
...
(\texttt{builder.rs}, \texttt{value\_balance} and
\texttt{build\_internal}). The proposal's change outputs
...
\]
(\texttt{builder.rs}, \texttt{build\_internal};
\texttt{zcash\_primitives/src/transaction/sighash.rs}); the digest
...
cache it (\texttt{builder.rs}, \texttt{build\_internal};
\textsf{orchard}~0.12.0 \texttt{src/circuit.rs:787--793}). The Sapling
provers, by contrast, are
...
\texttt{decrypt\_output\_}\allowbreak\texttt{with\_key} under the
internal-scope $\ivk$, Sapling via
...
(\texttt{wallet.rs}, \texttt{create\_proposed\_transaction}). Transparent
...
(\texttt{wallet.rs},
\texttt{create\_pczt\_}\allowbreak\texttt{from\_proposal};
\texttt{zcash\_primitives/src/}\allowbreak%
\texttt{transaction/builder.rs}, \texttt{PcztParts} and
\texttt{build\_for\_pczt}).
...
throughout this section we document the \emph{deployed} surface --- the
\textsf{pczt} crate, version 0.7.0, in the librustzcash workspace, the
release ZIP-374 names for the v1 encoding --- and classify the draft's v2
material as \emph{implemented but unreleased}: the workspace head already
carries it as the crate's 0.8.0-\textsc{planned} changes (\textsf{pczt},
\texttt{CHANGELOG.md}; \S\ref{sec:pczt-v2}).
...
\textsf{pczt} 0.7.0 implements only v1, but the workspace head parses
both: \texttt{Pczt::parse} accepts version numbers $1$ and $2$, rejecting
any other with \texttt{ParseError::UnknownVersion}, and its
\texttt{serialize} emits the v2 encoding (\textsf{pczt},
\texttt{src/lib.rs}).
...
The workspace-head \texttt{Pczt} struct is
$\{\texttt{global}, \texttt{transparent}, \texttt{sapling},
\texttt{orchard}, \texttt{ironwood}\}$ with all four bundles non-optional
(\textsf{pczt}, \texttt{src/lib.rs}); the released 0.7.0 struct carries
the first three protocol bundles, the \texttt{ironwood} slot being among
the 0.8.0-\textsc{planned} additions (\S\ref{sec:pczt-v2}). The bundles
...
$133$, and building with no spends or outputs yields the $32$-byte
version-$2$ stream below --- the encoding the workspace crate's
\texttt{serialize} emits (computed by script against the \textsf{pczt}
...
(what \texttt{Pczt::serialize} always emits --- the latest version; v1
output requires \texttt{v1::Pczt::serialize}; \textsf{pczt}
\texttt{src/lib.rs}) wraps each of
...
\mathrm{false})$; builder options \texttt{with\_fallback\_lock\_time} and
...
together with the Sapling, Orchard, and Ironwood bundle metadata
...
the workspace pins version 0.7, same algorithm).
\end{construction}
...
methods (\textsf{zcash-devtool},
\texttt{src/}\allowbreak\texttt{commands/}\allowbreak\texttt{pczt.rs}
and \texttt{src/}\allowbreak\texttt{commands/}\allowbreak%
\texttt{pczt/}\allowbreak\texttt{sign.rs}).
...
conventions: \emph{implemented but unreleased} --- specified in the Draft
ZIP and present in the workspace \textsf{pczt} source as the crate's
0.8.0-\textsc{planned} changes (the v2 module with \texttt{Option}-wrapped
bundles and empty-bundle omission, the \texttt{ironwood} slot,
\texttt{note\_version}, and optional anchors), though absent from the
released \textsf{pczt} 0.7.0.
...
fabricated same-receiver spend --- and \textsf{librustzcash} now pins
\textsf{orchard} 0.15.0, ahead of the local 0.14.0 checkout the citations
trace (\textsf{orchard}, \texttt{src/pczt/verify.rs},
\texttt{io\_finalizer.rs}, \texttt{prover.rs}, \texttt{parse.rs}, and
\texttt{src/builder.rs}; librustzcash \texttt{Cargo.toml}). When we cite
\textsf{orchard} PCZT behaviour above, the claims are about the 0.14 line
of the local checkout, which the deployed 0.15.0 tracks, except where
marked.
...
\item \emph{Dummy-key rejection is not implemented in the reference
  ...
  path in the \textsf{pczt} Signer, through 0.7.0 and the workspace head,
...
\item \emph{Version skew.} The ZIP names \textsf{pczt} 0.7.0 as the
  ...
  checkout has caught up --- it ships 0.7.0 and builds against
  \textsf{orchard} 0.15.0 --- so the only residual skew is that the
  checkout's unreleased 0.8.0-\textsc{planned} changes (v2 serialization,
  the Ironwood bundle) go beyond the named release (ZIP-374, Reference
  implementation; librustzcash \texttt{Cargo.toml}; \textsf{pczt},
  \texttt{CHANGELOG.md}).
\end{itemize}
...
Sapling and Orchard nullifiers into the
...
\texttt{proto/}\allowbreak\texttt{service.proto};
\textsf{zcash-devtool},
\texttt{src/}\allowbreak\texttt{commands/}\allowbreak%
\texttt{wallet/}\allowbreak\texttt{send.rs}). Zaino implements the same
...
\emph{Config}). The deployed light-wallet stack defaults to a stricter expiry than the ZIP
requires. The constructor \texttt{Builder::new} sets
\[
...
differs from the target height (\textsf{zcash\_primitives},
\texttt{src/transaction/builder.rs}). A wallet that accepts the default
...
count --- a fixed depth for all transactions, so anchor choice does not
leak whether the notes being spent were trusted, the property ZIP-315's
anchor-selection recommendation secures. The exact heights differ by one,
however: ZIP-315 measures from the chain head, recommending the final
treestate of block $\mathrm{chainTip} - 3$, whereas the deployed anchor
sits at $H - c_{\mathrm{tr}} = \mathrm{chainTip} - 2$, one block shallower. The tree and anchor mechanics themselves are the
...
\texttt{with\_expiry\_height}, \emph{pending}, with polling ending at the
...
\S4.13 (Balance and Binding Signature, Sapling), implies construction may
...
(protocol specification, \S4.13; field sizes from \S7.1, ``Transaction
...
at version $0.8.0$; release $0.1.0$ (2024-08-20) factored it out of
...
this volume. The optional-amount semantics shipped in release $0.7.0$
(2026-04-23), so the published crate and the checkout now agree.
...
\texttt{Ok} of the payment otherwise (\textsf{zip321},
...
\texttt{wallet/}\allowbreak\texttt{input\_selection.rs} and
\texttt{src/}\allowbreak\texttt{proposal.rs}).
...
(\textsf{zcash\_client\_}\allowbreak\textsf{backend},
\texttt{proto/}\allowbreak\texttt{proposal.proto} and
\texttt{src/}\allowbreak\texttt{proto.rs}). This round-trip is why the
