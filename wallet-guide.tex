...
\thispagestyle{empty}
\begin{abstract}
Assuming the cryptographic primitives of the \emph{Crypto Guide} and the
Orchard protocol of the \emph{Ironwood Guide}, this volume of \emph{The Zcash
Arboretum} documents the deployed wallet layer above them: hierarchical key
derivation (ZIP-32),
...
\texttt{src/keys.rs}; protocol specification \href{https://zips.z.cash/protocol/protocol.pdf\#orchardkeycomponents}{\S4.2.3} and the
...
\href{https://zips.z.cash/protocol/protocol.pdf\#concreteprfs}{``Pseudo Random Functions''} and \href{https://zips.z.cash/protocol/protocol.pdf\#orchardkeycomponents}{\S4.2.3}; ZIP-32;
...
\href{https://zips.z.cash/protocol/protocol.pdf\#orchardfullviewingkeyencoding}{\S5.6.4.4}, \emph{Orchard Raw Full Viewing Keys}; \textsf{orchard},
...
\texttt{fees/standard.rs}, \texttt{fees/zip317.rs}). This is the standard,
default surface. A fixed-fee rule and its
\texttt{SingleOutputChangeStrategy} are also available behind the
\texttt{non-standard-fees} feature (\texttt{fees/fixed.rs}).
...
builds \emph{two} steps (\texttt{input\_selection.rs}, TEX proposal
construction; ZIP~320 \S~``Design considerations for
...
(\textsf{orchard}~0.15.0, \texttt{src/builder.rs} and
\texttt{src/bundle.rs}). With $s$ requested spends and $o$ requested
...
(\texttt{src/builder.rs}, \texttt{num\_actions}). The spend list is extended to
...
\texttt{build\_bundle}). For cross-address-disabled bundles the requested
...
\href{https://zips.z.cash/protocol/protocol.pdf\#orcharddummynotes}{\S~4.8.3}, ``Dummy Notes (Orchard)'';
\textsf{orchard}~0.15.0, \texttt{src/note.rs}, \texttt{src/tree.rs},
\texttt{src/note/nullifier.rs}, and \texttt{src/builder.rs}) is built from a freshly random
...
\textsf{sapling-crypto}~0.7.0 (\texttt{src/builder.rs}), a
...
(\textsf{orchard}~0.15.0, \texttt{src/builder.rs}, bundle finisher). In
...
output uses \texttt{v1::Pczt::serialize} (\textsf{pczt},
\texttt{src/lib.rs}).
...
the workspace pins version 0.7, same algorithm). Ironwood uses the Orchard
...
(\textsf{pczt}, \texttt{src/roles/updater/mod.rs} and \texttt{orchard.rs};
\textsf{orchard}, \texttt{src/pczt/updater.rs}).
...
\texttt{create\_ironwood\_proof(pk)} does the same for Ironwood (\textsf{pczt},
