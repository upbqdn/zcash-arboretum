...
\textsf{librustzcash} PR~\#2199, absent from both the baseline and current
\texttt{91f448b5} crates.
...
current ZIP-318 canonical-crossing Ironwood bundle's explicit unpadded choice
(\textsf{zcash\_client\_\allowbreak backend},
\texttt{fees/\allowbreak common.rs} and
\texttt{data\_api/\allowbreak wallet/\allowbreak input\_selection.rs}). Fee
...
(``Delivery: the note reaches its owner''); this volume takes that spendable
set as given.
...
The detailed source trace below was established at the volume baseline
\texttt{e30517e4}; current \textsf{librustzcash} \texttt{91f448b5} preserves
that pipeline and extends every pool-indexed step to Ironwood. A proposal now
...
the Orchard bundle using the same Orchard spend authority. Pool lists below
that name only Sapling and Orchard describe the pinned pre-Ironwood snapshot;
the invariants and role ordering apply unchanged to the added pool.
...
signature'', subsection ``What the signatures sign: the ZIP-244 digest and
its v6 form'')
...
At the pinned source-line baseline, the Orchard proving key was reconstructed
inside every \texttt{Builder::build} call by
\texttt{orchard::circuit::ProvingKey::build()} (\textsf{orchard}~0.12.0,
\texttt{src/circuit.rs:787--793}). Current \textsf{librustzcash}
\texttt{91f448b5} derives the Orchard/Ironwood circuit version once and reuses
...
The current \textsf{librustzcash} head (\texttt{91f448b5}) also contains the
...
\begin{remark}[Source-line baseline versus current builders]%
\label{rem:crate-versions}
The line-by-line derivation above was written against
\textsf{orchard}~0.12.0, but the volume baseline actually pins
\textsf{orchard}~0.15.0.  That release includes the NU6.3 bundle versions,
...
are deployed, not design-stage.  As a currentness check, the standalone
\textsf{orchard} head \texttt{38bd2274} is release 0.15.5, while the current
\textsf{librustzcash} lockfile at \texttt{91f448b5} resolves 0.15.3.  Neither
...
The ZIP fixes the v1 reference encoding to \textsf{pczt}~0.7.0.  The current
\textsf{librustzcash} head \texttt{91f448b5} ships
...
describe the current released surface; references to 0.7.0 identify only the
ZIP's byte-exact v1 baseline.
...
(ZIP-225; ZIP-229; protocol specification \S7.1, \emph{Transaction Encoding
and Consensus}; \textsf{zcash\_protocol}, \texttt{src/constants.rs};
\textsf{zcash\_primitives},
...
\emph{Ironwood Guide} assembles this tree in full (``What the signatures sign:
the ZIP-244 digest and its v6 form''); we do not re-derive it.
