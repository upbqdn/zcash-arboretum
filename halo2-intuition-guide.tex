...
that coin toss with a hash of the transcript (Fiat--Shamir, \S\ref{sec:fs}).
...
(\S\ref{sec:halo} says why, and what the fold's one expensive step
costs). The full fold and its soundness are the business of the
\emph{Halo~2 Guide} (\S``The inner-product polynomial commitment'').
...
\section{Proving a value sits in a table: lookups}\label{sec:lookups}
...
toy field would eat the soundness budget outright (\S\ref{sec:random}),
and at any scale balloons the quotient polynomial the prover must
split, commit, and open (\S\ref{sec:vanish}). Yet Orchard's circuit
...
\S\ref{sec:perm} machinery with one deliberate change: \emph{no
...
low-degree. The deployed declarations of both are exhibited in the
\emph{Halo~2 Guide}, \S``From statement to circuit: arithmetisation in
practice''; the exact constraints, the boundary row, the padding of
short columns, and multi-column tables are its \S``The lookup
argument''.
...
\section{Removing the verifier: the Fiat--Shamir transform}\label{sec:fs}
...
coins inside each opening round (\S\ref{sec:commit}). Nothing else about
him matters: his
...
that \S\ref{sec:commit} fought for is now enforced by construction: a
...
$z$ (\S\ref{sec:random}). She may grind through as many
...
deployed system does (the \emph{Halo~2 Guide}, \S``The complete Halo 2
protocol'', describes the transcript it uses).

\section{The ``Halo'' in Halo 2: deferring the expensive check}\label{sec:halo}

One cost has stayed quietly non-trivial. The opening proofs of
\S\ref{sec:commit} shrink by halving, so the verifier's rounds are
...
batch (the \emph{Halo~2 Guide}, \S``Accumulation and the Halo trick'').
...
move this guide keeps meeting: commit, draw a challenge, take a random
...
like the commitments it manages (the \emph{Halo~2 Guide},
\S``Accumulation and the Halo trick''; the cycle of curves that keeps
the recursive arithmetic in the right fields is its \S``Recursive proof
composition and accumulation schemes'').

A last remark completes the picture begun in \S\ref{sec:commit}: none of
...
(\S\ref{sec:lookups}; the \emph{Crypto Guide},
...
$x$''; the soundness of the accumulation fold sketched in \S\ref{sec:halo};
and the cycle of curves that makes that recursion efficient. This guide has supplied the thing those formalisms are
