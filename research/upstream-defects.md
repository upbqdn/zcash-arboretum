...
This file preserves dated findings against the explicitly named source
snapshots. It is not a list of confirmed defects in current releases, and
severity or exploitability claims require their own reproduction. Resolutions
are noted where checked. Later DRAFT entries remain research leads; publication
of this file is not evidence that an upstream issue was filed or accepted.
...
All three are interim states in the inspected trees. Repository contents
alone do not establish whether a separate issue tracker records them.
...
## 16. zebra-crosslink — handler/accessor disagreement (original ordering claim corrected)

**DRAFT. Severity: medium.**

**Title:** The decided-block handler uses the deepest supplied header, while
`finalization_candidate()` returns the tip; neither is the specified snapshot

**Body:**

The initial finding trusted an inverted header-order docstring. Re-reading the
producer at the same pin confirms that `FindBlockHeaders` yields ascending,
deepest-first headers `[T−σ+1, …, T]`. The handler at `lib.rs:543` therefore
selects `headers.first()` at `T−σ+1`; the unused accessor at `chain.rs:124-125`
selects `headers.last()` at `T`. The specified snapshot is the **parent** of
the deepest supplied header, at `T−σ`. For `σ=3`, the two selections are two
heights apart, while the accessor is three heights above the true snapshot.

Entries 23 and 32 give the corrected evidence. The original tip/deepest
attributions, missing parent operation, and three-height first/last gap were
errors in this dossier, not additional upstream defects. Entry 26 independently
explains the disabled height assertion: the stored candidate height is zero.
...
with trailing zero bytes appended to `.data`. Re-serializing that padded value
does **not** keep growing the file: it is already aligned, so the instruction
table stays at the same offset. The defect is loss of the original logical data
length; repeated growth and offset drift do not follow from this padding alone.
...
`version: 1` (hardcoded; the conversion has no version argument).
Validations 2/3/4 are absent.
...
snapshot at T−σ; off by σ = 3 blocks. It is σ−1 = 2 blocks above the
handler's selected header at T−σ+1. Corroboration: the wired-up validation
...
current stub) would finalize the tip, σ blocks too shallow. Latent at this pin
...
→ two placeholders pushed, then `assert_eq!` at `:564` compares the empty predecessor's
...
lightwalletd: FetchService resolves the hash, while lightwalletd explicitly
rejects unsupported hash requests
...
height gets the stale-height block on StateService while FetchService returns
the hash's block and lightwalletd rejects the unsupported request — a backend
divergence on identical requests.
...
orchardkeycomponents / ZIP 2005), so such an `ak` is not a valid Orchard
spending-key component. This is not, by itself, a claim that a generic RedPallas
signature under an odd-Y key fails its signature equation; the circuit and key
derivation enforce the Orchard restriction. No runtime impact today: the shipped code
...
## 43. orchard-zsa / librustzcash-zsa (QED-it forks) — legacy v5 parsing accepts reserved bit 2 without a bundle-format gate
...
successfully (the vanilla circuit also leaves ENABLE_ZSA unconstrained) rather
than being rejected under `bad-txns-v5-reserved-bits-nonzero` — an accept-vs-reject split
against the legacy format. A full-node acceptance or chain split has not been
...
SIDE B (historical comparison + upstream). Orchard `0.10.0`
...
**Failure mode.** The fork parser accepts bytes `0x04` through `0x07`, consuming
the reserved bit as `zsa_enabled=true` and round-tripping it through `to_byte`.
The vanilla circuit does not constrain ENABLE_ZSA. That establishes a parser
and circuit discrepancy, not full transaction or block validity: `0x04` also
disables both spends and outputs, which violates an independent bundle rule.
An otherwise valid signed transaction using `0x05`, `0x06`, or `0x07` and the
actual node's remaining validation path would be needed to demonstrate the
claimed consensus impact. Orchard `0.10.0` and the upstream Ironwood
`PreNu6_3` format reject the set reserved bit at parsing. The earlier blanket
claim of a demonstrated chain split was stronger than the supplied evidence.
...
`transaction/mod.rs:1037`); historical orchard `0.10.0` (`src/bundle.rs:63,131-133`);
...
## 52. zaino — `get_tree_state` before Sapling activation returns status OK, where lightwalletd walks the `z_gettreestate` SkipHash chain and returns `InvalidArgument`
...
**Failure mode.** A client that copies the hash from `GetLatestBlock` into a
hash-only `BlockID` (height zero) and passes it to lightwalletd `GetTreeState`
sends the reversed hash string to `z_gettreestate` and ordinarily gets
`InvalidArgument`/not-found; zaino resolves the same bytes to the correct block.
Passing the original `BlockID` unchanged does not demonstrate this: the
nonzero height takes precedence in lightwalletd's `GetTreeState`. The defect
is in lightwalletd, not zaino — logged so the byte-order review is on record.
