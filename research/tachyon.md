...
This inventory retains its July pins. A 2026-09-05 check confirmed ZIP 248
merged on 2026-09-03 (`e753a6a301912cf77202db8f0d840f4796f5cca1`);
the historical “unmerged ZIP 248” statements below are not current status.
The corresponding parked guide now corrects the missing scalar reduction,
nonzero-polynomial/nonidentity-commitment conflation, and unsupported
availability “pick two” and unconditional post-quantum privacy claims.
...
- Note expiry was considered and rejected as the pruning mechanism ("users must move their funds
  ...
  delegated absence proofs avoid periodic note expiry, subject to the
  concrete GGM index horizon (SpendableInit/SpendableLift/
...
- Epoch parameterization: the period the entire pruning model hinges on is untuned ("requires
  empirical analysis", issue #79); the book leaves GGM depth D and arity A
  symbolic, while the crate fixes provisional D=10, A=4 and 4096-block
  epochs (`src/constants.rs`, `src/keys/ggm.rs`); the eprint ties the
